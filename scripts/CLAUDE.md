...
The `outlook_calibration_backtest.py`, `outlook_pick_capital_hypothesis.py`, `outlook_preseason_backtest.py`, `outlook_pick_capital_dated_values.py`, and `outlook_strength_source_compare.py` (once `--players-cache` is supplied) scripts are read-only and safe anywhere. The rest touch the local DB or the network — don't run those against production; `outlook_pick_capital_capture.py` only ever reads Sleeper's public API, never writes anywhere.
