// Seeded mock league standing in for real Sleeper league + Elo ranking data.
// Each owner has their own ranking set: personal player values derived from a
// shared consensus base, positional leans, an age bias, and a stable per-owner
// jitter. Disagreements between two owners' boards are what make mutual-gain
// trades possible — the same mechanic the real FTF engine exploits.
...
const P = (id: string, name: string, pos: Position, nflTeam: string, age: number, base: number): Player => ({
...
export const LEAGUE_NAME = 'Lakeview Dynasty (mock)';
...
export function ownerValue(owner: Owner, player: Player): number {
...
export function boardFor(owner: Owner): Record<string, number> {
  return Object.fromEntries(PLAYERS.map((p) => [p.id, ownerValue(owner, p)]));
}
