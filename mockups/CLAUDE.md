...
|---|---|---|---|---|
| [`shop-a-player-403/`](shop-a-player-403/README.md) | #403 | 2026-08-27 | in flight | What "shop a player" looks like: entry row on the Matches long-press menu, a pushed screen with three mode chips (**Tier up · Tier down · Same value**), a `1 / X` horizontal card pager, a same-value-only position picker, and a deferred-write dismiss with Undo. 13 frames incl. loading, three empties, error, post-like and post-undo. **Embeds the real `screens/mobile/matches/populated--mutual.png` (2026-08-10) as §1's before-pane; every other frame is a labelled reconstruction** — `scope.md` §3 waives the capture rule (no single-pin / asset-ideas / player-menu capture exists). 18 design calls logged; three gaps found in the contract (give-side guard on the entry row, unspecified `relaxed` ideas, under-determined counter styling). Binding docs: `docs/feedback/items/403-shop-a-player/` |
