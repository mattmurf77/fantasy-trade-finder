...
`archive/retired-tooling/mobile/maestro/` are kept as historical artifacts — they document intended behavior;
they are never run.
...
> traps it hit, and why the flows in `archive/retired-tooling/mobile/maestro/` look the way they do. Do not
> execute any of it.

You are operating the mobile UI-test harness specced in
`docs/plans/archive/2026/mobile-testing/` (plan/prd/hld/lld/test-cases). All three gating
...
| "smoke", "everything", "full session", post-change regression | **app** | All of `archive/retired-tooling/mobile/maestro/flows/smoke/*.yaml` (10 flows); optionally the full authored set |

Map features → cases with `docs/plans/archive/2026/mobile-testing/test-cases.md` (feature
list: `app-inventory-2026-07-10.md` §7; check its drift ledger for states the
cases don't know yet). Existing flows: `archive/retired-tooling/mobile/maestro/flows/smoke/` (01–10)
and `archive/retired-tooling/mobile/maestro/flows/` (spike flows). Available testIDs: the registry in
...
- **Full authoring-law list lives in `archive/retired-tooling/mobile/maestro/README.md` § Flow-authoring
...
the drift ledger in `docs/plans/archive/2026/mobile-testing/test-cases.md` and, if you
