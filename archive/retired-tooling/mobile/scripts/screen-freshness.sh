#!/usr/bin/env bash
# Archived under D-056 (2026-08-15); retained for historical reference only.
echo "Retired: simulator tooling is archived; use current CI and TestFlight evidence." >&2
exit 2
...
if [[ ! -f "$MANIFEST" ]]; then
  [[ "$QUIET" -eq 1 ]] || echo "screen-freshness: no manifest at $MANIFEST — run archive/retired-tooling/mobile/scripts/screen-capture.sh" >&2
  exit 2
fi
...
for screen in sorted(data.get("screens", {})):
    ...
    print("STALE: %s (source changed since %s) — run "
          "archive/retired-tooling/mobile/scripts/screen-capture.sh --screen %s" % (screen, when, screen))
