...
def test_failed_fetch_not_cached_then_recovers(monkeypatch):
    ...
    monkeypatch.setattr(srv, "load_consensus_values", lambda scoring: {})
    monkeypatch.setattr(srv, "load_consensus_elo", lambda scoring: {})
    ...
    vals, elo = _dp_maps()
    monkeypatch.setattr(srv, "load_consensus_values", lambda scoring: dict(vals))
    monkeypatch.setattr(srv, "load_consensus_elo", lambda scoring: dict(elo))
...
def test_fetch_exception_not_cached(monkeypatch):
    ...
    monkeypatch.setattr(srv, "load_consensus_values", _boom)
    monkeypatch.setattr(srv, "load_consensus_elo", _boom)
...
def test_backoff_skips_refetch_within_window(monkeypatch):
    ...
    def _failing_vals(scoring):
        calls.append(scoring)
        return {}

    monkeypatch.setattr(srv, "load_consensus_values", _failing_vals)
    monkeypatch.setattr(srv, "load_consensus_elo", lambda scoring: {})
