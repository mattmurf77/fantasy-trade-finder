...
# V2 keeps unsealed private work separate from the active scope pointer. Old
# readers never interpret these pages as a v1 JSON artifact.
prepared_trade_manifests_v2_table = Table("prepared_trade_manifests_v2", metadata,
    Column("inventory_id", String, primary_key=True),
    Column("scope_key", String, nullable=False, index=True),
    Column("active_scope_key", String, unique=True),
    Column("user_id", String, nullable=False, index=True),
    Column("league_id", String, nullable=False),
    Column("state", String, nullable=False),
    Column("target_id", String, nullable=False, index=True),
    Column("generation_token", String, nullable=False),
    Column("created_at", String, nullable=False),
    Column("expires_at", String, nullable=False, index=True),
    Column("header_json", Text, nullable=False),
    Column("header_sha256", String, nullable=False),
    Column("metadata_json", Text),
    Column("metadata_sha256", String),
    Column("root_sha256", String),
    Column("tree_sha256", String),
    Column("card_count", Integer, nullable=False, default=0),
    Column("impression_count", Integer, nullable=False, default=0),
    Column("ghost_count", Integer, nullable=False, default=0),
    Column("node_count", Integer, nullable=False, default=0),
    Column("page_count", Integer, nullable=False, default=0),
    Column("adoption_token", String),
    Column("adoption_lease_until", String),
    Column("adoption_prefix", Integer, nullable=False, default=0),
    Column("adoption_ghost_prefix", Integer, nullable=False, default=0),
    Column("adoption_json", Text),
    Column("adoption_sha256", String),
)
prepared_trade_pages_v2_table = Table("prepared_trade_pages_v2", metadata,
    Column("inventory_id", String, primary_key=True),
    Column("kind", String, primary_key=True),
    Column("page_index", Integer, primary_key=True),
    Column("ordinal_start", Integer, nullable=False),
    Column("ordinal_end", Integer, nullable=False),
    Column("real_start", Integer, nullable=False),
    Column("real_end", Integer, nullable=False),
    Column("ghost_start", Integer, nullable=False),
    Column("ghost_end", Integer, nullable=False),
    Column("logical_bytes", Integer, nullable=False),
    Column("encoded_bytes", Integer, nullable=False),
    Column("payload_sha256", String, nullable=False),
    Column("payload_json", Text, nullable=False),
    Column("proof_json", Text),
)
prepared_trade_nodes_v2_table = Table("prepared_trade_nodes_v2", metadata,
    Column("inventory_id", String, primary_key=True),
...
_MODEL_CONFIG_DEFAULTS = [
    ...
    ("sweetener_gap_frac",       0.0,   "2026-09-02 #414 relative band: the gap sweetener triggers at max(sweetener_gap_threshold, frac * max(give, receive package value)) so the trigger scales with the trade; 0 = the absolute threshold alone (byte-identical)"),
    ("sweetener_best_effort",    0.0,   "2026-09-02 #414 best-effort fallback: 1 = when no single asset closes the gap under the trigger, attach the gate-passing asset that minimises the post-add gap (strictly narrower, richer side unchanged) and stamp gap_sweetener.partial; 0 = all-or-nothing (byte-identical)"),
