...
(2nd of 19 in "worth a 2nd", band [1400, 1575]), down-voted 17 times. Under the
...
SECOND_LO, SECOND_HI = 1400.0, 1575.0
...
def build_service(scoring_format: str = "1qb_ppr"):
    ...
    svc._elo_overrides = {
        ...
        "edge_lo":  SECOND_LO,          # 1400.0 — floor of "worth a 2nd"
...
def test_adams_17_downvotes_move_him_down_inside_his_tier():
    ...
    "worth a 2nd" [1400, 1575] must move him materially DOWN and must never
    leave the band."""
    cfg()
    svc, _ = build_service()
    moved = elo_of(svc)
...
def test_a_pin_in_a_band_gap_keeps_its_own_value_as_the_ceiling():
    ...
    BELOW the pin, so the band is widened to [1400, 1577]: he can fall through
