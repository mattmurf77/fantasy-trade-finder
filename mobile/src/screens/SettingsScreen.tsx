...
export default function SettingsScreen({ navigation }: any) {
  ...
  // even an option to flip." The state, the fetch, the optimistic PUT and the
  // `pick_pricing_mode_changed` emitter all died with the control; the
  // backend serves the fixed state on GET and 410s the PUT.
  ...
  // It is gone, not hidden: the operator ruled market pricing is "not an
  // opt-in or even an option to flip", so there is no state left to render
  // and no flag left to read. Both former render sites below keep the
  // stud-tax block alone.
