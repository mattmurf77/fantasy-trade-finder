...
| `OutlookBiasReceipt` | #231 deck bias receipt (v3 mock): one quiet line above the finder deck ("Leaning young + picks — you're Rebuilding · Change"); Change → `TradesHome` with `editDna:true` (auto-expands the hub's DNA editor). Self-contained (own session/flag/prefs reads on the shared `['league-prefs']` cache) so TradesScreen mounts it with a single line; renders null unless flag `trade.outlook_direction` is on AND the resolved outlook (declared, else inferred — "you look" copy) is directional (`not_sure`/none ⇒ nothing) |
