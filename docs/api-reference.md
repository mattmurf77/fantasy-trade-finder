...
| Method | Path | Purpose |
|---|---|---|
| GET | `/api/trio` | Next 3-player matchup. Generic pick rungs carry the same year-explicit `name` / `pick_value` `/api/rankings` serves (#207 — see that row) so a rung reads identically in a matchup and on a board |
