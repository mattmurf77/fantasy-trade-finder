...
| GET | `/api/trades/status` | Generation job status (completed snapshots carry per-card `impression_id` under `deck.signal_v2`, the F3 `suppression_note`/`retest` fields under `deck.fatigue`, and the F7 `wildcard` card under `deck.exploration`, same as generate) |
