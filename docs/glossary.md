...
**Tier band** — Bucket of similar Elo ratings rendered as a labeled, colored badge. Since 2026-07-12 (#117) the taxonomy is the **8-tier pick-value ladder**: `firsts_4plus` ("4+ 1sts", red) / `firsts_3` ("3 1sts", fuchsia) / `firsts_2` ("2 1sts", gold) / `first_1` ("1st", teal) / `second` ("2nd", sky) / `third` ("3rd", pink) / `fourth` ("4th", lime) / `waivers` ("Waivers", gray) — each floor a rung of the anchor/pick Elo ladder, so a tier name states what a player in it is worth in draft-pick terms. Bands are position- and format-uniform in Elo space (pick value is position-uniform); the consensus seed recalibration shipped with #117 (`data_loader.seed_elo_for_value`) lets top consensus assets actually reach the multi-first tiers (top asset ≈ 4 firsts). Retired: the abstract Elite/Starter/Solid/Depth ladder (2026-07-11) and the interim six-tier `firsts_2plus`/`bench` keys (2026-07-12). Canonical bounds in `backend/tier_config.json`; keys/colors in `docs/cross-client-invariants.md`.
