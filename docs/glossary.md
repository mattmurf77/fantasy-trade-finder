...
**Player-ID crosswalk** — The `{espn_id → sleeper_id}` mapping built from DynastyProcess `db_playerids.csv` (`espn_service.get_crosswalk`, 24h in-memory TTL, bundled-snapshot fallback), with a normalised name+position fallback tier. Measured 99.2% coverage on the value-relevant pool (plan §3). Everything downstream of import stores only Sleeper ids; a roster player who misses both tiers is **skipped and reported** in the link response's `report.unmatched` — never invented as a placeholder.

**Synthetic member id** — `espn:{SWID}` (fallback `espn:{league_id}.t{team_id}`): the `league_members.user_id` written for ESPN counterparties who aren't FTF users. Deterministic across re-syncs; must never reach push/notification paths. The linking user's own team carries their real FTF `user_id` (bound via `leagues.espn_my_team_id`).
