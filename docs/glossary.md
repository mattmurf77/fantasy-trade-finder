...
**Tier band** — Bucket of similar Elo ratings rendered as a labeled, colored badge. Since 2026-07-11 the taxonomy is the **pick-value tier ladder**: `firsts_2plus` ("2+ 1sts", gold) / `first_1` ("1st", teal) / `second` ("2nd", sky) / `third` ("3rd", pink) / `fourth` ("4th", lime) / `bench` ("Bench", gray) — each floor a rung of the anchor/pick Elo ladder, so a tier name states what a player in it is worth in draft-pick terms. Bands are position- and format-uniform in Elo space (pick value is position-uniform). Retired: the abstract Elite/Starter/Solid/Depth ladder (Bench survives as the below-4th floor). Canonical bounds in `backend/tier_config.json`; keys/colors in `docs/cross-client-invariants.md`.
...
**Pick-value scale** — 1.5.4 #111: a user's declaration of how many firsts a top-tier dynasty asset is worth (`top_tier_firsts`: 2/3/4, default 2 = the consensus math). Set from the pill row at the foot of the Pick Anchor wizard, persisted per user + scoring format (`users.anchor_scale`, `GET/POST /api/anchor/scale`). Re-spaces only the wizard's multi-first anchors along a power curve so the user's own "N firsts" answer pins to the default top-tier Elo (≈ 1789) while "1 first" stays the actual generic Mid 1st (Elo 1650); every consensus-denominated surface (pick pool assets, calculator pick-gap line, the tier-ladder band floors) is unaffected — see `docs/cross-client-invariants.md` § Pick anchor keys.

**Quick set (tiers)** — 1.5.4 #104: the guided tier-assignment walk (`QuickSetTiersScreen`, entered from the Tiers header). Per position, presents one tier at a time from the top of the ladder ("2+ 1sts") down to Bench with a tappable grid of small player cards (name + position + current tier); saving a tier commits just that tier through `POST /api/tiers/save` (composes with the existing board — only submitted pids change; re-visited deselections travel as `cleared_pids`) and advances. Players claimed by an earlier tier drop from later grids.
...
**Account** — The durable identity anchor introduced by the account-auth plan P2 (`accounts` table, `backend/accounts.py`). Created the first time a user signs in with Apple/Google; binds (stickily, at most once) to a `sleeper_user_id`, which stays the app's working key for all engine/ranking/match data. Survives device loss: signing in with the same provider on a new phone restores the bound Sleeper-keyed session. Deleted — along with all user-keyed data — by `DELETE /api/account` (App Store 5.1.1(v)).
