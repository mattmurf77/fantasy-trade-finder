...
| arm C (`trade_gen_v2`) | **follow-up — DONE 2026-08-21**, on `feat/gap-sweetener-arm-c` (stacked on this branch; see [scope-arm-c.md](scope-arm-c.md)). It was not the copy-paste this row assumed: the sweetener had to run inside `_pair_survivors` and rebuild the `_Candidate`, and the equalizer universe had to be arm C's SEMANTIC pools rather than its enumeration-budget slices — wired to the budget slices the pass is a measured no-op. | decided by effort: arm C has its own value math (`consolidated_value`, `gen2_consol_*`), its own candidate pipeline (`_pair_survivors`) and its own gate stack — the pass would need its own integration and re-verification of the gen2 goldens, for an arm that serves only quota-capped bake-off slots. Its gap cards are real (CHANGELOG 2026-08-21 names C), so this is a named follow-up, not a skip. |
...
| 6. Fixture-only measurement | **DONE 2026-08-21** — prod replay complete on league `1312140920132497408`; TEST_LEDGER addendum 2026-08-21b. Confirms the direction, halves the deck cost (−1.6%, not −3.9%), and adds two things: (a) arm C is worse on real boards than on the fixture (29.4% → 37.2% over the line), which **strengthens item 5's `bakeoff_include_gen_v2 = 0` fallback**; (b) the benchmark fix ALONE raises arm B's over-line share (6.7 → 11.3% on v3) — the fix and the sweetener are **not independently shippable**, so `sweetener_gap_threshold <= 0` is not a rollback lever for the benchmark fix. |
