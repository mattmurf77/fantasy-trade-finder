...
- **D-099 bake-off censoring (accepted cost):** prod `bakeoff_serve_interleaved` is **1.0** (read 2026-09-01 via the admin API; the code default and docstring say 0 — G-064) and `bakeoff_include_challenger` is 1.0, so arm D is **serving** and inherits this knob by design. Flipping the live row mid-window is a logged engine-affecting knob change that censors the current bake-off window at its timestamp per D-099. Accepted under the operator's "push live once built" instruction; the flip timestamp must be logged.
...
- [x] **Unit tests:** `backend/tests/test_consensus_fit_sort_key.py` (25 tests):
  ...
  - **D-159 junk guard** — on three harness fixtures (`12t_1qb@u0`, `12t_1qb@u8`, `mirror@b`), the sub-450 body share of consensus cards at `w = 0.5` ≤ the `w = 0` share + 2pp and the deck does not shrink (clock monkeypatched, G-065 (frozen-clock harness determinism; lands in this PR's living-memory)).
