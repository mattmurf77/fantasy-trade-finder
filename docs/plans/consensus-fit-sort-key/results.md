...
Every number here comes from a run on this branch tip with `PYTHONHASHSEED=0`, the clock frozen (`time.monotonic` → constant, G-065 (frozen-clock harness determinism; lands in this PR's living-memory)), the D-159 prod bundle pinned (`filler_min_frac` 0.15, `overpay_adjusted` 0, `trade_elo_gap_max` 0, `v3_shape_max_delta` 2) and `max_per_opponent=5`. **Every baseline was run twice and was byte-identical in all 120 cells of all three sweeps** (`baseline_identical: true` in every raw row).
...
* **It reorders, it does not open the floor:** uncapped, `w > 0` emits the same SET of consensus cards as `w = 0` in a different order — same set except where the gap sweetener's order-dependent `seen` dedupe substitutes one card for another (QA fuzz, 200 rosters: 198/200 same set with the sweetener at its live 1539, 200/200 with it off; mechanism at `trade_service.py:~7292` `if n_key not in seen` + `:~7299` `give_ids, recv_ids = n_give, n_recv` — whichever of two 1-for-1s enumerates first is swallowed into the sweetened pair and consumes the pair key, so the other survives as a plain 1-for-1); **card COUNT is unchanged.** Every card at every `w` still clears `rv − gv ≥ user_gain_epsilon`.
* **The sort key decides which combos fill the per-opponent cap — not the served order.** `_generate_trades_v2` re-sorts the whole deck by composite (`trade_service.py:4951`) after generation, so the brief's "the composite never reorders anything" is true of *which* cards exist and false of their on-screen order — except where composites tie: that `sorted` is stable, consensus composites tie often (QA: 63/120 pairs), and pool order then leaks into on-screen order through the ties (QA: 6/120 pairs same set, different order). The knob's deck-level footprint is therefore the change in the emitted set under the cap.
...
**QA-fix pass (2026-09-02, after review of PR #261)** — two guards QA found non-load-bearing, made load-bearing and proven the same way (`__pycache__` cleared, byte-copy restore `cmp`'d):

| # | Sabotage | Target test | Sabotaged | Restored |
|---|---|---|---|---|
| S-C | remove the `is_pick_asset` short-circuit in `_fit_sort_key` (`:7129-7131`). Previously uncaught because every fixture pick had `position == "PICK"`; the pick-neutrality fixture now carries `PKR` — a universal-pool generic rung, `team == "PICK"` with a real `RB` position — against a user roster with three RBs, so without the guard `marginal_value` hands it a −750 RB asymmetry and sinks it | `test_picks_keep_their_relative_order` | **1 failed** | 1 passed |
| F2 | remove the `consensus_fit_weight: 0.0` pin from `MODEL_A_PROFILE` | new `test_bakeoff_arm_a_golden.py::test_consensus_fit_pin_is_load_bearing` (mirror fixture under the arm-A overlay with the live row at 0.5: unmoved with the pin, moved with the pin temporarily removed) | **1 failed** — while `test_arm_a_reproduces_the_pre_wave_deck` stayed **green** on the same sabotage, which is exactly the gap: that fixture is fit-symmetric and cannot see the pin | 1 passed |

After the QA fixes (perf hoist of the two `replacement_levels` calls — zero calls at `w = 0`, two per pair at `w > 0`): `pytest backend/tests/test_consensus_fit_sort_key.py backend/tests/test_bakeoff_arm_a_golden.py backend/tests/test_engine_quality_golden.py -q` → **39 passed**; both goldens still byte-identical at the default.
...
3. **Deck-size check (QA note):** after the PUT, split `basis == "consensus"` impression COUNTS before/after. The gap sweetener's order-dependent `seen` dedupe is the one mechanism that can change deck size under this knob — it measured 0 at `w = 0.5` across 470 QA decks, but a −1-card deck appeared at `w = 0.25`, so it is not structurally impossible. A count delta is the sweetener interaction, not a gate moving.
4. **D-099 bake-off censoring — and a corrected premise.** Prod `bakeoff_serve_interleaved` is **1.0** (read 2026-09-01 via the admin API; the code default and its docstring say 0 — that is G-064) and `bakeoff_include_challenger` is 1.0, so arm D (`challenger`) is **serving**, not dark, and it inherits the live knob by design (no profile pin — § code-walk §5). A mid-week flip is therefore a logged, engine-affecting knob change that **censors the current bake-off window at its timestamp** per D-099 (split, keep both segments). This is an accepted cost of the operator's "push live once built" instruction, stated here so the Friday readout expects it; the flip's timestamp must be logged (`config_json` diff) like any other censoring change.
5. **Guardrails to read from the corpus after a week**, split by `basis == "consensus"` as D-095's C2 prescribes: sub-450 share of consensus impressions (expect flat), consensus like rate (the hypothesis is up, on the strength of #96/#304 feedback), and `receive_positions` mix vs `give_positions` on consensus rows for viewers with no need position.
