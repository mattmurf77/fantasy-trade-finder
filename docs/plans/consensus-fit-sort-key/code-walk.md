...
* The blend touches **only the two `sort(key=…)` calls**. `_emit` (`:7195-7333`) is unchanged from `:7131-7262` on main: `package_value_v2` on both sides, `rv − gv ≥ user_gain_epsilon` (`:7227`), `consolidation_raw_loss_frac`, `user_gain_ok_1for1`, `pick_swap_ok`, `filler_ok`, the G6 hook, the fairness ratio, the gap sweetener. A reorder cannot admit a combo the gates would have rejected. [`test_sign_test_still_holds_on_every_card`] checks `receive_value − give_value ≥ ε` on every emitted card at `w ∈ {0, 0.25, 0.5, 1}` in both profile modes; [`test_knob_half_set_of_cards_is_the_same_as_knob0_uncapped`] shows the uncapped SET is unchanged and only the ORDER differs on the mirror fixture. That is not universal: with the live gap sweetener on, the emitted set is the same **except where the sweetener's order-dependent `seen` dedupe substitutes one card for another** — at `:~7292` (`if n_key not in seen`) and `:~7299` (`give_ids, recv_ids = n_give, n_recv`), whichever of two 1-for-1s enumerates first is swallowed into the sweetened pair and consumes the pair key, so the other survives as a plain 1-for-1 (QA fuzz: 198/200 rosters same set with the sweetener at 1539, 200/200 with it off). **Card COUNT is unchanged.**
