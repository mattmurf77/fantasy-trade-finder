...
☐ expo-updates spike done; decision + reasoning in `living-memory/DECISIONS.md` (OI-9). *If "adopt": stop, re-derive this plan from the new premise.*
☐ Hermes provides `TextDecoder` — checked **on a phone**, evidence in TEST_LEDGER — or the hand-rolled-decoder cost is accepted in writing (OI-12).
☐ OI-14 answered (vault wipe-vs-null deviation from PRD:144).
