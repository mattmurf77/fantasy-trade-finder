...
| `living-memory/DECISIONS.md` | **D-028** (HLD §7 — *not* `D-011`; root `CLAUDE.md`'s next-id column is stale per HLD §10.4): sentinel-not-flag routing predicate, `LinkSleeperSheet` extraction, no new flag. Records the rejected non-dismissible-sheet-over-`Main` alternative. |
| `living-memory/GOTCHAS.md` | Candidate: `GET /api/sleeper/leagues/acct_<id>` proxies a synthetic id to Sleeper — 503 `sleeper_unavailable` under the VCR harness, `null` (→ "No 2026 NFL leagues found") live. An account-only session must never trigger the Sleeper league fetch. *(W3-DOCS allocates the id; HLD §7 assigns G-027…G-029 to other findings, so this one is additive or folded into the LLD.md row at W3-DOCS' discretion.)* |
