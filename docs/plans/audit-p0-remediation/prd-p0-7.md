...
| `living-memory/DECISIONS.md` | **Updated — `D-030`** (id allocated by HLD §7; the "next id `D-011`" in five plans and in root `CLAUDE.md` is stale, per HLD §10.4) | P0-7's two non-obvious choices: (1) adopting the reserved `sleeper_send_*` names over `send_in_sleeper_*`, lighting up WAT / funnel stage 8 / the `send_in_sleeper` vertical; (2) the client/server split — success server-authoritative, attempt and failure client-side. |
| `living-memory/GOTCHAS.md` — **`G-029`** | **Updated** | A client `track()` name absent from `analytics_taxonomy.py` is counted and dropped **in silence** behind a 200 — third occurrence in this repo. |
