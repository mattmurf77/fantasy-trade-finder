...
| the 17 `market_floor_*` / `conf_source_*` / `deck_core_*` / `policy_*` / `conviction_deck_share` / `market_core_ratio` / `personal_gain_min_frac` knobs | personal-market policy (docs/plans/personal-market-policy/scope.md), 2026-09-04 | **EXCLUDED from MODEL_A_PROFILE — structurally unreachable, not merely identity-valued.** Every one of them is read only from `backend/trade_policy.py`, and every entry point into that module is behind `trade.valuation_telemetry` or `trade.personal_market_policy_v1`, both of which default false in code AND in `config/features.json`. `FLAGS` is a process-global proxy, so no `_cfg_override` overlay can reach them and no pin could change what arm A does; with the flags off there is no read at all, and with them on the policy is a whole-job property that applies to every arm alike (that is the point of keeping `policy_variant` orthogonal to `model_arm`). They are in `_PINNED_KNOBS` — the inventory guard — but not in the profile. Nothing to re-capture: the arm-A golden cannot see a knob no arm-A code path reads. `MODEL_CHALLENGER_PROFILE` does not pin them either, for the same reason. **If the policy is ever promoted to default-on, revisit this row**: arm A would then be evaluated under a policy that post-dates it, and the honest fix is a `r4_bypass()`-style thread-local policy bypass rather than a profile pin. |
