...
| Traded-pick awareness (so a derived order isn't wrong when picks changed hands) | **NOT FEASIBLE — live-confirmed** | ESPN's v3 API carries no persistent future-draft-pick-ownership model for dynasty leagues (confirmed by our own prior spike, `docs/plans/archive/2026/espn-league-linking-plan-2026-07-11.md` §2, `#158`, and live probe §6e) |
...
- `backend/database.py:1213-1246` — `espn_credentials_table`: one row per
  ...
  paste of the user's own espn.com session cookies (`docs/plans/archive/2026/espn-league-linking-plan-2026-07-11.md` §4)
...
- `docs/plans/archive/2026/espn-league-linking-plan-2026-07-11.md` — the original ESPN
...
- `docs/plans/archive/2026/draft-extensions/plan.md` §0 point 6 — independently names the
...
- **Persist / expose:** a new nullable set of columns or a small side table
  ...
  (`docs/plans/archive/2026/rookie-draft/`) ends up using for ESPN (currently: none —
  ESPN has no pick model in FTF at all, per `docs/plans/archive/2026/rookie-draft/plan.md`
...
| Prior-season auth/permission constraints | — | Same cookie model as the current season (public → no auth; private → `espn_s2`+`SWID`); **but ESPN purges old league data.** Our own plan doc's live probes (2026-07-11) found three known historical public test-league ids **all 404 on both the season and `leagueHistory` endpoints** — confirmed empirically, not just documented | `docs/plans/archive/2026/espn-league-linking-plan-2026-07-11.md` §1, "Data purging" row, "Live probes run for this plan" — **our own verified data**, most authoritative source in this table |
...
1. **Spike first, scope second.** Before writing the ordering algorithm,
   ...
   team already applied to the `mDraftDetail` probe (`docs/plans/archive/2026/draft-extensions/plan.md` §0.6, S-1).
...
- `docs/plans/archive/2026/espn-league-linking-plan-2026-07-11.md` (original go/no-go —
...
- `docs/plans/archive/2026/draft-extensions/plan.md` (current draft-surface roadmap;
  independently flags the same `mDraftDetail` verification gap)
- `docs/plans/archive/2026/rookie-draft/plan.md`, `docs/glossary.md` (confirm ESPN has no
