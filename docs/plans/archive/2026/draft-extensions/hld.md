...
| **I-10 · The mock's calibration artifact** | W2 calibration batch | W2 mobile, the reviewer | Fitted noise parameters, the hold-out split definition, the numeric pass threshold, the achieved score, and the pass/fail verdict — written into `docs/plans/archive/2026/draft-extensions/` as a durable file. **Its failure is W2's abort criterion**, so it is a gate artifact, not a report. |
