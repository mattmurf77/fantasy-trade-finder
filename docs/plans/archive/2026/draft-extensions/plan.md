...
2. **The mock draft feature does not exist.** No service, table, or flag. `docs/plans/archive/2026/rookie-draft/mock-draft-plan.md` is an **untracked, local-only** reviewed draft. So item 3 is build-from-zero, not an entry-point wire-up.
...
Adopt `docs/plans/archive/2026/rookie-draft/mock-draft-plan.md` §4–9 — **now tracked on main (`dc32c2a`); it had been untracked local-only, so the wave it scopes had no spec to read** — with three risk-lens amendments that are binding:
