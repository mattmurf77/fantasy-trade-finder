...
`archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml` photographs it as
...
- `archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml` is renamed
...
| `living-memory/DECISIONS.md` | **D-026** — first-use-wins over last-use-wins (the re-lock hazard already documented at `server.py:6177-6183`); the single `'anchor'` → tiers/quickset upgrade and why it is strictly improving; the rookie-scope / `rookie_ranks` / `draft_room` exclusions; backfilling to `'quickset'` rather than `'tiers'` and the labelling assumption that carries; startup migration over lazy on-read repair or a one-shot script; and the **suppressed fan-out** with its permanent consequence (§8). |
...
| `living-memory/GOTCHAS.md` | **Conditional** — only if the build loses >30 min to something new. The known trap (a startup backfill mutating seeded UI-test fixtures) is covered by the runbook row. Next free id is **G-029**, and `hld.md` §7 already assigns G-029…G-031 to P0-2/P0-6/P0-8-9, so W3-DOCS allocates any P0-1 entry after those. |
...
| **C-2** | **Analytics discontinuity.** Method-segmented series show a step change across the deploy boundary: the NULL bucket collapses and `'quickset'` jumps. | `hld.md` R16. CHANGELOG + D-026. There is no way to backfill "which method did they actually use" retroactively. |
| **C-3** | **The `'quickset'` label is an assumption.** `'quickset'` and `'tiers'` are behaviourally identical at the ladder (`server.py:6165`); the label is chosen because the default route lands on QuickSetTiers, so the overwhelming majority of NULL-method tier boards were built there. | Recorded in D-026 (waiver W-5). |
