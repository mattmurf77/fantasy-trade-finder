...
**How to run it: the flow already exists.** `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` is tagged `# flags: onboarding-v2` and `# profile: fresh`, walks S0 → S8, and declares `# captures: s2-wait, s2-1, s2-2, s2-3, s3-1, s3-2, s4-1, s5-1, s5-0, s5-5, s6-1, s6-2, s8-1` — **that is the 13-beat tour**. `archive/retired-tooling/mobile/scripts/screen-capture.sh` reads the `# flags:` header (`:113`) and passes `--flags backend/tests/fixtures/flags/onboarding-v2.json` to `archive/retired-tooling/mobile/scripts/sim-run.sh`, which merges it over the profile's seeded map (`sim-run.sh:56-68`) and round-trips the pinned map through `/api/feature-flags` as a handshake (`:118-119`). So flag pinning is solved and needs no new machinery.
...
| 3 | `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` | S8.1 header comment | Comment only: record that s8.1 now additionally requires `guideSeen['s2.2']`, so the flow's own s2-2 step is a precondition of its s8-1 step, not just an ordering coincidence. |
...
| 7 | `archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml` | **New** — the P0-8 regression under release flags (§6) | new Maestro flow |
| 8 | `docs/plans/archive/2026/audit-p0-remediation/scope-p0-8-9.md` | The feature scope block | doc |
...
Both halves, per the brief. Authored to `archive/retired-tooling/mobile/maestro/README.md`'s flow-authoring laws — **id selectors only**, no text matching, no bare sleeps.

### 6.1 Release-flag regression — NEW: `archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml`
...
`archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` already is the flag-pinned 13-beat flow (§3.1). Run it via `archive/retired-tooling/mobile/scripts/screen-capture.sh`, which resolves `# flags: onboarding-v2` → `backend/tests/fixtures/flags/onboarding-v2.json` → `sim-run.sh --flags`. Delta:
