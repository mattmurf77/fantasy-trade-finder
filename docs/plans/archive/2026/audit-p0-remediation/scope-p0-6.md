...
- [x] **New flow:** `archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml` — profile `espn`, flags
...
- [x] **Extended flow:** `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml` — add
...
- **Capture delta:** `matches` under the **`espn` profile only** — `matches__populated--espn-mutual`
  ...
  `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen matches` (see `docs/runbook.md` § Screen library).
  ...
  **Sleeper-profile captures must be pixel-identical** — that is the primary regression assertion.
  Run `archive/retired-tooling/mobile/scripts/screen-freshness.sh` and expect it to flag ESPN screens only.
...
- **Simulator-gate tier** (per the matrix in `docs/runbook.md` § Pre-ship simulator gate):
  **Tier 1 — mobile screen / navigation / state change.** Required: full smoke suite (11 flows)
  ...
  + `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen matches` for the ESPN-profile frames whose visuals
  changed.
