...
Design detail: docs/plans/archive/2026/audit-p0-remediation/plan-p0-3.md
-->
...
- **New flow:** `archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml` (`# flags: release`) —
  three blocks:
  ...
  (`archive/retired-tooling/mobile/maestro/README.md:140-146`, `mobile/src/utils/testRouteEntry.ts:14-18`):
...
- **Capture delta:** `signin`, `leaguepicker`, and the new `leaguejoin` screen — run
  `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen <x>` per `docs/runbook.md` § Screen library.
...
- **Simulator-gate tier:** **Tier 1** — mobile screen + navigation + state change
  ...
  `flows/league/invite-join.yaml`, plus `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen` for
