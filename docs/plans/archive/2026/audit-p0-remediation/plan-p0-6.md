...
**8. `archive/retired-tooling/mobile/maestro/…`** — see §5.
...
### 5.2 `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml` — must be updated *and* re-captured
...
### 5.3 New flow: `archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml`
...
- `archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml` — green (§5.3).
- `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml` — green with the new assertion, and its two shutters
  re-captured.
...
- **Sleeper league, unchanged** — the highest-value regression check. `matches` (profile `standard`)
  and `trades` captures must be pixel-identical; run `archive/retired-tooling/mobile/scripts/screen-freshness.sh` and expect
