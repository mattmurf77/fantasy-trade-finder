...
- `mobile/src/api/client.ts` builds `Error.message` for a failed request from
  ...
  (confirmed by the capture flow's own note, `archive/retired-tooling/mobile/maestro/capture/trades.yaml:70-76`).
...
**No Rank-tab error state carries a testID.** `docs/plans/archive/2026/mobile-testing/lld.md` Appendix A
...
### `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` *(new)*
20. See [Maestro delta](#maestro-delta).

### `screens/mobile/trades/`
21. Re-capture (`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen trades`): `error.png` changes,
...
driven from a flow by `archive/retired-tooling/mobile/maestro/capture/helpers/inject.js`. Two properties make all
...
### New flow: `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml`

Preamble copied verbatim from `archive/retired-tooling/mobile/maestro/flows/smoke/05-trades-render.yaml`
...
copy, which `docs/plans/archive/2026/mobile-testing/lld.md:253` permits. No fixed sleeps, no coordinate
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen trades` at ship. `error.png` changes materially;
...
4. `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` — all six legs pass.
5. `archive/retired-tooling/mobile/maestro/capture/trades.yaml` — passes with the updated error leg (it fails
...
on sim, **plus** `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen trades`. Log to
