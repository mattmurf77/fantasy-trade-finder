...
> `archive/retired-tooling/mobile/maestro/README.md` flow-authoring laws 1-23, `docs/runbook.md` § Pre-ship
> simulator gate.
...
| `archive/retired-tooling/mobile/maestro/flows/p0-1-quickset-unlock.yaml` **(new)** | P0-1 | |
| `archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml` | P0-1 | rename `--4-4-locked` → `--4-4-unlocked` (S-07) |
...
| `archive/retired-tooling/mobile/maestro/flows/p0-5-account-only-picker.yaml` **(new)** | |
| `archive/retired-tooling/mobile/maestro/capture/leagues@account-only.yaml` **(new)** | |
...
| `archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml` **(new)** | |
| `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml` | + `assertVisible: id: send-in-sleeper.copy` (S-28); comment rewrite |
...
| `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` **(new)** | six legs, Paths A/B/C + retries |
| `archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml` **(new)** | fails on the unfixed tree, passes on the fixed one |
| `archive/retired-tooling/mobile/maestro/capture/trades.yaml` | **mandatory** — its error leg asserts the bug and breaks the moment the fix lands |
| `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` | S8.1 comment + the **D3 `s5.1` validation variant** (S-43) |
...
| `archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml` **(new)** | three blocks incl. the signed-out landing |
...
> **Waiver condition (S-46 / P0-7 §7):** `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` and
> `archive/retired-tooling/mobile/maestro/flows/smoke/09-league.yaml` **must pass unmodified**. They are the
...
| `IS_TEST_BUILD` (client) | `Constants.expoConfig.extra.testMode`, baked at build time by `app.config.js` from `FTF_ENV=test` | produced solely by `archive/retired-tooling/mobile/scripts/sim-build.sh`; `false` in every EAS bundle; there is no runtime path that can set it |
...
| # | File | Owner | Proves |
|---|---|---|---|
| 1 | `archive/retired-tooling/mobile/maestro/flows/p0-1-quickset-unlock.yaml` | W1-BE | 4/4 ring **and** `rank.unlocked-banner` in one session (simultaneity is the acceptance criterion) |
| 2 | `archive/retired-tooling/mobile/maestro/flows/p0-5-account-only-picker.yaml` | W1-P05 | brand-new Apple sign-in reaches the platform choice with no Settings visit; relaunch leg; `assertNotVisible tab.trades` |
| 3 | `archive/retired-tooling/mobile/maestro/capture/leagues@account-only.yaml` | W1-P05 | screen-library frame for the new visual state |
| 4 | `archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml` | W1-P06 | stated reason **and** useful action on an ESPN match |
| 5 | `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` | W2-TS | Paths A/B/C each produce a named persistent state with a working retry |
| 6 | `archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml` | W2-TS | **fails on the unfixed tree** — the only true regression test in the batch |
| 7 | `archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml` | W2-P03 | authed pin / authed non-member / signed-out banner (block 3 needs §5) |

### Modified

| # | File | Owner | Why mandatory |
|---|---|---|---|
| 8 | `archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml` | W1-BE | its whole header argues the 4/4-but-locked contradiction; rename the capture id and re-justify the `league.works-now` step |
| 9 | `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml` | W1-P06 | its `assertNotVisible "Send in Sleeper"` passes before **and** after; add the positive assertion (S-28) or lose the detector |
| 10 | `archive/retired-tooling/mobile/maestro/capture/trades.yaml` | W2-TS | its error leg waits for `trades.empty-text` to reappear — it **breaks the moment the fix lands** |
| 11 | `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` | W2-TS | S8.1 precondition comment + the **D3 `s5.1` variant walk** (real chip selections so `fresh > 0`) |
...
| # | File | Guards |
|---|---|---|
| 13 | `archive/retired-tooling/mobile/maestro/capture/leagues@fresh.yaml` | the non-account-only empty state did not move (asserts its literal sentence) |
| 14 | `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` | P0-7's Maestro waiver — drives every `tabPress` handler it edits |
| 15 | `archive/retired-tooling/mobile/maestro/flows/smoke/09-league.yaml` | P0-7's Maestro waiver — mounts both League screens |
...
`mobile/src/screens/RankScreen.tsx`, `archive/retired-tooling/mobile/maestro/flows/p0-1-quickset-unlock.yaml`,
`archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml`.
...
`mobile/src/components/Toast.tsx`, `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml`,
`archive/retired-tooling/mobile/maestro/capture/trades.yaml`.
...
`mobile/src/api/league.ts`, `archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml`.
...
`archive/retired-tooling/mobile/maestro/flows/p0-5-account-only-picker.yaml`,
`archive/retired-tooling/mobile/maestro/capture/leagues@account-only.yaml`.
...
`archive/retired-tooling/mobile/maestro/flows/p0-6-espn-copy-trade.yaml`, `archive/retired-tooling/mobile/maestro/capture/matches@espn.yaml`,
...
`archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml` (new),
`archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml`.
