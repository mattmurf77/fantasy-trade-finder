...
(matches in `docs/plans/archive/2026/audit-p0-remediation/*` and `docs/business/product/2026-08-09-mobile-ux-audit/*`
...
|---|---|---|---|
| `capture/onboarding-signin@fresh.yaml` | `fresh` | `s0.1`, `s0.2`, `s0.err-notfound`, `s0.err-down` | `archive/retired-tooling/mobile/scripts/screen-capture.sh --flow capture/onboarding-signin@fresh.yaml` |
...
```
archive/retired-tooling/mobile/scripts/screen-capture.sh --flow capture/onboarding-tour@fresh.yaml --interactive --state s4-1
```
...
Two files. Both conform to `archive/retired-tooling/mobile/maestro/README.md`'s flow-authoring laws 1-23; the laws each
one turns on are named inline.

### 7.1 NEW — `archive/retired-tooling/mobile/maestro/flows/guide-no-false-signoff@release.yaml`
...
### 7.2 MODIFIED — `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml`
...
`archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` and `archive/retired-tooling/mobile/maestro/flows/smoke/09-league.yaml` are P0-7's
...
- **Simulator gate: tier 1** for the batch (HLD §4 Wave 3), one run covering all seven findings:
  ...
  `archive/retired-tooling/mobile/scripts/screen-freshness.sh` to confirm nothing else drifted. The V2 captures are re-run
