...
Companion plan: docs/plans/archive/2026/audit-p0-remediation/plan-p0-5.md
-->
...
- **New env vars / `model_config` keys:** **none.** The Maestro seam reuses the existing
  ...
  `mobile/src/utils/testRouteEntry.ts:51-56`, set only by `archive/retired-tooling/mobile/scripts/sim-build.sh`). No new
  knob, and nothing new that could be set in Render.

## 3. Test scope (mobile test platform)

- [x] **New flow:** `archive/retired-tooling/mobile/maestro/flows/p0-5-account-only-picker.yaml` — covers: brand-new Apple
  ...
  *not* visible (did not route to `Main`), asserts the old false "No 2026 NFL leagues found" /
  ...
  `plan-p0-5.md` §5.
- [x] **New capture:** `archive/retired-tooling/mobile/maestro/capture/leagues@account-only.yaml` — new visual state of an
  existing screen, one screenshot, same launch-argument entry.
- [x] **Extended flow:** none extended. `archive/retired-tooling/mobile/maestro/capture/leagues@fresh.yaml` is
...
- **Capture delta:** `leagues` (new companion state + unchanged existing states), `settings`
  (post-`LinkSleeperSheet` extraction). Run `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen leagues`
  and `--screen settings`.
...
- **Simulator-gate tier** (matrix in `docs/runbook.md` § Pre-ship simulator gate): **Tier 1** —
  ...
  `p0-5-account-only-picker.yaml` flow**, plus `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen leagues`
