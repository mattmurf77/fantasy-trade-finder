...
| A-4 | `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` | all six legs pass |
| A-5 | `archive/retired-tooling/mobile/maestro/capture/trades.yaml` | passes **after** the §9.3 edit; **fails against the fix if left as-is** — which is itself the regression proof |
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen trades` + `screen-freshness.sh`. Evidence to
