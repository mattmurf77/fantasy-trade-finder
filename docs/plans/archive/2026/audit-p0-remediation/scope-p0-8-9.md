...
Companion plan: docs/plans/archive/2026/audit-p0-remediation/plan-p0-8-9.md
-->
...
- **Smoke-suite impact:** the six flows in `archive/retired-tooling/mobile/maestro/` (`01-launch` … `06-tiers-drag-no-crash`) plus the wider smoke set cross `TradesScreen`. None asserts on guide bubbles, so all are expected green; the Tier-1 gate runs them anyway.
...
- **Simulator-gate tier:** **Tier 1** — *"Mobile screen / navigation / state change"*. P0-8 changes guide state behaviour on `TradesScreen`, the app's most-trafficked screen. Required before merge to `main`:
  ...
  - `archive/retired-tooling/mobile/scripts/screen-capture.sh` — **not required**; no release-path visual changes. Run `archive/retired-tooling/mobile/scripts/screen-freshness.sh` to confirm nothing else drifted.
