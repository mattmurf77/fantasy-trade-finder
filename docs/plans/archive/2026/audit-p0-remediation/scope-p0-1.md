...
Companion plan: docs/plans/archive/2026/audit-p0-remediation/plan-p0-1.md
-->
...
- [x] **New flow:** `archive/retired-tooling/mobile/maestro/flows/p0-1-quickset-unlock.yaml` — covers
...
- [x] **Extended flow:** `archive/retired-tooling/mobile/maestro/capture/league@quickset-done.yaml` —
...
- **Capture delta:** `league@quickset-done` (renamed capture — must be re-run
  ...
  is a `testID` prop. Run `archive/retired-tooling/mobile/scripts/screen-freshness.sh` and re-capture
  only what it flags.
...
- **Simulator-gate tier** (matrix in `docs/runbook.md` § Pre-ship simulator gate):
  ...
  plus `archive/retired-tooling/mobile/scripts/screen-freshness.sh` with re-capture of whatever it
