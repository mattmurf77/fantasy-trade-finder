...
- [x] **WAIVED because:** **the change is not user-visible.** No rendered element, copy,
  ...
  1. **Existing flows must pass unchanged and unmodified** — specifically
     `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` (drives every `tabPress` handler this
     change edits) and `archive/retired-tooling/mobile/maestro/flows/smoke/09-league.yaml` (mounts both League
...
- **Capture delta: none** — no visual change, so no `archive/retired-tooling/mobile/scripts/screen-capture.sh` run.
  `archive/retired-tooling/mobile/scripts/screen-freshness.sh` is run as part of the tier-2 gate; it is expected to
...
- **Simulator-gate tier:** **Tier 2** — *"Mobile logic touched, no UI change"*
  ...
  `archive/retired-tooling/mobile/scripts/screen-freshness.sh`, re-capturing **only** what it flags (expected:
  nothing).
