...
| **NFR-6** | **Hermetic harness safety:** no code path added by this build may cause a Sleeper fixture miss during a sim run (`vcr_misses > 0` fails `archive/retired-tooling/mobile/scripts/sim-run.sh:178`). Enforced by the single-call-site rule in LLD §2.0. |
...
| `living-memory/DECISIONS.md` | **D-028** (id per HLD §7) — legacy `/?league=` parsed forever; a 302 into the existing web landing rather than a new page; a 14-day persisted invite intent instead of `invitedBy`'s in-memory-only lifetime. | W2-P03 |
