...
`archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml` (new).
...
`archive/retired-tooling/mobile/scripts/sim-run.sh:178`:
...
**One new file: `archive/retired-tooling/mobile/maestro/flows/league/invite-join.yaml`.** Header
...
on iOS 18 (law 17, `archive/retired-tooling/mobile/maestro/README.md:140-146`). Params are a **query string, never
...
| **D-7** | §4: the sign-in banner "fetches `/api/league/invite-meta` once" (no statement about other screens) | `fetchInviteMeta` has **exactly one call site**, and the resolved name is cached into the persisted invite intent for downstream screens | Forced by `archive/retired-tooling/mobile/scripts/sim-run.sh:178`: a fixture miss increments `vcr_misses` and **fails the sim run**. A meta fetch from `LeagueJoinScreen` or the picker would run with unseeded ids in the non-member leg. §2.0 states the rule and the reviewer check. |
