...
| `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml` **(new)** | 6 legs |
| `archive/retired-tooling/mobile/maestro/capture/trades.yaml` | **mandatory** — its error leg currently asserts the bug |
...
`archive/retired-tooling/mobile/maestro/capture/leagues@fresh.yaml`, `04-tabs-navigation.yaml`,
...
`archive/retired-tooling/mobile/maestro/README.md` flow-authoring laws 1-23 (law citations inline below).
...
### 9.2 New flow — `archive/retired-tooling/mobile/maestro/flows/trades-generation-failure.yaml`
...
- **Law 1** — every `text:` matcher is wrapped in `.*` (`".*took too long.*"`,
  ...
  `docs/plans/archive/2026/mobile-testing/lld.md:253` permits.
...
### 9.3 Mandatory — `archive/retired-tooling/mobile/maestro/capture/trades.yaml`
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen trades` at ship (executed by **W3-QA**, HLD
