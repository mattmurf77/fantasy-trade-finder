```project-status
{
  "status": "shipped",
  "updated": "2026-07-09",
  "summary": "Manual Trade Calculator — Plan (2026-07-02)",
  "evidence": "Carried forward as the last documented disposition, not a fresh production audit. Prior index merge corrections take precedence over older pre-merge notes. Last documented index disposition: shipped 2026-07-09 — Modes A and B both implemented — status banners are inline at the top of the doc.. Prior row preserved at ../../../reviews/2026/2026-09-06-project-status-migration/plans-index.md."
}
```

# Manual Trade Calculator — Plan (2026-07-02)

> **Status 2026-07-04: Mode A IMPLEMENTED.** Backend: `POST /api/trade/evaluate` + `GET /api/trade/values` (open, consensus-basis; `backend/server.py`, tests in `backend/tests/test_trade_evaluate.py`). Mobile: `TradeCalculatorScreen` "Real values" mode (server-authoritative verdicts, format toggle, debounced evaluate) with the mock dual-board league kept as "Demo league" mode. Deviations from this plan: a values-list endpoint was added for the picker (this plan assumed `/api/players`, which carries no values); picks remain demo-mode-only (v2 here as planned). Consolidate route contracts with the staged backlog-#27 web calculator when it lands (see api-reference.md).
>
> **Status 2026-07-09: Mode B IMPLEMENTED.** `/api/trade/evaluate` gained the in-league branch (`league_id` + `opponent_user_id`, authed) — prices each side by BOTH owners' real `member_rankings`, returns `basis`/`your_value_delta`/`their_value_delta`/`mutual_gain`, consensus fallback for unranked opponents (3 tests in `test_trade_evaluate.py`). Mobile: new `InLeagueCalculator` component + an **"In league"** mode on `TradeCalculatorScreen` (opponent picker w/ `has_rankings` badge, roster-constrained pickers, two-board verdict) — and it carries the **Send in Sleeper** button, making the "manually created trade" the 4th send surface. Roster source = Sleeper public rosters (`getLeagueRosters`); only universal-pool-valued roster players are pickable (graceful-drop). Confidence shrinkage is skipped (member_rankings stores no per-player sample count) — raw personal Elo vs. seed fallback.
