...
`archive/retired-tooling/mobile/maestro/flows/p1-9-trade-found-inbox.yaml` (§5) or the named manual check.
...
### New flow — `archive/retired-tooling/mobile/maestro/flows/p1-9-trade-found-inbox.yaml`

Header per `archive/retired-tooling/mobile/maestro/README.md`: `appId`, `# tc:`, `# profile: likes-you-waiting`,
...
defaults), `# source: docs/plans/archive/2026/audit-p1-remediation/PRD-p1-9.md`,
...
| 5 | `assertVisible: id: "topbar.bell"` → `tapOn: id: "topbar.bell"` | **New testID** (LLD D22; the reserved registry name, `docs/plans/archive/2026/mobile-testing/lld.md:313`). Opening the sheet triggers the server hydrate (`TopBar.tsx:118-141`) |
...
copy edit.** Run `archive/retired-tooling/mobile/scripts/screen-freshness.sh` and re-capture what it flags. Per
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen settings`. **The tier is not final until P-1 is
...
| `docs/plans/archive/2026/mobile-testing/lld.md` (Appendix A, testID registry) | **Updated** | Confirm `topbar.bell` (`:313`) and add the four `settings.notif.*` ids under `:329`. **Note in the ship notes that Appendix A `:311` and `mobile/src/components/CLAUDE.md:3` disagree about where the registry lives** — flagged, not arbitrated here. |
