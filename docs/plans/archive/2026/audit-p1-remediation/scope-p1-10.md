...
- **Grep result stated first:** `grep -rn "sleeperconnect\|SleeperConnect\|sleeper-connect"
  archive/retired-tooling/mobile/maestro/` returns **zero hits** — **no existing flow asserts this surface**, so
  there is no flow to update as part of the fix. (The twin has one:
  `archive/retired-tooling/mobile/maestro/flows/espn-connect-capture.yaml`.) The only related hits are
...
- **Optional new flow (NOT taken as written — plan §9 Checkpoint C):**
  `archive/retired-tooling/mobile/maestro/flows/sleeper-connect-capture.yaml` mirroring the ESPN flow would drive
...
- **Capture delta: none** — no visual change, so no `archive/retired-tooling/mobile/scripts/screen-capture.sh` run.
  `archive/retired-tooling/mobile/scripts/screen-freshness.sh` is run per the tier-2 requirement; expected to flag
  nothing.
...
- **Simulator-gate tier: 2** — `docs/runbook.md:97`, *"Mobile logic touched, no UI change →
  feature's flow + affected smoke subset **+ run `archive/retired-tooling/mobile/scripts/screen-freshness.sh`;
