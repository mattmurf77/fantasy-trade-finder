...
> photographed and diagnosed in `archive/retired-tooling/mobile/maestro/capture/matches@near-unlock.yaml`. The new block
...
**Prior state, verified:** `grep -rn "invite" archive/retired-tooling/mobile/maestro/` → **zero hits.** No existing flow
...
(`archive/retired-tooling/mobile/scripts/sim-run.sh:21, 31, 54`, handshake assert at `:114`), so a single YAML cannot span
...
### File A — `archive/retired-tooling/mobile/maestro/flows/growth/invite-promotion.yaml`
...
### File B — `archive/retired-tooling/mobile/maestro/flows/growth/invite-promotion@espn.yaml`
...
5), **not** by P1-5's own commit. `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen league --screen matches`;
