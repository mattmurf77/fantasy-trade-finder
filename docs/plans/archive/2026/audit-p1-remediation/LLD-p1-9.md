...
`docs/plans/archive/2026/mobile-testing/lld.md` Appendix A, `:313` — **already reserves `topbar.bell`**
...
- **The registry is not `mobile/src/components/CLAUDE.md`.** That file says so itself
  (`:3`: *"testID grammar/registry: `docs/plans/archive/2026/mobile-testing/lld.md` Appendix A"*), and
  ...
  not the registry. **Update Appendix A** (`docs/plans/archive/2026/mobile-testing/lld.md:313` and `:329`)
...
|---|---|---|---|---|
| D1 | `:267-271` (`FLAG_KEYS`, `notif.*` block) | five `notif.*` keys, last is `"notif.honest_winbacks"` | append `"notif.trade_found",  # P1-9: quality-gated counterparty-intent push (docs/plans/archive/2026/audit-p1-remediation/LLD-p1-9.md)` **inside the block**, not at the end of the tuple | `"notif.honest_winbacks"` |
...
| D28 | `archive/retired-tooling/mobile/maestro/flows/p1-9-trade-found-inbox.yaml` (new) | PRD §Maestro |
| D29 | `docs/plans/archive/2026/mobile-testing/lld.md:313`, `:329` | register `topbar.bell` (already reserved — confirm) and the four `settings.notif.*` ids (C6) |
...
- [ ] **New rows this LLD adds to the checklist:**
  ...
  - [ ] `docs/plans/archive/2026/mobile-testing/lld.md:313` still reserves `topbar.bell` and `:329` still
