...
| Maestro law 17: deep links are dead; launch-arg entry only. Law 23: a green run is not a good capture | `archive/retired-tooling/mobile/maestro/README.md:140-146, 173-175` |
| Existing assertions that cross these surfaces: `flows/smoke/09-league.yaml:34` (`league.hero`), `flows/smoke/08-matches.yaml:38` (`matches.empty-text`) | grep of `archive/retired-tooling/mobile/maestro/` |
| **No existing flow asserts any invite affordance.** `grep -rn "invite" archive/retired-tooling/mobile/maestro/` → zero hits. Nothing asserts the bug being fixed; nothing needs un-asserting | verified |
| Capture library variants that will change visually: `league`, `league@espn`, `league@near-unlock`, `league@quickset-done`, `league@single-format`, `league@two-leagues`, `matches`, `matches@fresh`, `matches@near-unlock`, `matches@two-leagues`, `matches@espn` | `ls archive/retired-tooling/mobile/maestro/capture/` |
