...
Conventions: `archive/retired-tooling/mobile/maestro/README.md`, flow-authoring laws 1–23. **id-selectors
only** (law 1: text matchers are full-match regex).

**Nothing currently asserts the bug — verified.**
`grep -rn "1 2nd\|1 3rd\|1 4th\|No value\|4 1sts" archive/retired-tooling/mobile/maestro/ screens/`
...
### Flow 1 — `archive/retired-tooling/mobile/maestro/flows/p1-7-anchor-labels.yaml` (new, mandatory)
...
### Flow 2 — `archive/retired-tooling/mobile/maestro/capture/anchors.yaml` (extend, mandatory)
...
### Flow 3 — `archive/retired-tooling/mobile/maestro/flows/p1-7-anchor-unlock.yaml` (new, **gated on RL-9**)
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen anchors` and
`archive/retired-tooling/mobile/scripts/screen-freshness.sh` to confirm nothing else is flagged
...
`archive/retired-tooling/mobile/maestro/flows/smoke/01-…11-…`) + flow 1 (+ flow 3 if RL-9) + the
...
>   (`docs/plans/archive/2026/audit-p1-remediation/DECISIONS-p1.md` **D-P1-02**): "the manual
...
>   `docs/plans/archive/2026/audit-p1-remediation/LLD-p1-7.md` §5 and `HLD-p1.md` **SQ-1**
