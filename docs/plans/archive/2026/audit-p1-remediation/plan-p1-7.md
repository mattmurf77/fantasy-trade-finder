...
- **No Maestro flow asserts the anchor labels.** `grep -rn "1 2nd|1 3rd|1 4th|No value|4 1sts" archive/retired-tooling/mobile/maestro/ screens/` → empty. `capture/anchors.yaml` anchors on `".*in draft capital.*"` and `".*Pull down to refresh.*"`; `AnchorSheet`'s rungs carry key-based testIDs (`anchor-sheet.rung.${key}`, `AnchorSheet.tsx:128`). **No flow asserts the bug** — a welcome exception to this audit's pattern. (`screens/manifest.json:55` *does* list `mobile/src/utils/anchorRows.ts` as a freshness source for the `anchors` screen, so editing it forces a re-capture — see Maestro delta.)
...
Conventions: `archive/retired-tooling/mobile/maestro/README.md`, flow-authoring laws 1-23.

### Nothing currently asserts the bug — verified

`grep -rn "1 2nd|1 3rd|1 4th|No value|4 1sts|2 1sts|3 1sts|1 1st" archive/retired-tooling/mobile/maestro/ screens/`
...
### 1. New flow — `archive/retired-tooling/mobile/maestro/flows/p1-7-anchor-labels.yaml`
...
### 2. Extended flow — `archive/retired-tooling/mobile/maestro/capture/anchors.yaml`
...
### 3. New flow — `archive/retired-tooling/mobile/maestro/flows/p1-7-anchor-unlock.yaml` *(gated on C6)*
...
`archive/retired-tooling/mobile/scripts/screen-capture.sh --screen anchors`. Evidence: `TEST_LEDGER.md` entry
