...
- **No flow asserts the surface, and I checked for the "flow pins the bug" trap.** Every Maestro file mentioning Apple, sign-in, or privacy was read:
  - `archive/retired-tooling/mobile/maestro/flows/smoke/11-apple-entitlement.yaml` — entitlement regression sensor. Taps `signin.apple-btn`, asserts the native failure copy is absent, then a Settings reachability leg (`settings.link-apple-btn`). It cannot reach a real Apple consent sheet (Maestro cannot drive it), so it never exercises token claims. Unaffected.
  - `archive/retired-tooling/mobile/maestro/flows/s1-spike-signin-ids.yaml` — testID visibility spike, backend-less. Unaffected.
  - `archive/retired-tooling/mobile/maestro/capture/signin.yaml`, `capture/onboarding-signin@fresh.yaml` — screen-library captures. No visual change on any screen, so **no capture delta**; `archive/retired-tooling/mobile/scripts/screen-freshness.sh` should flag nothing.
