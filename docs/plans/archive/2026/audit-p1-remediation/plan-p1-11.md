...
| `archive/retired-tooling/mobile/maestro/capture/*.yaml` — **12 files** | header-comment prose only, listed in [Maestro delta](#maestro-delta) |
...
25. `archive/retired-tooling/mobile/maestro/capture/*.yaml` — header-comment prose only, 12 files (§[Maestro delta](#maestro-delta)).
26. `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` — one added label assertion (§[Maestro delta](#maestro-delta), subject to CP-6).
...
```
grep -rn "Acquire" archive/retired-tooling/mobile/maestro/            → 12 hits, ALL inside # comments
grep -rn "Acquire" archive/retired-tooling/mobile/maestro/ | grep -v "#" → 0 hits
grep -rn "text:.*[Tt]rade"  archive/retired-tooling/mobile/maestro/   → 1 hit, unrelated
...
|---|---|---|
| `archive/retired-tooling/mobile/maestro/capture/trios@near-unlock.yaml` | 36 | quotes the RankScreen banner copy (edit 5) — **must** be updated or it misquotes the fixture |
| `archive/retired-tooling/mobile/maestro/capture/trades@single-format.yaml` | 43, 49 | "never on Acquire" / "`near-unlock`'s Acquire" |
| `archive/retired-tooling/mobile/maestro/capture/trades@fresh.yaml` | 19 | "the resting Acquire landing" |
| `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` | 131 | "lands the first run on Acquire" |
| `archive/retired-tooling/mobile/maestro/capture/portfolio.yaml` | 21 | "the Acquire subnav's Portfolio pill" |
| `archive/retired-tooling/mobile/maestro/capture/portfolio@two-leagues.yaml` | 10 | "the Acquire subnav's" |
| `archive/retired-tooling/mobile/maestro/capture/anchors.yaml` | 11 | "(Acquire tab is never visited)" |
| `archive/retired-tooling/mobile/maestro/capture/trios.yaml` | 11 | "that sheet belongs to the Acquire tab" |
| `archive/retired-tooling/mobile/maestro/capture/trends.yaml` | 11 | "(Acquire tab is never visited)" |
| `archive/retired-tooling/mobile/maestro/capture/manual-ranks.yaml` | 11 | "(Acquire tab is never visited)" |
| `archive/retired-tooling/mobile/maestro/capture/quick-set.yaml` | 12 | "(Acquire tab is never visited)" |
| `archive/retired-tooling/mobile/maestro/capture/tiers.yaml` | 11 | "an Acquire-tab" |
...
`archive/retired-tooling/mobile/maestro/` (or a written waiver)."* Nothing breaks, but nothing **guards** the label
either — a future silent rename would pass the full suite.

**Proposal:** extend `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` (already the tab-bar smoke flow,
...
6. `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` with the new assertion (or the CP-6 waiver).
