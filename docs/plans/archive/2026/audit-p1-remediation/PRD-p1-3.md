...
18. Zero files under `mobile/src/` or `archive/retired-tooling/mobile/maestro/` are changed.
...
- **The "existing flow pins the bug" trap was checked for and is absent.** Every Maestro file
  ...
  of an email field.**
- **No capture delta** — no screen's visuals change, so `archive/retired-tooling/mobile/scripts/screen-freshness.sh` should
