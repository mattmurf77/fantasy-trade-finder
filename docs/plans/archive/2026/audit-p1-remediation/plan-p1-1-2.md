...
| **Zero callers in `web/` or `extension/`** | same grep across the repo returns only `backend/`, `docs/`, `docs/plans/archive/2026/competitor-feature-waves.md` |
...
| Fact | Evidence |
|---|---|
| No Maestro flow references `calc.share-image` or any share affordance | `grep -rn "share" archive/retired-tooling/mobile/maestro/` → only prose uses of "shares"/"shared"; **no existing flow asserts the bug**, so nothing has to be un-asserted |
| The iOS share sheet is `UIActivityViewController` — out of process. Law 20 records that a native confirm "poisoned every later step" | `archive/retired-tooling/mobile/maestro/README.md:159-166` |
| Deep links are unusable (`openLink` → SpringBoard confirm); launch-argument entry is the substitute | `archive/retired-tooling/mobile/maestro/README.md:140-146` |
...
**Nothing existing asserts the bug.** `grep -rn "share" archive/retired-tooling/mobile/maestro/` returns only prose;
...
law 20 records a native dialog "poisoned every later step" (`archive/retired-tooling/mobile/maestro/README.md:159-166`).
...
**New flow — `archive/retired-tooling/mobile/maestro/flows/growth/share-links.yaml`** (`# flags: release`,
...
Run `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen calc --screen tiers [--screen trades]`; verify
...
| `docs/feedback/items/<id>/` | **n/a** | Audit-driven, not feedback-driven; home is `docs/plans/archive/2026/audit-p1-remediation/`. |
...
9. `archive/retired-tooling/mobile/scripts/screen-freshness.sh` flags `calc`, `tiers` (+`trades` if M11/M12 taken);
