...
WAIVED with a reason. Build plan: docs/plans/archive/2026/audit-p1-remediation/plan-p1-1-2.md
-->
...
- **Explicit non-assertion (W-1, waived with reason):** **no flow taps a share button.** The
  ...
  (`archive/retired-tooling/mobile/maestro/README.md:159-166`) records a native dialog poisoning every later step. The
...
- **Pre-existing-flow audit (required by the build brief):** `grep -rn "share" archive/retired-tooling/mobile/maestro/`
...
- **Capture delta:** `calc` and `tiers` — and `trades` if [OC-2](plan-p1-1-2.md#operator-checkpoints)
  ...
  Run `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen calc --screen tiers [--screen trades]`;
...
| `docs/feedback/items/<id>-<slug>/` | **n/a** | Audit-driven, not feedback-driven. Home is `docs/plans/archive/2026/audit-p1-remediation/`. |
