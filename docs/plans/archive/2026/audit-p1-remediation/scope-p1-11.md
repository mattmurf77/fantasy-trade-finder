...
- [x] **Extended flow:** `archive/retired-tooling/mobile/maestro/04-tabs-navigation.yaml` — add one assertion after
  ...
  **Rationale:** **zero** flows in the harness currently assert the tab label — verified
  exhaustively (`grep -rn "Acquire" archive/retired-tooling/mobile/maestro/ | grep -v "#"` → 0 hits; every tab
...
- **Capture delta:** `screen-freshness.sh` will flag **5 screens / 27 captures** —
  ...
  **Declared gap, flagged rather than hidden:** that under-reports. Only 2 of 32 screens
  ...
  Command: `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen <x>` (4–7 min each).
