...
archive/retired-tooling/mobile/scripts/sim-build.sh                   # §2.3
archive/retired-tooling/mobile/scripts/sim-run.sh                     # §2.2 single-cell executor
mobile/scripts/ui-test.sh                     # §2.1 matrix runner
mobile/scripts/testid-lint.sh                 # §2.6
archive/retired-tooling/mobile/scripts/mutations.md                   # §7 seeded mutations (exact patches)
archive/retired-tooling/mobile/maestro/flows/**.yaml                 # ~30 flows
archive/retired-tooling/mobile/maestro/payloads/*.json               # simctl push payloads
archive/retired-tooling/mobile/maestro/README.md                     # traceability TC ↔ flow
...
docs/plans/archive/2026/mobile-testing/layer3-reports/     # §2.7
```
...
1. **Seeded mutations (M3, `archive/retired-tooling/mobile/scripts/mutations.md`):** (a) revert the FB-45 401-guard in `client.ts` ⇒ TC-XC-02 fails; (b) break the FormatGate condition ⇒ its TC fails; (c) swap Check/X wiring ⇒ its TC fails. Run after major suite refactors + quarterly.
...
| Matches | `matches.segment.<mutual\|awaiting>` `matches.filter.<league_id\|all>` `matches.card.<n>` `matches.dismiss.<n>` `matches.empty-text` `matches.league-chip.<league_id|all>` `matches.value-details` `matches.open-in-calc` `matches.awaiting-dismiss` |
