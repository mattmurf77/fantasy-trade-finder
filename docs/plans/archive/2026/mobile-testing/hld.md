...
│   │        C5 flows in archive/retired-tooling/mobile/maestro/ (clearState+clearKeychain per flow)       │
...
| C5 | Maestro flow suite | ~30 YAML flows in `archive/retired-tooling/mobile/maestro/` implementing `test-cases.md` L1 cases; `# tc:`/`# profile:`/`# flags:` headers are the single source the runner parses; conventions LLD §4.4 |
