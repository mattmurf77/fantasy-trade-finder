...
| D9 resume entry | `OverhaulEntryCard` on the Acquire landing — since G-425 (2026-09-08) a full-width hero directly above the utility row / mode bar (was: directly below `TeamReviewEntryCard`; see §9). One **active** overhaul per (account, league); starting a new one archives the previous only if it has no `proposed`/`outcome_unknown` attempts (else 409 `active_sends_exist`). |
...
- **Analytics** (register in `backend/analytics_taxonomy.py` in the same commit as the emitters):
  - client (`ALLOWED_CLIENT_EVENTS`): `overhaul_started` `{league_id, outlook?, entry}` · `overhaul_roadmap_selected` `{overhaul_id, roadmap_id, package_count}` · `overhaul_batch_requested` `{overhaul_id, package_count, offer_count, tied_package_count}` (fired on press, before the server mints a `batch_id`; correlate through `overhaul_batch_reconciled`) · `overhaul_fallback_requested` `{overhaul_id, package_id, tier, offer_count}`.
...
|---|---|---|---|
| — | `components/OverhaulEntryCard.tsx` | Mobile A | **Full-width hero tile** on the Acquire landing, mounted directly ABOVE the utility row / mode bar wrapper (`modeBarWrap`) — the slot the strip cohort's Draft cell held until G-425 (#425/#426, 2026-09-08) removed that cell; previously a card below `TeamReviewEntryCard`. Whole tile pressable; ice fill via the single `HERO_TONE` constant (red is not a sanctioned action colour — operator Q1). "Team overhaul / Start overhaul ›" or "Resume overhaul" + `<outlook> · <summary line>` on one line. Shown when `overhaul.enabled` **or** an active overhaul exists. |
...
Structural guard `mobile/tests/check-team-overhaul.js` (Mobile B) pins: every overhaul screen registered once, in the Trades stack, none in RootNav; no `FeedbackFAB` import in any overhaul screen; entry hero rendered in `TradesScreen` exactly once, inside the main ScrollView, above the first `styles.modeBarWrap` and above `TeamReviewEntryCard` (§3a, G-425) and gated on `useFlag('overhaul.enabled')` or an active overhaul (§3b); `TradeHomeUtilityRow` and its mount carry no Draft cell / `onDraft` (§3c) while `TradeFinderModeBar` still declares `DRAFT_CHIP` and its mount still passes `onDraft` (§3d); `OverhaulEntryCard.tsx` declares one `HERO_TONE` with `fill: ice.base`, imports no `flare`, has no hex literal, reads every `borderRadius` from `radii.` and keeps `minHeight: 64` (§3e); no overhaul file imports `saveLeaguePreferences`/`/api/league/preferences` or `/api/tiers/save` or `swipeTrade` (preference isolation, no taste learning); priorities screen imports `react-native-draggable-flatlist` and defines `accessibilityActions`; summary screen renders the race warning keyed by `package_id`; the `AttemptState` union in `api/overhaul.ts` has a matching label in the plan screen; every testID family above appears literally. Add `"test:team-overhaul"` to `package.json`.
