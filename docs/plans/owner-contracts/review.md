...
The initial public-publication safety block was resolved by the owner's explicit “Push to github and testflight” authorization on 2026-09-05. The later “Turn it on” request authorized main merge and backend release after final exact-head CI passed. Render is live on the verified squash tree; pre/post runtime hashes are identical. Local history and EAS archive checks confirmed the five raw source files are excluded; config, fixtures, lockfile and secrets are unchanged. No destructive migration or experimental policy activation was performed. Manual TestFlight remains a follow-up, not a claimed pass. All worktrees are retained; no cleanup/deletion is part of this change. See [release.md](release.md) for build/deploy IDs, live flag posture and rollback limits.
