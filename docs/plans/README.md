...
| [ram-mascot/](ram-mascot/) | 1 | **active — brief, awaiting D1** · 2026-08-22 | Replace The Analyst guide avatar with the ram (the shipped app-icon character) so the branding holds together: [brief](ram-mascot/brief.md) in three parts — §1 decision (ram = mascot, closes Q-009, a scoped raster exception for painted art), §2 six-pose art via Higgsfield in two explorations (A logo-match painted sprites — the operator's preference to explore; B Chalkline-flat SVG as the control) judged in a new avatar-lab page, §3 the renderer swap behind `onboarding.mascot_ram` keeping the `AnalystAvatar` API, pose enum and analytics unchanged. Starts after #384 W6-B lands. |
