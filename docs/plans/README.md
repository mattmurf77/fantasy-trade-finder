...
| [negative-results-memory/](negative-results-memory/) | 14 | **planned — suite complete, build gated on operator** · 2026-08-21 | Per-league memory of reasoned rejections as a SOFT generation-time prior (M1) + feeding gen_v2's unfed `acceptance_prior` (M2). Planning only — full gates; build blocked on three operator decisions (scope §6) and three-way reconciliation with the Receipts + counterparty-breaker sibling plans. No flag exists yet (`trade.negmem` proposed). |
