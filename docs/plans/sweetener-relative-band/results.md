...
* **QA's regression reproduces and is closed.** V1 (750 alone) lifts the `>1539` share from 0 → **3.5%** on `12t_1qb@u0` (B, v3) and doubles arm C's on `16t_sf` (10.5 → 20%): a card whose partial close no longer counts ships at its ORIGINAL gap. V2/V3 (best-effort on) **never raise `>1539` above V0 in any live-arm cell, and take it to 0 in every arm-B and arm-D v3 cell**; the previously-unclosable card ships as a stamped partial. Arm C keeps its pre-existing over-the-line residue at V3 — 4.55% (`12t_1qb@u0`), 10.0% (`16t_sf`), 13.3% (`12t_1qb@u8`), each ≤ its V0 value — because no equalizer passes arm C's own band on those cards; the sweetener cannot close what its gate stack refuses.
* **The band matters at the top, not the bottom.** V4 (frac 0.15) re-opens the window — `>1539` back to 3.5% on `12t_1qb@u0` for both B and D — because 0.15 × a 10,000+ package sits above 1,539 again. 0.12 keeps the trigger under 1,539 for every H < 12,825, which covers every package the harness corpus holds — by ~3% (§ Headroom).
* **Sweetened share rises; junk stays flat on the live arm, not on every arm:** B v3 cells go 6.9 / 0.0 / 4.8% → **10.3 / 11.1 / 19.1%** (partials 1 / 0 / 1); the sub-450 body share on those three B cells moves **0 / +3.5 / 0 pp** (two extra multi-asset cards on `16t_sf`, both sweetened). Elsewhere it moves more: **arm C `16t_sf` +9.47 pp** (10.5 → 20.0%), arm D `16t_sf` +5.62 pp, arm C `414_1x1` +6.05 pp, arm D `16t_sf` v2 +6.52 pp — all sweetened multi-asset cards. Caveat on the metric itself: the harness's "sub-450" counts any traded asset under 450 of **consensus seed value**, while the #141 floor gates on **max(user board, opponent board)** value, so a card this column counts as junk can be floor-clean on the boards the path actually gated on; it is a D-159 comparability line, not a gate violation. **Top-5 Jaccard is 1.0 in every B and D v3 cell**, set Jaccard 0.87 / 0.84 / 0.68 on B; **three V3 cells fall under 0.7 on top-5, all arm C**: `16t_sf` 0.667 (set 0.696), `12t_1qb@u8` 0.667 (set 0.875), `414_1x1@u4` 0.429 (set 0.286 — arm C's 13-card deck reorders wholesale once four of its cards are sweetened). **Arm A is unmoved in every cell.**
...
## The #414 card in the app's own unit

What the user sees is not the raw gap but `_pick_gap_equivalent` (`server.py:841`; base = a generic Mid 1st, **2,117.0** of value on the current pool). Run on the fixture's numbers:

| card | gap | firsts | pick label shown | bar |
|---|---|---|---|---|
| served 1x1 (today) | 1,396.0 | **0.66** | **≈ Late 1st Round Pick** (1,491.8) | `favors: receive`, `add_to: give` |
| best-effort partial, ≤1,200 bench (live triple) | 1,058.9 | **0.50** | **≈ Early 2nd Round Pick** (860.7) | `favors: receive`, `add_to: give` — ratio 0.859 < 0.95, so not `even` |
| full close with a 1,500 piece | 772.1 | 0.36 | ≈ Early 2nd Round Pick | ratio 0.896, still `add_to: give` |
| planted-league full close (eq 1,687.7) | 586.2 | 0.28 | ≈ Early 2nd Round Pick | ratio 0.921 — still under 0.95 |

So the partial moves the operator's card **a whole pick round in the app's own unit** — "over a first" becomes "an early 2nd" — which is the answer to "there are other players I can add to make the trade more fair". The caveat is real: `even` is `ratio ≥ 0.95` (`server.py:11835`), so every one of these closes still shows `favors: receive` with an `add_to` suggestion on the value bar. **The card reads fairer, not even** — the sweetener narrows, the bar keeps asking for more, and that is the intended reading (the bar's `add_to` is the calculator's one-tap evener, a different feature).
...
Evidence line: *V3 is the only variant that never raises the `>1539` share above V0 in any live-arm cell and takes it to 0 in every arm-B and arm-D v3 cell (V1 re-opens it to 3.5% on `12t_1qb@u0` and doubles arm C's 16t share to 20%; V4 re-opens 3.5% on `12t_1qb@u0` for B and D; arm C keeps a ≤ V0 residue of 4.6 / 10.0 / 13.3% that its own band refuses to let the sweetener touch); sweetened share rises from 6.9 / 0.0 / 4.8% to 10.3 / 11.1 / 19.1% on the three B cells with the sub-450 share flat there (0 / +3.5 / 0 pp) and up to +9.5 pp on arm C / +6.5 pp on arm D (a consensus-value column, not the #141 board metric); top-5 Jaccard is 1.0 in every B and D cell and set Jaccard ≥ 0.68 on B, with three arm-C cells under 0.7 (0.667, 0.667, 0.429); arm A is unmoved in all 280 cells; the QA regression is closed in the unit test and in the harness; the #414 card closes in the unit test (partial at a 1,200 bench, full at 1,500) and in the planted league (full, equalizer 1,687.7).*

Why 0.12 and not 0.15: the band's job is to keep the trigger under the late-1st line for every card the corpus actually holds; 0.12 × H < 1,539 for H < 12,825 (§ Headroom: the largest package the harness corpus prices is 12,436.6, so 0.12 clears the line by ~3%), while 0.15 crosses 1,539 at H = 10,260 and the harness shows the window re-opening there. Why 750 and not lower: 750 is the operator's proposed floor (≈ half a late 1st); below it the sweetener would start touching 1x1s the fairness bar already calls fair.

## Headroom — how close 0.12 sits to re-opening the window

The relative band re-opens the `>1539` window the moment `frac × H > 1,539`, i.e. at `H = 1,539 / frac`: **12,825 at 0.12**, 10,260 at 0.15, 15,390 at 0.10. QA measured the largest packaged side across the harness corpus at **12,436.6** (a 3-asset side, the shape `v3_shape_max_delta` 2 permits), so 0.12 clears the crossover by **~3% of package headroom** — enough for these fixtures, not a margin. A richer league (a 16-team SF roster with two 7,000+ studs on one side, or a `v3_shape_max_delta` bump) re-opens the window at 0.12 for its biggest packages; 0.10 buys 20% more headroom at the cost of firing on every package over 7,500 that is 10% apart, which the harness did not sweep. **Ship 0.12; re-measure the max packaged H (a one-line read off `deck_impressions.give_value` / `receive_value`) if package sizes grow, and drop to 0.10 before raising the shape cap again.**
...
## Findings for the lead's ledger (pre-existing, not fixed here)

* **The consensus generator's `seen` dedupe keeps the unsweetened key alive after a sweetened close.** `_generate_consensus_for_pair` adds the ORGANIC combo's key to `seen` at `trade_service.py:7296` and, when the gap pass rewrites the card, adds the SWEETENED key at `:7337` — without discarding the first. The v2 pair path does discard (`_picked_keys.discard` at `:7002`), and so does v3 (`card_keys.discard`, `trade_optimizer.py:748`). Consequence, measured by QA: two pairings lose one card at V1 (the threshold cut alone — the stale unsweetened key blocks a later sibling combo that would have filled the slot); best-effort adds nothing on top, and league totals are flat. Latent since 2026-08-21, surfaced by the lower threshold, one line to fix, out of this change's scope. Follow-up for the lead.
