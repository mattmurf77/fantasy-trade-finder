...
* **Knob 0.** The loop at `:6528` is guarded by `if _bm_frac > 0`, and the helper's first line returns `None` for `frac <= 0` anyway. At the default no card object is touched, no `reasons` list is appended to, and the serializer — which emits `reasons` only when the list is non-empty (`server.py:11886-11887`) — produces the same bytes as before. Proof: [`test_wire_at_knob_zero_is_byte_identical_to_origin_main`] — the FULL `generate_trades` deck on the engine-quality fixture, through `trade_card_to_dict`, flag ON, captured on a `git archive origin/main` tree at `02d2eac2` (8 cards, sha256 `8ad11872…` of the captured line; the branch capture `cmp`'d byte-identical) and **re-verified against `e16bb487`**, the tip the branch is rebased on (the golden test green post-rebase; QA's independent 457,236-byte wire capture on that tip `cmp`'d identical). [`test_the_wire_golden_is_not_vacuous`] shows the same fixture DOES move at 0.15.
...
| **injectors**: likes-you (`server.py:3535`), standing offers (`:3650`), client-echo rebuild on swipe (`:12717`) | construct `TradeCard` directly, outside any generator | **no** — never reach the stamp; carry no `reasons` | n/a |

So on the served arms A/B/D every organic card can carry the line; on C and `fit` none can, and **injected cards never can** — the same give headliner explains itself on an organic card and not on a likes-you / standing-offer / echo-rebuilt one. That is the pre-existing pattern every post-generation stamp (`need_fit`, `lane`, `gap_sweetener`, …) already has; recorded in scope.md §2 as a follow-up, not fixed here. [`test_reason_is_stamped_on_both_bases`] covers divergence + consensus through the real generator; [`test_deck_is_invariant_at_every_knob_value_on_100_random_leagues`] runs under the live flag set (v3 on, presentment rules, need fit, lanes, …) with random boarded/unboarded partners so every prod path in the table is exercised.
