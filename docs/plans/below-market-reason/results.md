...
* **Knob 0 is byte-identical to `origin/main`** — the FULL `generate_trades` deck on the engine-quality fixture, through `trade_card_to_dict` with `trade_math.human_explanations` ON (the prod posture), captured on a `git archive origin/main` tree at `02d2eac2` and `cmp`'d against the same capture on the branch (8 cards, sha256 of the captured line `8ad11872083c4ae4…`); the branch is rebased on `e16bb487` and the golden was re-verified against that tip (golden test green post-rebase; QA's independent 457,236-byte wire capture on `e16bb487` `cmp`'d identical). Pinned by `test_wire_at_knob_zero_is_byte_identical_to_origin_main`; the same fixture is shown to MOVE at 0.15 (non-vacuity).
...
| Run | Command | Result |
|---|---|---|
| clean `origin/main` @ `e16bb487` (`git archive` into a private scratch tree, no new file) | `PYTHONHASHSEED=0 python3 -m pytest backend/tests -q -p no:cacheprovider` | **4519 passed, 1 skipped** in 444.8 s |
| branch tip (`06378525`, on `02d2eac2`) | same | **4532 passed, 1 skipped** in 543.9 s |
| branch tip after rebase onto `e16bb487` (`0ad4cd8d`) | `… test_below_market_reason.py test_bakeoff_arm_a_golden.py test_engine_quality_golden.py test_consensus_fit_sort_key.py` | **62 passed** — the knob-0 wire golden still holds against the rebased `server.py` |
| rebased branch, full suite | `PYTHONHASHSEED=0 python3 -m pytest backend/tests -q -p no:cacheprovider` | **4542 passed, 1 skipped (in 411.6 s)** |

**Base moved mid-session.** `origin/main` advanced from `02d2eac2` to `e16bb487` (three commits: web parity #263, its ledger #265, a recovery note #266) while this branch was being built. Those commits touch `backend/server.py` only at `:9967` (`rankings_import_apply`), plus `web/` and one `docs/plans/README.md` row — neither `trade_service.py`, `database.py`, `trade_card_to_dict` nor any test this change relies on — so the golden captured on the `02d2eac2` tree is the golden for `e16bb487` too, and the rebase (`git rebase origin/main`, no conflicts) was re-verified by re-running the goldens above. A first "clean main" baseline run was discarded: the session-scoped scratch directory is shared with a sibling builder, whose own test file (`test_sweetener_relative_band.py`, written at 16:01 into the shared `main_tree/`) contaminated that tree and produced 18 failures that belong to neither `origin/main` nor this branch; the number above is from a second run in a privately named tree cut from `e16bb487`.
| new module alone | `… backend/tests/test_below_market_reason.py` | **24 passed** in 25.9 s after the QA fixes (23 before; the 100-league property test is ~23 s of it) |
...
| S4 (QA coverage gap) | drop the `team == "PICK"` clause from `is_pick_asset` (`trade_service.py:2214-2216`) — the universal pool's generic picks carry a REAL position and only that clause knows them | `test_a_generic_pick_never_headlines_and_is_never_named` (new: a `position="WR"`, `team="PICK"` asset out-seeding every player and 30% below on the user's board must neither headline nor be named) | **1 failed** | 1 passed |
...
**QA's shrinkage table — what the knob demands of the RAW board.** The knob is an Elo distance *after* shrinkage, `Δelo = ln(1/(1 − frac)) / 0.005`, and the shrink weight is `n / (n + 4)` (`shrink_pseudocount` 4), so the RAW gap a player needs before the line fires is `Δelo · (n + 4) / n`:

| `frac` | shrunk Δelo | raw gap needed at n = 1 | n = 2 | n = 4 | n = 8 | n = 16 |
|---|---|---|---|---|---|---|
| 0.10 | 21.1 | 105 | 63 | 42 | 32 | 26 |
| **0.15** | **32.5** | **162** | **97** | **65** | **49** | **41** |
| 0.25 | 57.5 | 288 | 173 | 115 | 86 | 72 |

The operator's Adams case as described in Q-035 — "well below consensus", modelled in the tests as 23.3% below raw = **53 raw Elo** — shrinks to 53·n/(n+4): 14.7% at n = 6 (**below** the 0.15 bar), and clears 0.15 only from **n ≥ 7** comparisons on Adams; it clears 0.10 from n ≥ 3. So 0.15 asks for a held, repeatedly-voted opinion and 0.10 for a lightly-held one — which is the same trade-off the harness bands show from the other side (the `[0.10, 0.15)` band is the lightly-sampled fires). **QA's counter-argument for 0.10** is recorded: on a real board most players carry few comparisons, so at 0.15 the line may stay silent on exactly the Adams card that motivated it. **The recommendation stays 0.15 pending the lead's read of the operator's actual shrunk gap on Adams from prod** (`comparison_counts` for Adams + personal vs seed Elo → `1 − e^{0.005·(shrunk − seed)}`): if it lands in [0.10, 0.15) the live value is 0.10; if it clears 0.15, 0.15 stands.
