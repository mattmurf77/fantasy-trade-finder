...
| **OI-9** | **RESOLVED 2026-08-13 — DO NOT ADOPT `expo-updates`** ([memo](device-side-platform-auth-oi9-expo-updates-memo-2026-08-13.md), D-048). Three of this item's premises did not survive the evaluation: there is **no subtree carve-out** (whole-bundle replacement only), OTA is a **native** module so it can never reach the `1.13.2` fleet and obsoletes **none** of §2.5/§4.5/§6.2, and it does not touch R7. It also has zero coupling to S3–S5, so it never belonged on Gate C. Re-open at the first public App Store release. | resolved | — |
...
| **OI-21** | **§4.3 step 3's host allowlist must not use `new URL()`.** RN 0.81.5 polyfills `URL` itself (`setUpXHR.js:35` → `Libraries/Blob/URL.js`) with a 198-line **regex** whose one-arg constructor stores the string unvalidated; no WHATWG polyfill is installed and `new URL()` appears nowhere in `mobile/src` today. `fetch` then dispatches via **native iOS**, which parses properly — a parser differential in the security control, one layer above the GraphQL guard. Measured over 16 adversarial URLs: **0 bypasses** (RN is strictly over-strict, so the allowlist cannot be tricked) but **4 false refusals**, incl. `https://sleeper.com/a:b@c` reported as carrying userinfo because the password regex `/https?:\/\/.*:(.*)@/` scans the **path** — and §4.4 pages the operator for that refusal code. Harmless in r1 (the sole endpoint has no `:` or `@` and tests ALLOW), not harmless once ESPN's REST paths arrive. **Fix at S4: exact-match `lease.request.url` against a compiled `ALLOWED_ENDPOINTS` set** — strictly stronger than parse-then-allowlist, no parser to diverge, and it deletes the scheme/userinfo/port checks as a class. Evidence: [OI-12 memo](device-side-platform-auth-oi12-runtime-primitives-memo-2026-08-13.md) §3. | **S4** | eng-mobile |
| **OI-22** | **The R1–R8 risk list has no source in the repo.** `git grep -nE '\bR[1-8]\b'` finds **zero** occurrences in the PRD; `R1–R6` / `R1–R2` appear only in the three documents that *assert* the claim (HLD decisions, this LLD's OI-9, Plan §10). The list lived in the superseded HLD on `design/device-side-platform-auth`, which is not in the tree — so the sentence that gave OI-9 its "upstream of the whole programme" standing propagated through three artifacts and four review rounds **unverifiable**. Either restore the list into the PRD or stop citing it. | docs | eng-architect |
