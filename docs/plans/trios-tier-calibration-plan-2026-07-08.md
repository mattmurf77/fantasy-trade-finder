```project-status
{
  ...
  "updated": "2026-07-10",
  "summary": "Trios → Tier Calibration — Plan (2026-07-08, rev. 2 after pressure test)",
  "evidence": "Carried forward as the last documented disposition, not a fresh production audit. Prior index merge corrections take precedence over older pre-merge notes. Last documented index disposition: partly-shipped 2026-07-10 — Shipped with the FB-#97 variety fix; addenda are inline. Trio-driven Lever B remains design-only.. Prior row preserved at ../reviews/2026/2026-09-06-project-status-migration/plans-index.md."
}
```

# Trios → Tier Calibration — Plan (2026-07-08, rev. 2 after pressure test)

> **2026-07-10 addendum (variety fix, FB #97 "I get Bijan Gibbs Jeanty way too frequently"):** the shipped Lever-A selectors were *deterministic within each strategy*, so the small pools at the top of the board kept re-serving the same faces even with anti-repeat on. Four stacked causes, verified against the DP RB snapshot (elite = 5-6 members; the elite/starter edge holds only ~2 straddlers above + ~4 below within the ±60 margin): (1) the within-tier selector always took the tier's max/min-Elo members, putting the #1 RB in **100 % of elite within-tier trios**; (2) `_within_tier_cursor` started at 0 = *elite* on every service rebuild, and all variety state (`_recent_trios`, cursor) is in-memory — so **every app session opened with the same elite trio** (simulated: the top RB appeared in 6/6 session openings); (3) avoid-relaxation was all-or-nothing (`fresh if len(fresh) >= 3 else members`), re-serving the *identical* trio whenever a 3-4-member tier sat inside the avoid window; (4) boundary edge choice broke integer score ties by iteration order (elite/starter first) and picked candidate/opponent deterministically closest-first. Fix (same-day, `ranking_service.py`): random cursor start per rebuild; within-tier top/bottom sampled from the top/bottom **two**; partial avoid-relaxation (longest-unseen re-admitted first, via `_last_seen_at`); boundary candidate/opponent/third sampled from the top-2/2/3 eligibles + `random.random()` score jitter to break edge ties; `trio_repeat_avoid` default 3 → 8 (matches the live prod tune). Calibration intent is untouched — boundary trios still straddle an edge and within-tier trios still span top-vs-bottom of one tier; only *which* eligible straddlers/extremes get served is randomised. Guardrails: FB #97 tests in `backend/tests/test_trio_variety.py` (repetition bound over 20 trios: max player freq ≤ 3, max pair ≤ 2; session-rebuild opening variety; partial-relax; pick-variety). Post-fix sim (20 consecutive trios, DP RB pool): max same-player frequency 2 (was 3-4), top-3-RB session-opening streak eliminated.
>
