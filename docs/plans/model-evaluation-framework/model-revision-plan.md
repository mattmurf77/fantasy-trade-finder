...
- Picks: audit existing year-decay behavior against the owner's explicit **no later-year discount** direction; prior code review identifies this as an unresolved implementation gap, not a settled owner-aligned runtime policy. Propose and independently test any correction before changing runtime. Use projected slots only for the next draft and only with an as-of model snapshot. Do not use eventual known slots in historical evaluation. Validate ownership/year/expiry; no 2026 drafted picks reintroduced.
