...
- **P3 — Experiment engine ✅ built (2026-07-18):** `backend/experiments.py` (two-stage deterministic hash — layer bucket + version-keyed variant bucket — targeting over the FR-33b registry, persist-on-first-eval, status machine, launch validation, readout, design calculator; fail-open everywhere, gated on `experiments.engine`), `backend/analytics_stats.py` (hand-rolled norm/betainc/gammainc/chi²/z/Welch/SRM + power calc — golden-tested to **1e-9 vs scipy**, scipy NOT shipped), 5 experiment tables + env-keyed layer salts, CRON-gated `/api/admin/experiments[...]` routes (create/launch/transition/decide/revise/readout/preview), the flag-endpoint resolve wiring, and the Experiments dashboard tab (monitor cards + design calculator). `trade.aggression_ab` migrated via a **safe bridge** (experiment drives the bucket when running; MD5 fallback otherwise — zero behaviour change on deploy). Tests: `test_analytics_stats.py` (56, golden), `test_analytics_p3.py` (16 — determinism, **in-layer exclusivity**, cross-layer independence, targeting, validation, status machine, fail-open, aggression bridge). Verified live in-browser. **Dark until `experiments.engine`=true** (default false). **Set `EXPERIMENT_SALT_KEY` in Render before launching any experiment.**
- **P4 — Long tail ✅ built (2026-07-18):** web SDK `web/js/events.js` (`window.FTFTrack`, wired into `app.js` navigation/auth/find-trades — closes the `source:"api"` web blindness); extension emitter `extension/background.js` `emitAnalyticsEvent` (taxonomy-legal `app_opened`/`signin_succeeded`, flag-gated default-dark); `/an-experiment` self-service skill (registered in the roster). Web SDK verified live in-browser (queues, `seq`, device_id, `disposition:"disabled"` → queue retained). Remaining optional: Sentry DSN arm (SDK already compiled in, `sentrySetUser` pseudonymous), snapshot cron (only if dashboard latency warrants — beta scale doesn't yet), richer extension events (need a taxonomy addendum).
