...
Date: 2026-09-06. **Completed author draft for independent planner critique; not implemented or tested.**

Inputs: [plan](plan.md) at `ea5cc2b5`, [research](../../research/2026-09-06-trade-package-shapes.md), [scope](scope.md), [reconciliation](reconciliation-log.md). Source citations identify audited runtime `4026ebc8`; parent must reconcile current integration changes before build.
...
| R2 | Apply one stable permutation within disjoint six-absolute-position windows and identical slot classes, maximum five absolute slots moved. Preserve objects/multiset/assets/values/eligibility, every class at its original position, protected special positions and equal-complexity order. | T1, T2 |
...
| R5 | Reorder exactly once after final policy composition and before first evaluated publication. Every subsequent publication, serialization and impression writer consumes that order; no `/status`, cached-dict or client re-sort. | T1, T4, T6 |
| R6 | Existing-column provenance records original/final positions honestly, preserves source/group ranks and actual draw propensity, and versions serving separately from valuation policy. Off/exempt jobs retain baseline order/payload/features. | T5, T6 |
...
Input is the final policy-survivor sequence and its existing card/object-keyed metadata. Never regenerate, deduplicate, truncate, add a candidate, call valuation, or modify a package. Produce a reordered list containing the **same object occurrences**, plus frozen per-occurrence presentation records. Do not mutate input lists or source attribution; original ranks travel with their card, not their new slot.
...
Old-ID polls and retained client screens retain their captured order; no backfill. Existing later #419/safety filtering may remove a card, preserving remaining relative order. Frozen indices still describe the original served batch, not the compressed post-action list; do not rebuild windows or rewrite impressions after removal. Rollback affects compatible fresh generation, not an already viewed deck.
...
Indices are zero-based positions in the **post-policy/pre-presentation input** and frozen final served list. Counts are nonnegative integers when classification is known; use null for an unknown side, never a guessed zero. Records must remain aligned with occurrences, including duplicate-shaped cards, not be accidentally keyed only by a trade hash. No additional raw player IDs or private values are introduced. Off/exempt jobs have no new presentation feature. Existing JSON-column use avoids a migration but requires data-dictionary review.
...
| T4 — one first publication | Normal/forced, both formats, pregen and headless replenishment use one helper before evaluated publication. Poll at that boundary with logging on/off; first evaluated/final/cached response order agrees. New disposition filtering only removes cards and preserves survivor order; no helper at status time. | Impression-only reorder, early snapshot leak, separate pregen order, status-time rerank/refill. |
...
| T6 — frozen provenance | Actual legacy/F1 writers agree with served indices; original arm/group rank, attribution/agreement, valuation variant and true propensity unchanged. Presentation record every row/occurrence including first locked/unattributed card; null unknown count; no phantom rows. Signal/suggestion logging combinations and mid-job knob flip retain frozen semantics. | Rewrite source rank, first-row-key omission, stamp current knob, fabricated fourth arm/propensity. |
