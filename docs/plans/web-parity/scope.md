...
| W5 | **`league-rankings.html` still 401s when reached by direct URL** | Its flag is off by design; the page shows an unavailable state. Not user-reachable through the UI | Accepted | **Narrowed 2026-09-02.** Still 401s, still accepted — but it was the one dark page *listed in `sitemap.xml` and not `Disallow`-ed*, i.e. actively advertised to crawlers as indexable while returning 401. `player.html`/`profile.html` were already handled. Sitemap row removed, `Disallow` added, both carrying a restore-when-lit note. |
| W6 | **P2-3 ships with NO app-promotion CTA** | There is no TestFlight or App Store link anywhere in the repo (`docs/feedback/items/239-invite-universal-links/status.md:79`), and a dead "become a tester" button converts worse than none. Operator directed "build it link-free now" (2026-09-02) | Operator | **Open — needs a URL.** The section is built and the CTA's home is identified; supplying the link is the whole remaining task. |
| W7 | **P2-3 ships with no screenshots and no social proof** | `web/` has no image assets, and `screens/` is frozen at 2026-08-11 — weeks behind the shipping build, so those captures would misrepresent the product | Operator | **Open.** Needs current captures, which D-056 left without a producer. |
