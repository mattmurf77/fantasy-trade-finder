...
> **2026-08-09 correction (dual-lens source-code reconciliation, [dynastydaddy-sim-reference.md](dynastydaddy-sim-reference.md)):** the characterization above was written from the marketing site and README, not the source, and two things need correcting now that the actual code (`Leondoff/dynasty-daddy`, the repo matching the *current* live app — `G-Sher/dynasty-daddy` above is a stale 2021 snapshot that never implemented Elo) has been read line-by-line. (1) The **playoff calculator specifically does not use KTC value at all** — it runs on a separate metric, redraft-season **ADP consensus**, not the dynasty trade value the rest of the app uses; "KTC-only" is true for DynastyDaddy's trade-value pages but not for its playoff-odds sim. (2) It is **not a points-based Monte Carlo** — no point score is ever simulated; every matchup is a win/loss coin flip off a rating-derived probability, so there is no simulated `points_for` and ties are broken by another coin flip. "Historical Elo" is real (logistic, margin-of-victory-scaled K) but is an **opt-in Beta-badged alternative** to the default static-ADP model, not DynastyDaddy's primary method. (3) The repo license question is resolved: `G-Sher/dynasty-daddy` (cited here) genuinely has no LICENSE file (confirmed via GitHub API); the current-app-matching `Leondoff/dynasty-daddy` mirror is MIT-licensed. Full detail, byte-level source citations, and the comparison table against FTF's own engine: [dynastydaddy-sim-reference.md](dynastydaddy-sim-reference.md).
...
5. **Off-season caveat (load-bearing — it is July 2026, preseason):** **no NFL games have been played, so there are no current-season standings and no weekly scoring history.** Any odds shown now are pure priors off preseason projections — **low signal and easily mistaken as authoritative.** Recommend **gating the odds surface until real scoring data exists (~Week 3–4)**, or labeling every preseason number "projected/beta" exactly as the mockups already do. Do not ship a hard "86% playoff odds" preseason.
   ...
   > **2026-08-09 correction:** re-verified directly against DynastyDaddy's source and UI copy — **DynastyDaddy does not hedge its preseason odds at all.** No small-sample or low-confidence warning exists anywhere in its playoff-calculator documentation or UI; it shows the same bare percentage at week 1 as week 14. This recommendation should not be read as "do this because DynastyDaddy already does" — it doesn't. FTF's `is_preseason`/`beta` ribbon is a deliberate point of differentiation from this incumbent's actual (unhedged) behavior, and the recommendation stands on its own merits. See [dynastydaddy-sim-reference.md](dynastydaddy-sim-reference.md) §4.
