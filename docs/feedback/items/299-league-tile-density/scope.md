...
- [x] **New flow:** `archive/retired-tooling/mobile/maestro/flows/league/05-drill-in-back-affordance.yaml` — tags `[league, nav]`, profile `standard`, flags fixture `release`.
...
- **Capture delta:** `league-summary` — **and it needs a new state first.** `screens/manifest.json` lists `loading | error | populated | basis--personal | populated--single-format`; there is **no `drill-in` state**, and `archive/retired-tooling/mobile/maestro/capture/league-summary.yaml` never taps a bar. So the screen this item is entirely about has never been captured, and the lab's "current" frames are token-exact reconstructions rather than traced screenshots. **Requested:** add a `drill-in` state to the capture yaml, then `archive/retired-tooling/mobile/scripts/screen-capture.sh --screen league-summary --state drill-in`. One capture also unblocks #300, which reuses this tile.
