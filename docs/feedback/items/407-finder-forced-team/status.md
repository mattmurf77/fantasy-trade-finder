...
- **SHIPPED 2026-08-30** — PR [#250](https://github.com/mattmurf77/fantasy-trade-finder/pull/250) squash-merged to `main` @ `287aed09` on green CI ×3; EAS build **140 / v1.16.12** submitted to TestFlight (auto-submit FINISHED; build 139 errored on an EAS-side "Failed to upload application archive" and was retried). Feedback status → `fixed`. **Owed: the operator's 6-step TestFlight checklist on build 140** ([mini-prd.md](mini-prd.md)) — step 6 (Clear → Find a Trade) should now search ALL teams, since the #406 build closed B-1.
