...
| ADR or `DECISIONS.md` entry | **updated (`DECISIONS.md`) / ADR WAIVED** | **D-362-1** — *A standing offer is bounded by round and by the user's own team selection, not by a pick-value band* (text in [`hld-delta.md`](hld-delta.md) §9). Originally slated as DECISIONS.md D-093, but `origin/main` took D-093–D-097 before ship; recorded item-scoped instead (the D-306-1 / D-320-2 convention). **ADR waived because** this decision reverses a *lab proposal*, not a shipped architectural commitment, and it is fully explained by two existing decisions (D-079, D-090) plus one open question (Q-023) — a DECISIONS entry pointing at those is the honest weight. An ADR would restate them. **Operator sign-off requested.** |
