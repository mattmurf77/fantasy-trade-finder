...
- [x] **Structural guard — implemented and run:** `mobile/tests/check-win-now-recovery.js`, registered as `npm run test:win-now-recovery`, executes production logic with deferred transports/fake time and pins lifecycle wiring, all init callers, safe GET-only recovery, finite attempts and layer boundaries. Existing Win Now/session-seed guards also pass. [Exact build evidence](build-evidence.md); all 95 mobile guard programs passed independently in the integrated root run.
- [x] **Unit tests — implemented and run:** 38 recovery cases cover native behavioral T1–T5/T7–T12 in the executable helper/handler harness. Root separately passed 128 focused backend tests including existing Win Now API/service and real persistent/verified-session guard regressions; integrated full backend passes 5,645 tests with one optional offline-backtest skip. No live provider, full React renderer or physical-device execution is implied.
- [x] **Code-walk proof — completed:** [final file:line trace](code-walk.md) covers every init writer, generation-before-intent, dispatch/settlement, seeds/verification/navigation commits, exact 409 repair/replay, same-token ordering, latched uncertainty, caller detach, stale 401/403 and deadline-bound awaits. The trace and [build evidence](build-evidence.md) are implementation evidence; independent batch verdicts remain separately recorded.
