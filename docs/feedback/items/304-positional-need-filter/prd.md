...
| **R-9** | Per-job per-rule kill counters are always logged; a WARNING with rule attribution fires when a post-rule deck has <5 cards despite >15 pre-rule survivors. | all | U-R9-1 |
| **R-10** | **No API/payload change**: no new request params; card dict keys and enum values identical flag-ON vs flag-OFF (`trade_card_to_dict`, `server.py:9727`). This is G4's contract guarantee. | contract | U-R10-1 |
