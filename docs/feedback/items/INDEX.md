...
| 417 | pushed-deck-second-search | in-progress — QA A/B PASS, round-1 resolution applied | 2026-09-03 | fast-track bug — the pushed anchored deck still rendered the unanchored `trades.find-btn`; a second tap merged model cards into the fair deck (`TradesScreen.tsx` legacy CTA arm → `dispatchGenerate`, no reset). Fixed on `feat/fb417-pushed-deck-research` (not committed/pushed/merged): CTA hidden for the whole anchored lifecycle (`isResultsPushed && (fairDeck \|\| fairSweepPending)`), `handleFindTrades` resets a fair deck before dispatching, `fairSweepPending` double-tap guard disarmed at **every** epoch bump, the sweep window narrated by the in-progress card, deck-done copy quoting a control that renders (#316), and R-6: a failed anchored sweep retries the ANCHORED search. Both QA agents PASS; six non-blocking findings resolved. Guard: `check-results-push.js` §8 — 23 assertions, 24 sabotages |
