...
Height 40px (36px compact), radius `--r-sm`, Archivo 600 14px, padding 0 16px. Focus: ice ring. Disabled: 45% opacity, no pointer. Mobile: 44px touch floor; under flag `a11y.text_scaling` fixed heights become `minHeight` + vertical padding so Dynamic-Type text can grow the button, and under `ux.touch_polish` the 36px compact variant carries default 4pt vertical hitSlop (44pt effective).
...
| **TradeFinderModeBar** (#156, flag `trades.finder_hub`) | Lateral quick-switch row atop each focused trade-finder mode: a "‹ Hub" back link (ice `body-sm`) over a horizontally-scrolling pill row — Guided · Team · Player · Calc (`--r-pill`, 1px `--line`, min 36px; active = ice border + `--ink-2` well + chalk text), then the mode `heading` title + `body-sm` hint. Guided/Player switch in place (setParams); Team opens the host's in-screen manager sheet then lands via setParams (#156 finish — also the "change team" path when already scoped); Calc/Hub navigate. Presentational — host owns nav | `mobile/src/components/TradeFinderModeBar.tsx` |
...
Two-column pin board (#156 finish, hub Specific Player mode only): the same pins render as the mockup's side-by-side board — two `--ink-1` hairline columns at radius `--r-md`, each with a 2px semantic top rule + matching `label` header (**TRADE FOR** = `--pos`, **TRADE AWAY** = `--flare`; data encodings, not actions), `--ink-2` mini chips (7px position dot + `body-sm` name + 12px `x`, tap removes) and a dashed `--line-strong` add button per column pre-directing the FB-47 picker. Below it (both layouts) the #174 **Trade as one package** row appears with 2+ trade-away pins: the fairness toggle's binary-slider construction + `body` label + `body-sm` state hint; ON = `pinned_give_mode:'all'`.
...
| **TradeValueBar** (FB-157 + FB-169) | Pick-denominated trade verdict — a **diverging** 14px `--ink-3` track centered on "even", NOT the single-direction FairnessMeter. Fill grows from the center line (`--line-strong`) toward the winner: `--pos` (You win) / `--warn` (They win) / centered ice nub (Even). Half-scale = ±1 generic Mid 1st, so fill length **is** `gap.firsts` (clamped to the ±1-first edge). Pick-landmark ticks −1st · −2nd · Even · +2nd · +1st (±2nd at the mid-2nd/mid-1st value ratio ≈0.57). Above: a `heading` who-wins line + margin in mono (`+4,520`) and pick terms (`a Mid 2nd`, tier-`first_1` teal); below: a left-accented counteroffer card ("Ask them to add ≈ a 2nd"). Presentation only — all inputs from `/api/trade/evaluate` (`give_value`, `receive_value`, `favors`, `gap.{firsts, pick_equivalent}`); the engine denominates the GAP as ONE pick-equivalent (never a package pick-sum). Reusable: wired into the calculator verdict (`ConsensusVerdictCard`), shared with the finder card | `mobile/src/components/TradeValueBar.tsx` — reframes the FairnessMeter for trade verdicts |
...
**League rankings — stacked bar chart** (mobile `LeagueSummaryScreen`, #142/#144/#169; spec source `mockups/outlook-odds/league-summary.html`): since #181 this is the **League tab's root page** (stack route `LeagueRankings`, Chalkline stack header "League rankings", no back control at root) with a **"League home" entry row** at the top (LeagueRow construction, testID `league-summary.league-home`) pushing the classic league page (`LeagueScreen` as `LeagueHome`, shared always-on HeaderBack); the legacy root-stack `LeagueSummary` push (deep link `app/league/summary`) renders the same screen without the home row. Every league team is a hairline-separated bar row (rank numeral Plex Mono `--chalk-dim`, ice for the "you" row · name Archivo semi + You badge · a stacked value track · active value Plex Mono + chevron, right). The **track** is a 16px `--ink-2` well (radius 3) whose fill width = the team's active value ÷ league max (min 4% so a bar always shows); the fill is a flex row of **position segments** in the position hexes (QB/RB/WR/TE — data encodings, `cross-client-invariants.md`), each sized to its share of the segment sum so segments fill the track exactly. Below the chart a legend maps the four position swatches. **Basis toggle** (subnav pills): Consensus | My board | disabled "Redraft (soon)" — backend 501s `basis=redraft` (dynasty values only). **Position filter** (pill row, `--r-pill`): "All" (ice border/text when on) + one color-dotted toggle per core position; single OR multi select. On change the chart RE-VALUES to the selected position(s) only and RE-SORTS teams most→least by that filtered value — a pure client-side transform over the payload's per-team `positions[pos].value` (no refetch). A team with zero value under the active filter shows an empty track and a "—" value (never a fabricated bar). Tapping a bar opens the roster drill-in overlay (grouped QB→RB→WR→TE→Other, value-desc within group, dense PlayerCard rows) with its own position filter; the Other group only ever holds players with known metadata (K/DEF) — id-only roster entries (IDP/team-DST ids) are filtered server-side (#183). The mockup's odds/projection layer is DEFERRED — this is the dynasty near-term slice.
