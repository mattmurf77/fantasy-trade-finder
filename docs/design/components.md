...
**Tier step buttons** (feedback #90 — **removed 1.5.4 #98**): the per-tile chevron Icon Button pair is gone from the tile rows. The no-drag paths for tier moves are multi-select's "Tier up / Tier down" buttons (FB-73), the tier-target chips (FB4-62), and the guided Quick set walk (#104) — same movement rules (up → bottom of higher tier, down → top of lower tier; clamps at the top tier ("2+ 1sts")/Bench; never into/out of Unassigned).
**Pool drag guard** (mobile `TiersScreen`, feedback #68): dragging a TIERED player into the Unassigned pool is rejected — the row snaps back with a warning haptic + toast. One-directional: Unassigned → tier drags still work. Matches the tier-move rule that tier stepping never crosses the pool boundary. With the pool section hidden while empty (#105), a drop above the first tier header lands at the top of the first tier ("2+ 1sts") instead of the pool.
**Quick set walk** (mobile `QuickSetTiersScreen`, 1.5.4 #104): guided per-position tier assignment, entered from the Tiers header's "Quick set" ghost action (the slot the Anchors link held — #99). Walks the ladder top → bottom ("2+ 1sts" → Bench): step header = tier tick-label in tier color + `Tier N of 6` progress (tier labels read in pick terms since 2026-07-11 — the former #103 sublabel is folded into the name); body = 3-per-row grid of small tappable player chips (`--ink-1`, hairline, radius `--r-sm`, ≥48px tall: name Archivo semi 12 + position/current-tier micro-labels in their encode colors). Tap toggles membership (ice border + `check` — two signals); footer = Back / Skip / primary "Save {Tier}" that commits ONE tier via `/api/tiers/save` (only the submitted pids change; deselected previously-saved-this-run pids go in `cleared_pids`) and advances. Players claimed by an earlier tier drop from later grids; finishing returns to the Tiers board, refreshed via query invalidation.
