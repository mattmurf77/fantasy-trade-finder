...
**Tier step buttons** (feedback #90 — **removed 1.5.4 #98**): the per-tile chevron Icon Button pair is gone from the tile rows. The no-drag paths for tier moves are multi-select's "Tier up / Tier down" buttons (FB-73), the tier-target chips (FB4-62), and the guided Quick set walk (#104) — same movement rules (up → bottom of higher tier, down → top of lower tier; clamps at the top tier ("4+ 1sts")/Waivers; never into/out of Unassigned).
