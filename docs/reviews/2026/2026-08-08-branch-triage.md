...
| `feat/wave2-init07` | 2 | 2026-06-07 / 394 | DELETE | Merged as PR #71 (confirmed by main's own `docs/plans/archive/2026/perf-optimization/status.md`); persisted query cache + format-scoped keys all on main, superset (`formatExplicit`). |
...
| `feat/mobile-parity-plan` | 1 | 2026-05-20 / 426 | DELETE | Docs-only strict subset of the parity branch; both docs on main under `docs/plans/archive/2026/mobile-feature-parity/` (plan.md byte-identical). |
...
| `feat/feedback-backend-sync-plan` | 1 | 2026-05-20 / 423 | DELETE | 216-line plan doc byte-identical on main at `docs/plans/archive/2026/feedback-backend-sync/plan.md` (renamed by `2e9d542`). |
...
2. **`audit/perf-optimization` + `feat/wave1-perf` (the Q2 question).** Main's own
   `docs/plans/archive/2026/perf-optimization/artifacts/questions-for-user.md` Q2 already asks
