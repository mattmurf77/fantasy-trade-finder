...
> Research caveat: web access was down during this session **and the follow-up second-pass session (2026-06-09)**, so the external-research section is grounded in training knowledge (mechanisms are well-established; exact figures/URLs flagged for verification). §2b adds second-pass code findings verified directly in the repo.
>
...
| Domain | Mechanism | Takeaway for FTF |
|---|---|---|
| **KeepTradeCut** | Crowdsourced "which side would you rather" → 0–9999 value scale; explicit *value adjustment* favoring the fewer-but-better side of uneven trades | Validates trio-voting; do package math in value units, derive the multi-player premium from best-player gap |
| **FantasyCalc** | Regresses per-asset values from thousands of *actually accepted* real-league trades (`Σ side A ≈ Σ side B`), per format | Gold standard for acceptance-calibrated fairness; FTF already logs the needed data (likes, dispositions). Their public API is a drop-in upgrade over `search_rank` decay |
| **DynastyProcess** | Exponential value curve over expert consensus rank + a *win-now ↔ future* blend slider | Same curve family FTF uses; the now/future blend is the clean way to implement team outlook |
...
| **Kidney exchange (Roth/Sönmez/Ünver 2004; Abraham/Blum/Sandholm EC'07)** | Max-weight vertex-disjoint cycles, length ≤ 3, solved by ILP | Blueprint for 3-team trades — at 12-team league scale this is milliseconds of compute and a genuine differentiator |
...
| **eBay Best Offer (Backus et al., QJE 2020)** | ML model predicting P(accept \| offer); acceptance falls steeply with distance from reference price | Train P(accept) on FTF's own swipe/disposition data; rank by expected matched value, not hand-tuned composite |
| **Endowment effect (KKT 1990; T&K 1992)** | WTA ≈ 2×WTP; λ ≈ 2.25 | "Fair" trades get rejected; only perceived-surplus-on-both-sides trades clear. Maximize the *min* perceived surplus |
