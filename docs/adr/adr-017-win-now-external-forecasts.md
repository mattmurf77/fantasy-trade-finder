...
Date: 2026-09-04
Status: Implemented and parent-reviewed; rollout pending
...
Store immutable forecast/projection/scenario evidence and viewer-scoped durable jobs. Record Win Now decisions separately from dynasty Elo and real proposals. Keep three independent serving/title flags default false. Championship output requires separate graduation; legacy title estimates remain unrenderable.
...
The beta can measure roster-sensitive season tradeoffs with reproducible inputs while preserving existing dynasty discovery. It cannot claim calibrated championship improvement or acceptance propensity. Current-status injuries, independent normal residuals, absent NFL game/team and multiweek injury correlations, conservative game-date cutoff, incomplete source coverage and unsupported formats restrict serving. Forecast-source quality, historical or prospective calibration, hosted Python 3.12 CI and a physical TestFlight pass remain rollout dependencies; existing Sleeper authorization governs use. See [BUILD](../plans/win-now/BUILD.md) and [EVIDENCE](../plans/win-now/EVIDENCE.md).
