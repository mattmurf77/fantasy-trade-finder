...
| `POST /api/cron/value-snapshot` | once daily | Upsert consensus value of every universal-pool player into `player_value_history` (#57). Kept separate from `daily-tick` so a push-scan failure can't stop history collection. Provisioned in `render.yaml` (`value-snapshot-daily`, 06:00 UTC) since 2026-07-26; `hourly-tick` additionally carries an idempotent fallback guard that writes today's snapshot if missing (shared `_write_daily_value_snapshots`), so a missed run self-heals within the hour. |
