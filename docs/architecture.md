...
6. **Ordering stack** — the gates above decide what is *eligible*; everything from here only *reorders* (a bounded multiplier never rescues a gated-out card). Applied in `_order_deck` and its callers, each layer flag-gated and inert when off:
   - **F3 fatigue + durable suppression** (`deck.fatigue`): per-user decay over *viewed* impressions keyed by trade_hash / centerpiece / archetype, plus a 30-day near-duplicate ban on declined cards with one low-exposure retest.
   - **F5 taste multipliers** (`deck.taste_vectors`): per-user decayed attribute preferences, clamped to [0.7, 1.4].
   - **F6 learned value model** (`deck.value_model`, **ships dark**): when on, `P(like)·V_like + P(like)·P(propose|like)·V_propose` replaces `composite_score` as the ordering base key; any failure falls back to composite byte-identically.
   - **Thompson sampling** (`trade.thompson_deck` / `deck.thompson_v2`): per-arm Beta draws over archetype×shape, viewed-gated with a pessimistic prior and per-day decay — the deck's only stochastic layer, and the source of the logged `propensity`.
   - **Diversification** (`trade.deck_diversity`): penalizes league-saturated targets; `_cap_per_target` caps cards per counterparty above a floor.
   - **F7 exploration** (`deck.exploration`): one labeled wildcard per deck ≥8 cards at a fixed slot, drawn from the low-taste/low-data pool, with global archetype audition staging.
   - **F9 first-session shaping** (`deck.first_session`): the first deck per league is clamped to 8–10 cards with confidence-passing cards floated into the top 5.
   Consequence for clients, documented in `api-reference.md`: **deck order is deliberately stochastic — `cards[0]` is not the strict `composite_score` max.**
7. **Impressions logging:** two writers run per completed job — the legacy `trade_impressions` rows (generation-time, no shared key with outcomes) and the **F1 signal spine** (`deck.signal_v2`) `_log_deck_signal_impressions`, which writes one `deck_impressions` row per card in final served order with `features_json` **frozen at serve time** plus the applied `propensity`. That frozen snapshot — not a later recomputation — is what off-policy replay and model training read, which is why train/serve skew is structurally impossible. Outcomes (`deck_outcomes`: viewed / like / pass / not_interested / propose / undo) append against `impression_id`; writes are validated for impression ownership and recency before any label or taste update.
