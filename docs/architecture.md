...
flowchart LR
  ...
  SL -->|users, leagues, rosters, players, traded_picks - pick sends| SRV
  ...
  SRV <--> DB
  DB -->|draft_picks grid - propose/validate pick sends| SRV
  RS <--> DB
  TS <--> DB
  TS --> TN
  TR <--> DB
...
**Pick-ownership data flow (draft-extensions W3 M-A, flag `picks.assign`, [ADR-010](adr/adr-010-user-asserted-pick-ownership.md)).** For a league whose platform has no readable draft object — ESPN, which has no rookie-draft concept at all — ownership is asserted by the league's own members and stored **in `draft_picks`**, one row per slot, tagged `source='user'`:

```
POST /api/league/pick-assignments/order   -> database.seed_pick_grid
                                             (pristine grid: every team owns
                                              its own picks, current season + 3,
                                              rounds clamped 1..ROOKIE_MAX_ROUNDS)
                                               |
PUT  /api/league/pick-assignments/<id>    -> database.assign_draft_pick
                                             (CAS on assigned_at, inside the
                                              UPDATE's WHERE clause)
                                               |
                                             record_event('pick_assignment_changed')
                                               |          <- THE audit trail
                                               v
                                          draft_picks (source='user')
                                               |
                        +----------------------+-------------------------+
                        v                                                v
...
1. **Containment is the read default.** `load_draft_picks(..., source='platform')` is the signature default and `NULL` reads as platform, so no pre-W3 reader can see an asserted row by accident. An AST test enumerates every call site.
2. **A writer only ever deletes rows it could have written.** `replace_draft_picks(..., preserve_source=...)` scopes its DELETE to one provenance, in both directions.
...
| `trade_narrative.py` | ~155 | Deterministic template-based rationale strings for trade cards. No LLM. Used by `trade_service.generate_trades()` to fill `TradeCard.narrative`. **Positional-honesty rule (2026-08-15):** a sentence that names a position and a player resolves both from the same source — `_top_received(card, players, positions)` returns the highest dynasty-value received player *whose own position* is in the candidate set (needs / overlap / the `fit_premium` position). Positions from the roster analysis (`match_context.user_needs`) are never pasted next to a player chosen independently; when nothing received fills a need the builder falls through to the neutral fairness sentence instead of asserting a benefit |
| `trade_breaker.py` | new (2026-08-21, ~1.2k) | **Counterparty-breaker evaluation layer** (flags `trade.breaker` = compute+stamp, `trade.breaker_narrative` = the on-card line; both default OFF), [plan](plans/counterparty-breaker/PLAN.md) / [HLD](plans/counterparty-breaker/HLD.md) / [LLD](plans/counterparty-breaker/LLD.md). Predicts the **counterparty's** most likely decline reason for every served card, across six classes (`fit_outlook`, `fit_duplicate`, `value_giving`, `other_player_keep` in pass 1; `fit_new_weakness`, `roster_crunch` in the pass-2 feasibility tier) in the shipped `trade_pass_reasons` layer-2 vocabulary plus the one registered extension `roster_crunch` — so "predicted objection" vs "filed pass reason" is a direct join with no new instrumentation. **Evaluation, never generation:** it runs *after* the whole deck-mutation stack, sets only the two new attributes `card.breaker` / `card.breaker_shadow`, and can change neither deck order nor composition (`test_breaker_zero_ordering_effect`); a grep guard (D-11) keeps every generator and `trade_service` from importing it or reading its stamps. Same leaf discipline as `suggestion_telemetry.py` — imports `trade_service` / `trade_optimizer` / `trade_narrative` and nothing from `server.py`; no DB writes, no HTTP, no LLM, no RNG, no wall clock in any verdict. Card give/receive are swapped as a **view** at evaluation time, never as data, and the partner's present state is built once per `target_user_id` per job (two bulk readers over `league_preferences` / `asset_preferences`). One job = one knob state: all 25 `breaker_*` knobs plus the shared `waiver_slot_cost` are frozen into a per-call snapshot (`_knob_snapshot`), and every valuation runs under an explicit `ts.stud_tax_override("market")` pin so an ambient mode cannot move a verdict. **Absence is impossible by construction:** every card leaves `stamp_breaker` carrying the attribute — a scored payload or a labeled minimal marker `{ver, degraded, objections:null}` — under a five-rung degradation ladder (partner-snapshot / pass-2 budget / budget-exhausted / per-card exception / seam exception), and a per-class predicate error is contained to that class (`skipped:"predicate_error"`) with the card still rung 0. `compose_narration` is a separate deck-level pass with its own eligibility chain and per-(partner, code) repetition suppression; the sentence itself is composed by `trade_narrative.hesitation_line` (templates + `HESITATION_TMPL_VERSION`), which the breaker calls **down** into and which never imports back |
| `negmem.py` | new (2026-08-22, ~1.0k) | **Negative-results memory** (flag `trade.negmem`, default OFF), [plan](plans/negative-results-memory/PLAN.md) / [HLD](plans/negative-results-memory/HLD.md) / [LLD](plans/negative-results-memory/LLD.md) / [ADR-015](adr/adr-015-negmem-soft-prior-not-fourth-filter.md). Turns the viewer's own filed rejections into a **soft generation-time prior**: one bulk read per (user, league) job builds an in-memory `NegmemMap` of `(partner × reason-family)` evidence cells — decayed, shrunk below `negmem_min_evidence`, netted against admitted likes, clamped at `negmem_floor` — which the four generation seams consult as a pure multiplier on `composite_score`. **A leaf, strictly:** it imports only stdlib, `feature_flags` and `database` (`test_n24_leaf_import_contract`), so no engine module and no `server.py` is reachable from it; every seam holds a live module binding (`from . import negmem as _negmem`) rather than a value import, which is what lets a rebind be sabotage-tested. **Derive-on-read, zero tables:** nothing is persisted — the memory is a function of `(user, league, as_of)` and deleting the source rows deletes it. **No literals:** all six knobs are read by the caller and passed in (DE-3), so a missing `model_config` seed row raises rather than silently pricing the memory differently. Two layers ride the one map: **M1**, the multiplier at the seams, and **M2**, aggregate `(accepts, responses)` per partner feeding `trade_gen_v2.acceptance_prior` — whose kill is the GLOBAL `gen2_accept_prior_strength`, never an arm overlay. The ON-condition is **flag ∧ allowlist** (`config/negmem_leagues.json`, `"*"` = every league); a non-allowlisted league is deliberately indistinguishable from flag-off. Operator surfaces are functions and scripts, no route: `negmem_readout` + `backend/scripts/negmem_readout.py` / `negmem_rfps.py`, plus `scripts/negmem-stamp-rate.sql` and `scripts/negmem-gr4-joint.sql` |
| `pick_values.py` | new (#158) | Shared draft-pick value ladder — single home for `GENERIC_PICK_SEEDS` + `pick_pool_value` (generic Mid-tier value, year-discounted in value space). Imported by both `server.py` (re-export; universal pool, anchor wizard, calculator gap) and `database.py` (`sync_draft_picks` `pool_value`), avoiding the `database`→`server` cycle. Also the ladder #157 leans on, so the pick mapping can't drift |
| `draft_status.py` | new (#207) | Per-league rookie-draft detection: signal ordering (Sleeper drafts chain → MFL `draftResults` → rookies-on-rosters), the confidence model, the conflict rules (rosters veto a stale `pre_draft`; an empty drafts list is ambiguous, never "not drafted"), and the `current_year_picks_visible` fail-safe. **Dependency-free by design** — callers fetch (`server._detect_league_draft_status` owns the HTTP), this module decides, so the whole matrix is unit-testable without a network or a DB. Its verdict is cached on `leagues.draft_status*` and read by `/api/rankings` + `/api/trio` to year-tag the generic pick rungs (flag `picks.rank_year_labels`) |
| `draft_board_service.py` | new (rookie-draft M3) | The Draft Room payload builder — one versioned `schema:1` object (`state`/`kind`/`order[]`/`picks[]`/`undrafted[]`/`my_picks[]`/`order_confidence`/`as_of`/`stale`/`degraded`) for Sleeper + MFL. **Read-only and injection-driven:** no Flask import, no HTTP of its own; every upstream read arrives through an injected `Fetchers` (`PlatformFetchers` wraps `server._sleeper_get` and `mfl_service.fetch_draft_results`'s `_opener`), so the committed cassettes under `backend/tests/fixtures/draft/` drive it directly. Owns the per-`(platform, league, draft)` TTL cache (20 s live / 5 min upcoming / 24 h complete) with per-key single-flight, a rolling-60 s ≤3-fetch budget and a 3-strike / 120 s circuit breaker — N viewers of one draft cause ≤1 upstream read per TTL. Polls the 1.2 KB draft-detail object's `last_picked` and fetches the 20 KB `/picks` list only on change (a complete draft's picks are CDN-cached ~24 h). **Two structural invariants, both test-enforced:** it never imports `database.load_draft_picks` (the draft object is truth for the board; #228 deletes `draft_picks` rows at completion, so a board sourced from them empties at the finish line), and it never reads `slot_to_roster_id` (pre-draft it is the identity map — an invented order); slots exist only when `draft_order != null`. Routed at `GET /api/draft/board` behind flag `draft.room` — `server.draft_board_route` is a shim that resolves session + league, binds `_sleeper_get` (which buys fixture replay) and `_rookie_player_ids` (the pool-generation memo) as the production fetchers, and jsonifies. **Only Sleeper is bound today**; MFL rendering exists here and is tested, but M5 wires it behind `draft.mfl`. **W3 M-B adds `assigned_board`** (flag `picks.assign`): the ESPN room, built entirely from the league's own assignment grid — which the ROUTE passes in, because the never-import-`load_draft_picks` invariant above still holds. It does not participate in the cache / single-flight / breaker / budget at all: ESPN has no draft object to protect, so there is **zero platform egress in every state** and a grid is never "stale" (it is a fact in our own DB). `build_board` is untouched and unreachable for ESPN — the route branch precedes it. Fleaflicker and everything else still get the honest `platform_unsupported` state |
| `mock_draft_service.py` | new (draft-extensions W2) | The FTF-native mock-draft engine (flag `draft.mock`). Pure and injection-driven like `draft_board_service`: a frozen `MockContext` carries the consensus map, the rookie predicate's ids, the player rows, the league's rosters and its lineup template, so the module imports no HTTP client and performs no I/O at all — "zero platform egress after creation" is structural, not incidental. (Since #323 `state_payload` labels picks via `RankingService.tier_for_elo` — a new `ranking_service` import, but a pure classmethod over the checked-in `tier_config.json` loaded once at module import, so the no-I/O claim holds at runtime.) **It declares no consensus of its own:** the candidate pool IS `draft_board_service._undrafted(basis="consensus")` over the `server._get_universal_pool()` seed, so the Draft Room's undrafted order and the bots' board are the same list by construction; a user's own board re-sorts only the user's list and never enters a CPU decision. One scoring function, `argmin(rank − need_bonus − reach_noise)`, with the persona (`trade_service.outlook_alpha`) as the only per-team parameter. The noise term is the W2b two-parameter mixture: with probability `mock_bpa_prob` the strict board pick, otherwise a per-candidate `Gumbel` draw whose argmin makes the reach depth geometric in `mock_reach_decay`. **Since W2e the reach is bounded by the operator's round-tiered policy** — at most 3 / 5 / 15 consensus slots in rounds 1 / 2 / 3+, and at most 3 / 2 / 5 reaching picks per round league-wide, after which the round is strict best-available — rather than by the candidate window, which is now a performance bound only (widened 12 → 24 so it never binds). The budget is re-derived from the persisted picks, so a resumed mock spends it identically. State persists in `mock_drafts` as two JSON blobs; per-pick RNG is `Random(rng_seed*10007 + pick_no)`, a pure function of `(seed, pick_no)` rather than of call order, so a resumed mock replays byte-identically. Routed at `POST/GET /api/mock-draft` (+ `/pick`, `/abandon`) via thin shims. **The CPU half is LIVE** — `CPU_MODEL_VALIDATED = True` since 2026-08-08 (`6caca35`) by operator override rather than by the gate passing, so `advance_cpu` is reachable from the routes. The recorded statistical verdict is still FAILED and `test_w2_16_calibration_gate` pins it independently. **Since 2026-08-10 (#290) the scoring function is value-aware:** the candidate set is truncated at a locally-significant value gap (a "run") composed with the round cap via `min()`, so it can only tighten the W2e policy, and positional need is aggregated denominator-weighted rather than by `max()` (which was inert — TE's `(S,B) = (1,0)` made `severity("TE") == 1.0` for almost every August roster). Background in [mock-calibration-2026-08d.md](plans/archive/2026/draft-extensions/mock-calibration-2026-08d.md) (W2d re-balanced the fit/hold-out split for draft depth and added a third validation corpus, with the model frozen: all three KS bars pass, all three paired-mean bars fail, and the residual localises to the candidate window's support bound, artifact §6). **Create-path wiring (W2d, per-platform since #328):** the create route resolves `order` + the traded-pick `ownership` overlay from three sources — Sleeper via `draft_board_service.build_board` (the Draft Room's own cached board; the ONE place the mock touches a platform, and only at creation), ESPN via `server._assignment_grid` → `dbs.assigned_board` (the same grid the ESPN Draft Room renders; DB-only, zero egress), MFL via the normalized `draft_picks` store (`server._mock_owned_pick_overlay` — ownership anchored to the original owner's seeded-shuffle slot; the order itself is never invented, KD-6). Every path stamps `settings.ownership_source` (`platform`\|`user`\|`partial`\|`none`) so the fallback is labeled, never silent; `personas` from `league_preferences` falling back to `trade_service.infer_team_outlook`. `GET` answers a `capability` probe off `mock_draft_service.start_refusal`, the single refusal ladder the create route also reads |
...
| `experiments.py` | ~450 | Analytics P3: the layered experiment evaluator — two-stage deterministic hash (layer bucket + version-keyed variant bucket), targeting over the FR-33b registry, persist-on-first-eval, status machine, launch validation, readout. Fail-open; gated on `experiments.engine`. `variant_overlay` powers the `trade_service` aggression migration |
| `analytics_stats.py` | ~330 | Analytics P3: pure hand-rolled statistics (norm/betainc/gammainc/χ²/z/Welch/SRM + power calculator), golden-tested to 1e-9 vs scipy — **scipy is NOT a runtime dep** (cold-start + RSS budget). No Flask/DB imports |
...
| `trade_block_service.py` | new | FB-147 — imports Sleeper trade-block state. Reads the **public** GraphQL `league_players` query (`settings.otb` = flagging roster_id, `settings.otb_added_at` = epoch ms; unauthenticated, unlike the write surface in `sleeper_write.py`), validates each flag against v1 rosters (drops Sleeper's stale flags; skips pick pseudo-ids), and replace-syncs the `trade_block` table. Called by `session_init`'s background daemon behind flag `sleeper.trade_block`. **Engine hook (WIRED, FB-147): `trade_service._load_on_block_by_uid` reads `database.load_trade_block(league_id)` and `_generate_trades_v2` applies a SOFT, acquire-side `block_boost` (flag `trade.block_boost`, knob `block_boost_weight`) — a card whose acquire side holds a counterparty-flagged player gets a bounded post-gate composite bump, mirroring `need_fit` (reorders acceptable trades, never rescues a gated one). See [feedback/items/147-trade-blocks/engine-hook.md](feedback/items/147-trade-blocks/engine-hook.md).** This module only imports + stores; `server.trade_card_to_dict` still stamps the display-only `on_block` field (which boosted cards reuse for client inspectability). **Owned-pick sync (#158) rides the same daemon:** alongside the trade-block sync, `session_init`'s background writer calls `database.sync_draft_picks` (Sleeper: `_fetch_sleeper_traded_picks` + rosters + league `settings.draft_rounds`) behind flag `picks.owned_sync` — the revival of a sync that went dead in the trade-engine-v2 rebuild. MFL leagues normalize `platform_future_picks` into the same `draft_picks` store via `server._sync_mfl_owned_picks` at link/import, on the daemon's #200 re-normalization step, and (since the #207/#228 MFL parity fix) after every draft-status refresh — which also re-fetches the `futureDraftPicks` snapshot itself (`server._refresh_mfl_future_picks`) so a league linked before its rookie draft stops carrying that season's picks. `_sync_mfl_owned_picks` applies #228's current-season exclusion off the cached `leagues.draft_status` verdict, the same write-path layer Sleeper uses. ESPN writes none. |
| `sleeper_write.py` | ~400 | **Send in Sleeper adapter** (flag `trade.send_in_sleeper`, ToS-adverse — reproduces the private `propose_trade` GraphQL mutation captured 2026-07-02, `docs/plans/sleeper-write-capture-runbook.md`). Pure, no Flask/DB imports, `_opener`-injectable: token crypto at rest (Fernet via `SLEEPER_TOKEN_KEY`, fail-closed), unverified JWT introspection, the live-token oracle (`verify_token_live`), `build_propose_trade_body` (every player in both `k_adds`/`k_drops`, paired with roster ids) and `propose_trade` / `reject_trade`. **Picks (#413):** `encode_draft_pick` formats one `draft_picks` element `"orig,season,round,from,to"`; the propose route (`server.propose_trade_to_sleeper`) splits FTF pick ids out of the mixed give/receive arrays and, only when a pick is present, reads two ground truths — the `draft_picks` grid (`load_draft_picks`, platform rows = existence) and live `/v1/league/{id}/traded_picks` (`_fetch_sleeper_traded_picks`, current holder) — via `server._sleeper_encode_ftf_picks`, refusing the whole send (422 `sleeper_pick_unmapped` / `sleeper_pick_not_owned`) on any unresolvable pick before the write; `/api/trades/validate` mirrors the same two reads as blocking advisories. The client never encodes a pick (a non-empty `draft_picks` body key is 400). Same split-and-encode pattern as the MFL route (`_mfl_encode_ftf_picks`); ESPN hard-blocks. Field 1 (original-owner roster id) is captured, unconfirmed on a multi-owner pick — Q-037 |
| `sleeper_trades_service.py` | new | Market-data readiness (PRD #43 Phase-1 data foundation / #26) — captures **executed Sleeper league trades** raw into `sleeper_trades`: sweeps public v1 `/league/<id>/transactions/<week>` (all 18 legs once as a league's first-time backfill, then only the live leg + the one before it; nothing in the offseason), keeps `type=trade` + `complete` only, idempotent on transaction_id (`database.record_sleeper_trades`). Rides `session_init`'s background daemon behind flag `market.trade_capture`, same best-effort contract as `trade_block_service` / owned-pick sync. Capture only — no scoring/aggregation/UI; `load_sleeper_trades` is the future read seam |
...
| `value_model.py` | new (F6) | **TikTok-discovery F6 (flag `deck.value_model` — SHIPS DARK until an F8 replay win):** learned acceptance heads × hand-set V-vector. Two Platt-calibrated pure-Python logistic heads (no sklearn/numpy) — P(like\|user,card) on viewed→like/pass labels, P(propose\|like) on liked→proposed — trained on the F1 impression spine via `backend.eval.data.load_decks`, features from the frozen `features_json` + F5 prefMatch numerics + partner history (grouped L2: one-hots penalized ~50× harder than numerics; served position is a train-only debias feature pinned to its mean at predict). `rank_score = P(like)·V_like + P(like)·P(prop\|like)·V_propose` with V's read live from model_config (`value_model_v_*`) replaces `composite_score` as `_order_deck`'s **base ordering key only** (`server._deck_value_scores` → `_order_deck(value_scores=…)`); all gates and presentation layers apply on top, and any fallback (flag off, no model, zero-history user, error) serves the composite exactly. Model store: append-only `data/value_model/models.jsonl` (env `VALUE_MODEL_DIR`); nightly refit rides daily-tick after the F8 eval block (flag-gated, idempotent/UTC-day); registered as F8 scorer `value_model` for the nightly replay grade; operator CLI `python3 -m backend.value_model --refit` |
| `outlook/` (package) | new (#169) | **Outlook-odds pipeline** — playoff/championship odds behind flag `outlook.odds`, served by `GET /api/league/outlook`. Five swappable phases, **each a `typing.Protocol` with concrete impls registered in a per-phase lookup**; `pipeline.py` wires them from config via the factories so nothing downstream imports a concrete provider: `league_state.py` (Phase 1 `LeagueStateProvider` — `SleeperLeagueState` reuses `server._sleeper_get`; mfl/fleaflicker/espn are NotImplemented stubs), `strength.py` (Phase 2 `StrengthProvider` — **the key swap seam**: `RosterValueStrength` preseason default from starting-lineup roster value, `TrailingScoresStrength` in-season, `BlendedStrength`; `SleeperProjectionsStrength`/`OwnModelStrength` stubs; source via env `FTF_OUTLOOK_STRENGTH_SOURCE`), `simulator.py` (Phase 3 pure `simulate()` — deterministically seeded `stable_hash(league_id) ^ outlook_seed`, no clock/global-random), `playoff_format.py` (Phase 4 `StandardFormat` seeding/byes/`points_for` tiebreak, plus `settings.playoff_seed_type` bracket-advancement modeling — `0` fixed/no-reseed, `1` reseed, else logged fallback to reseed; the raw value is captured from the Sleeper league-meta response by `server._outlook_sleeper_fetch()`'s `captured` side-channel — no extra network call — and threaded through `run_outlook(..., playoff_seed_type=...)`), `serialize.py` (Phase 5 fixed payload; `meta.beta` is an independent `completed_weeks<6` confidence signal, not an alias of `meta.is_preseason`). Numeric knobs under `model_config` (`outlook_*`). Route builds the value/position maps (same basis machinery as `/api/league/power-rankings`) and calls `run_outlook`; the pure core is unit-tested without I/O in `test_outlook_odds.py`. **Roster-value→points calibration is a flagged heuristic, not fit.** |
...
1. Client `POST /api/trades/generate`; `server._run_trade_job` runs in a daemon thread, loads real league-mate rankings (`member_rankings`), the user's per-player comparison counts (`service.comparison_counts()` — Tier 1 confidence threading), outlook + positional prefs.
   - **Owned-pick candidates (#170/#171, flag `trade.picks_in_pool`):** each team's owned draft picks (from `draft_picks`, capped `picks_pool_cap`, top-N by `pool_value`) are injected as priced `position="PICK"` pseudo-assets into `trade_service._players` and appended to each side's roster, so a suggestion can send/receive a pick. Pure **data inclusion** — the existing PICK pricing (`dynasty_value`) values them; scoring/gates are unchanged. Off ⇒ no pick appears.
...
4. **Mutual-gain gate + harmonic ranking:** packages are valued KTC-style in *each side's own* value space (`package_value_v2`), the side receiving more players pays `waiver_slot_cost` per extra slot, and a trade surfaces only when BOTH sides' surpluses clear `min_side_surplus(_marginal)`. Candidates rank by the harmonic mean of the two surpluses blended with range-overlap consensus fairness, kept in a bounded top-K heap. No QB/star/clogger taxes in this path.
   - **Presentment rules — construction layer (G6 2026-08-16, flag `trade.presentment_rules`):** inside every generator's gate stack (v3 loop + sweetener re-validation, v2 `_consider`, consensus `_emit`, after the #141 filler gate and before feasibility/surplus/fairness) a shared per-job predicate kills packages that are insane on raw consensus sums — R1 #340 overpay ceiling (both sides, fairness-toggle-independent), R2 #341 per-position signed net cap, R3 #339 pick-is-the-gap band — plus the R5 #304 window-scaled need gate on **untargeted** discovery decks (targeted jobs bypass R5 via a server-derived flag in `_run_trade_job`). Construction-time placement means a killed candidate's slot **refills** from the enumeration rather than leaving a hole; never relaxed by the #189 pass. Per-job per-rule kill counters always log; a `presentment-tripwire` WARNING fires on rule-explained thin decks (post-ghost served count).
   - **Presentment rules — eligibility layer (same flag):** `_dedup_and_sort` filters a per-job, league-scoped, **windowless** exclusion set (R4 #336: un-retracted awaiting likes + `pending`/`accepted` matches, built once in `_run_trade_job`) on top of the existing 7-day past-decision filter — covering streaming snapshots and the final deck; `declined` matches and retracted likes may regenerate.
5. **Likes-you injection** (`trade.likes_you`): cards whose mirror a league-mate already liked are flagged/synthesized and pinned to the top (cap 3). The R4 exclusion set also applies here (dedup only — the quality rules stay off this surface per Q21; the D-055 user-gain floor remains its quality gate).
...
7. **Counterparty-breaker evaluation** (`trade_breaker.py`, flag `trade.breaker`): once the mutation stack has finished and the deck is final — post-shaping, pre-ghost-split, so this list is exactly what impression logging receives — every card is scored from the *counterparty's* seat and **stamped** with `card.breaker` (plus the viewer-seat `card.breaker_shadow`). It mutates nothing else, so deck order and composition are byte-identical to the flag-off run. `_log_deck_signal_impressions` then copies both keys **inside** `features_json` (uniform-keys rule: present on every row of a flag-on deck, a labeled marker rather than a bare null when unscored). With `trade.breaker_narrative` also on, a second pass composes the on-card "their likely hesitation" sentence for the narration-eligible subset and, if any card narrated, republishes the job snapshot so the sentence reaches the client on every flag combination. Serialization is **narration-gated**: `trade_card_to_dict` emits a `breaker` object only for a narrated card, so during the dark-stamp window the payload carries no breaker key at all, and `breaker_shadow` never serializes anywhere. Flags off ⇒ the module is never imported and rows, payloads and publish counts are unchanged. See [plans/counterparty-breaker/](plans/counterparty-breaker/).
7b. **Negative-results memory** (`negmem.py`, flag `trade.negmem` ∧ the league allowlist): *before* generation — not after it, unlike the breaker — `_run_trade_job` builds ONE `NegmemMap` for the job and puts it in `_generate_kwargs`, so every bake-off arm consults the same frozen map and the arms differ only through each one's overlay-read `negmem_strength`. Four seams consult it: the v1/v3 serving stack's per-member loop (last in the multiplier stack, after every gate — it reorders acceptable trades, it never rescues or removes one), `trade_gen_v2`, `trade_gen_fit` (ordering only, applied *inside* the 1e-9 quantizing round so same-partner plateau ties survive), and the two `bakeoff_runner` adapters that are the only callers of those two generators in a bake-off job. `_log_deck_signal_impressions` then **copies** the card's consult-time stamp into `features_json.negmem` — copies, never recomputes, because by logging time every arm's config overlay has exited. Flag off, or the league not allowlisted ⇒ no map, no kwarg, no seam executes, no stamp: decks, scores, order, payloads and rows byte-identical. See [plans/negative-results-memory/](plans/negative-results-memory/).
...
| `POST /api/cron/value-snapshot` | once daily | Upsert consensus value of every universal-pool player into `player_value_history` (#57). Kept separate from `daily-tick` so a push-scan failure can't stop history collection. **NOT provisioned in `render.yaml`** — a dedicated `value-snapshot-daily` cron was added 2026-07-26 and reverted the same day (`1e50d3e`, broke blueprint sync). The **operative mechanism is `hourly-tick`'s idempotent fallback guard** (shared `_write_daily_value_snapshots`), which writes today's snapshot whenever a format is missing — cadence is guaranteed by the existing hourly cron alone. (This row previously claimed the dedicated cron existed; corrected 2026-08-14 as part of ADR-011 — the runbook § value-snapshot monitoring had the truth all along.) |
| `POST /api/cron/roster-snapshot` | none (manual/external lever) | ADR-011 Writer C: force the weekly league-state snapshot sweep by hand (daemon + budget, idempotent per `period_key` via the existing `'weekly'` rows). Nothing schedules it — the scheduled path is a weekday-gated pass inside `daily-tick` (Writer B), and the on-sync writer (A) needs no scheduler at all. When a cron migration lands, provisioning this route is a one-line workflow addition. |
...
`trade_roster.py` is a pure exact-assignment and both-team coverage evaluator; `trade_roster_adapter.py` converts provider snapshots without I/O. The server captures one snapshot after package mutations, applies roster protection before market-policy composition, and copies its evidence into impressions. Provisional cards are held until enforcing gates finish. Personal opportunity remains primary under market policy; weaker-team roster/outlook utility precedes market-distance ties. No-board fallback gains roster ordering. Estimated settings and incomplete availability cannot pass enforcement. Details: `docs/plans/post-trade-roster-evaluation/code-walk.md`.
