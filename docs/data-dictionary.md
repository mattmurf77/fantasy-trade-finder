...
| `draft_slot_order` | text (JSON) | **D-090 (2026-08-19)**, flag `picks.slot_labels` — the league's resolved **CURRENT-season** draft order: `{schema:1, season:int, teams:int, type:'linear'\|'snake', reversal_round:int, slots:{roster_id: slot}, source:'sleeper_draft_order'}`. Written by `server._sync_sleeper_owned_picks` from the `draft_order` already carried on the `/league/<id>/drafts` payload that function fetches for the #228 exclusion, composed with its `roster_id → user_id` map — **zero additional upstream calls**. **NUMBERING ONLY**, exactly like `pick_assignment_settings` above and for the same reason: the ORDER is stored, the SLOT never is, so renumbering can never touch an owner (D18). Season-stamped, so a future season resolves nothing (#273). Written as NULL when the order is unset upstream or the season's draft has completed, so a stale order cannot outlive the draft it described. NULL = unresolved → owned-pick labels fall back to their generic round, which is every pre-D-090 row and every MFL league. Read at label time only (`server._league_slot_order`, 300 s TTL cache); **no engine input reads it and no price depends on it**. Accessors: `load_draft_slot_order` / `save_draft_slot_order`; resolution logic: `backend/pick_slots.py` |
...
Market-data readiness (operator directive 2026-07-26; PRD #43 Phase-1 data foundation / backlog #26) — **executed Sleeper league trades, captured raw**. Source: the documented public v1 endpoint `GET /league/<id>/transactions/<week>`, swept by `backend/sleeper_trades_service.sync_league_trades` during `session_init`'s background daemon (flag `market.trade_capture`, Sleeper numeric league ids only, best-effort). The sweep is incremental (2026-09-03): all 18 legs ONCE, as the first-time backfill for a league with no rows here (`database.has_sleeper_trades`), and thereafter only the live NFL leg and the one before it — nothing at all in the offseason. Only `type="trade"` + `status="complete"` rows are stored. Capture ONLY — no scoring, no aggregation, no UI; this table exists so a future observed-market model (PRD #43 Phases 2–3) and league-specific market signals have raw material accumulating from today. Also populated by the operator-run backfill `scripts/backfill_sleeper_trades.py` (2026-08-16), which additionally walks each league's `previous_league_id` chain — so `league_id` here may be a prior-season Sleeper league that has no row in `leagues`.
...
| `confidence_weight` | float, nullable | Weight written with this latest-board row. Historical fractional values remain stored, but D-185 gives identified deliberate ranking entries full authority for **new recommendations**, irrespective of method. Original impression/proposal valuation snapshots remain immutable; never reconstitute an old valuation from this mutable latest-board table. |
| `confidence_source` | str, nullable | `votes` / `seed` / `cross_format` / `explicit`, per player. Placement-band IDs read `explicit`; copy-from-format marks only the actual copied override IDs, not untouched rows published alongside them. Positive comparison count establishes deliberate vote input. Missing source/count remains unknown; do not fabricate provenance. |

**Why these three exist.** The requesting user's live `RankingService` has comparison counts, so `trade_service._shrink_user_elo` could shrink *their* board toward consensus. A league-mate's published snapshot carried only `elo`, so it was used **raw** — the engine trusted a stranger's board more than the owner's own. That asymmetry was measured at 86.9% of boarded-pair cards existing in only one direction (`docs/reviews/2026/2026-08-19-armb-audit-claims-3-4.md` §3) and was called "structural, not tunable" because no knob could close it. These columns are what make symmetric shrinkage possible; `LeagueMember.comparison_counts` / `.confidence_weights` / `.confidence_source` carry them into generation.

**Per-entry read contract (D-185).** `load_member_rankings` adds internal `confidence_sources: {player_id: source}` beside the existing snapshot-majority `confidence_source`, weights and counts. `LeagueMember.confidence_sources` carries it through server/pair policy evaluation. Row provenance takes precedence over old method-based weights for new calculations; a majority of seeded rows must not erase a deliberate entry. This is an additive internal Python contract, not a new column or public partner-ranking payload.

**Deliberately not backfilled.** The evidence behind a historical row may no longer exist; do not fabricate counts or rewrite valuation snapshots. Rows without usable evidence fall back to consensus under the policy evaluator. Existing legacy raw-board generator paths remain a separately documented limitation, not proof of complete fallback coverage.
...
- `breaker` — the counterparty-seat verdict: `{ver ("brk-1"), objections: [one entry per class, all six, always], top, narrated, suppressed, tmpl_ver, outlook_src, outlook_pair, format_gap, degraded, them, ms}`. Each `objections[]` entry is `{code, severity (0–1, 3-dp), evidence{}}` plus `skipped` when that class did not run (`format_gap`, `not_applicable`, `predicate_error`). `code` is drawn from the **shipped `trade_pass_reasons` layer-2 vocabulary** (see [`trade_pass_reasons`](#trade_pass_reasons)) plus the one registered extension `roster_crunch` — that shared vocabulary is the whole point, since it makes "predicted objection" ⨝ "filed pass reason" a join rather than a mapping exercise. `other_text` is never emitted, and `shape_aversion` (a `producer=negmem` code) is a defect. `top` is the argmax over classes clearing their per-class floor, tie-broken by a version-pinned priority constant, and is `null` when nothing clears. `narrated` holds the rendered sentence (non-null ⇒ `top` non-null) and is always null while `trade.breaker_narrative` is off; `suppressed` names why a candidate did not narrate (`below_floor` / `repetition` / `class_ineligible` / `format_gap` / `template_error`). `them` is a passthrough of `fit_diag.them`, never re-scored. `ms` is diagnostics only and never an input to a verdict.
- `breaker_shadow` — the same shape evaluated from the **viewer's own seat** (knob `breaker_shadow_run`), the primary calibration population: the viewer files real pass reasons, the counterparty mostly does not. `narrated` is always null here — the shadow never renders. Null when the shadow knob is off.
- **Readout rules.** Filter on `ver` (a `ver: null` row is the synthetic `flag_flip_or_unstamped` marker written when the flag reads on at log time but the card was never stamped — degraded, never "covered"); rung-marked rows are likewise never covered. `is_ghost = 0` regardless (the no-ghost ruling). Flag-off rows are byte-identical to pre-feature: neither key is written at all.

**Negative-results-memory key (2026-08-22, flag `trade.negmem`; [LLD](plans/negative-results-memory/LLD.md) §3.3/§6.5):** one key, `negmem`, written on **every** row a job produces — served and ghost, every bake-off arm, organic decks included — whenever the job built a map (flag ON **and** the league allowlisted). The value is **never null and never absent within such a job**: the same executemany reasoning as the two families above, one step further — absence carries information here, so "no influence" is a stamped `m: 1.0`, not a missing key. The ON-condition being both flag **and** allowlist is why a non-allowlisted league is byte-identical to flag-off (invariant C1).

- `negmem` — four shapes, mutually exclusive, all carrying `ver` (integer, `negmem.NEGMEM_VER`, currently `1`):
  - **influenced** — `{m, keys, ev, ver}`. `m` is the effective multiplier applied to this card, in `[negmem_floor, 1.0)`, rounded to 4 dp. `keys` is the sorted list of reason families that produced it (⊆ `["fit", "value"]`) — the "named down-weight" the operator checklist looks for. `ev` maps each of those families to its decayed evidence mass at serve time (2 dp), so a row explains itself without re-running the builder.
  - **uninfluenced** — `{m: 1.0, ver}`. The card was consulted and nothing fired: no cell, sub-`min_evidence` evidence, or `negmem_strength = 0`. Arm A (`model_arm = 'baseline'`) stamps exactly this on every row, because `MODEL_A_PROFILE` pins `negmem_strength = 0.0`.
  - **exempt** — `{m: 1.0, exempt: true, ver}`. A likes-you **injection**: built by the injector, which has no consultation seam, so it was never offered to the memory. Only cards with no consult site get this — an *organic* card the injector merely boosted keeps its real stamp, because it was consulted at generation before its like status was known.
  - **degraded** — `{m: 1.0, degraded: true, ver}`. The map built but was discarded as unhealthy (a read raised, or the build exceeded `NEGMEM_DEGRADE_MS`). Overrides every card's own stamp for that job, so a degraded window is visible as a block rather than inferred from missing influence. A **hard** build failure is different: no map exists, so the key is absent entirely and the job is indistinguishable from flag-off.
- **Stamps are COPIED at logging, never recomputed.** The value is whatever the generation seam attached to the card. By logging time every arm's config overlay has exited, so recomputing would stamp arm-A rows with the live arm's multiplier — the reason the assembly code contains no `effective_mult` call and no knob read.
- **`m` on a fit-arm row is real but does not appear in the scores.** The fit seam is ordering-only: it multiplies the rank key, not the published `composite_score`. So on `model_arm = 'fit'` rows `base_score` is pure, while on serving and gen_v2 rows `base_score` already contains `m` — which is what makes the GR4 joint (`negmem_m × final_score / base_score`) count `m` exactly once on every path. Query: `scripts/negmem-gr4-joint.sql`.
- **Readout rules.** Stamp coverage is the health tripwire, not a nice-to-have: while the flag is on for an allowlisted league the expected rate is **exactly 1.0** (`scripts/negmem-stamp-rate.sql`). Anything less means map builds are failing silently. Zero *rows* means the allowlist is empty, not that builds failed. Old blobs are inert serve-time facts — never re-stamped, never backfilled — so `ver` is the only compatibility handle.
...
| `created_at` | str | Immutable ISO UTC of the first reason-bank write; independent of whether a pass disposition was persisted. Refinements never reset this episode anchor |
| `updated_at` | str | ISO UTC — the most recent tap |

Indexes: `ix_trade_pass_reasons_user_league` on `(user_id, league_id)`.

Companion-pass checks bind the immutable reason episode to exact actor/league/trade/exposure evidence. The legacy pass-before-reason bridge is limited to ten seconds and respects intervening consent; elapsed time never resets a surrogate reason key into a new episode. See the [episode-repair trace](feedback/items/419-rejected-interest-resurfacing/backend-code-walk.md#qa-a-episode-repair--final-path-atop-e02d074e).

**This table's `detail` vocabulary now has a second consumer** (2026-08-21, flag `trade.breaker`): the counterparty breaker *predicts* an objection in exactly these layer-2 codes and stamps it into `deck_impressions.features_json.breaker` (above), so the calibration question — did the predicted objection match the reason actually filed? — is a join on `impression_id` and not a mapping table. Two rules follow from that and belong here: a code added to `PASS_REASON_LAYER2` gains a **producer** (who may emit it — user, breaker, or a future sibling), and the breaker's one extension code `roster_crunch` has **no filed-reason anchor**, so it can never be scored for precision against this table. `other_text` rows are unmatched by construction and are excluded from every per-class precision denominator.
...
| `pool_value` | float | **#158** — pick value on the **engine/calculator scale** (`elo_to_value` units), = `pick_pool_value(round, years_out)` = the generic-ladder **Mid**-tier value of the round, year-discounted (0.85/yr) in value space. `years_out=0` equals the generic "Mid <round>" pool pick exactly. Shared ladder lives in `backend/pick_values.py`. **⚠️ This is the STORED value, not the SERVED one (D-148, 2026-08-21).** It is written by the league-wide sync and never rewritten; every read site — the calculator, suggestions, `GET /api/league/picks`, Power Rankings, eveners — prices through `pick_values.priced_pool_value` at read time and reaches this column only as the waterfall's step 3 (DP unreachable, or a season DP neither publishes nor extrapolates to). Do not read this column expecting the number a user sees. A NULL here does not mean "unpriceable": the market can price a row the sync never did (distinct from the legacy `pick_value`, the 0-100 round-tier scale used only for share ratios). |
...
| `team_value_picks` | float, nullable | pick capital, separate pipeline. **⚠️ TIME-SERIES BOUNDARY at 2026-08-21 (D-148, ADR-011).** Rows written from that date price picks through `_priced_pick_value` (per-slot where the draft order resolves); rows written before hold the flat `pool_value` ladder. `team_value` = players + this, so it moves too. Nothing historical was recomputed — `roster_history` has no pricing path of its own, it stores the dict `_power_picks_by_owner` hands it (pinned by `test_league_pick_value_alignment.py::test_history_snapshot_reads_the_same_priced_picks_as_power_rankings`) — so the series is append-only with a step, not rewritten. Measured on a 12-team grid: per-roster pick capital moves +12.4 % (roster picking 1.01) to -40.3 % (1.12), -22.1 % league-wide. Any trend read across that date must say so. |
...
ESPN league linking (`espn.link`, [plan](plans/archive/2026/espn-league-linking-plan-2026-07-11.md)). Encrypted ESPN session cookies for private-league reads — one row per FTF `user_id`. Written by `upsert_espn_credential`; read/deleted by `get_espn_credential` / `delete_espn_credential`. Crypto reuses `backend/sleeper_write.py`'s Fernet helpers (same `SLEEPER_TOKEN_KEY` — one credential-encryption key per deployment).
...
Identity-anchor layer above the app's working key (`sleeper_user_id`) — account-auth plan P2 (docs/plans/archive/2026/account-auth-plan-2026-07-11.md). One row per durable account; provider identities hang off it via `linked_identities`. Managed by `backend/accounts.py` (`find_or_create_account`, `bind_sleeper_user`, `delete_user_data`).
...
FTF-native mock-draft state (draft-extensions W2, [plan](plans/archive/2026/draft-extensions/plan.md) §5 / [lld](plans/archive/2026/draft-extensions/lld.md) §3.3). One row per simulation. A resumable simulation is genuinely stateful — in-memory state dies on a Render spin-down, which is a real event on the free plan — so this is the wave's only new table. Written by `backend/database.py`'s four `*_mock_draft` helpers; every rule lives in `backend/mock_draft_service.py`.
