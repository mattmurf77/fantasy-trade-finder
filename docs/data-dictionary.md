...
| `created_at` | str | ISO UTC |
| `email` | str | Plaintext, normalized lower/trim. **Live since 2026-08-11** (`auth.email_capture` = true; per [email-capture spec](business/product/2026-07-17-email-capture-spec.md)). Populated from the **server-verified Apple/Google identity-token `email` claim** at sign-in — *not* the first-auth-only native credential property — so a repeat auth backfills a NULL (`find_or_create_account`), and an address already present is never overwritten. Apple "Hide My Email" **private-relay proxies are stored indistinguishably**; the only signal is the `@privaterelay.appleid.com` suffix, derived at query time (Apple's `is_private_email` claim is never read — zero hits repo-wide). Plaintext at rest, no encryption. Retained indefinitely while the account exists — no TTL, no reaper. Deleted with the row (`delete_user_data` hard-deletes `accounts`; it does **not** null the columns, contrary to the 2026-07-17 spec) and included in `export_user_data`'s `tables["accounts"]` |
| `email_source` | str | `'apple'` \| `'google'` \| `'user'` — how the address arrived. Written as the **provider name** by `find_or_create_account`, and as `'user'` by `set_account_email`. Two of the three are unreachable today: `'google'` because `POST /api/auth/google` 503s without `GOOGLE_OAUTH_CLIENT_ID`, `'user'` because `set_account_email` has **no callers** (the Settings/onboarding capture UI is unbuilt). `'apple'` is the only value in the wild |
| `email_consent_at` | str | ISO UTC, stamped at capture (consent to product updates + research outreach) |
| `email_unsubscribed_at` | str | ISO UTC, intended for unsubscribe/STOP — **declared but dead: no writer anywhere in the repo**, and no send path to unsubscribe from. `web/privacy.html` §6 deliberately promises no unsubscribe for this reason. Build the writer with the first send, not before |
