...
**Owner contracts (D-185, 2026-09-05):** trade/disposition feedback can reorder within a player's tier but cannot cross it, even cumulatively or after persisted replay. Ranking-service replay fixes the tier bounds after deliberate ranking actions and before the full indirect-feedback sequence. Explicit ranking workflows have equal authority; absent input is not an opinion. Personal tiers define requested Tier up / Same value direction; market value still prices package companions. None of this changes the tier enum, colors or API label vocabulary, and Elo remains internal. These contracts do not authorize new counterparty tier disclosure. Search continuity implementation scope is [owner-contracts](plans/owner-contracts/scope.md).
...
(`SwipableTopCard` a11y actions), `archive/retired-tooling/mobile/maestro/flows/smoke/06-trades-deck.yaml`
+ `archive/retired-tooling/mobile/maestro/capture/onboarding-tour@fresh.yaml` (the flows that tap
...
**Analysis, measured prod impact, and the external sources that DISAGREE with the round-1 call:** [docs/reviews/2026/2026-08-19-pick-year-valuation.md](reviews/2026/2026-08-19-pick-year-valuation.md).
...
| `ftf_tip_199` / `ftf_tip_499` / `ftf_tip_999` | Tip jar, consumable ($1.99/$4.99/$9.99) — money with **NO entitlement**: `entitlements.is_tip_product()` stores the webhook event and grants nothing. Served in the config's `tips` array (`{product_id, display_price}` only). A tip SKU **must** keep the `ftf_tip_` prefix — drifting out of it hands the tipper Pro via the default mapping (pinned by `mobile/tests/check-tipjar.js` §9 + `backend/tests/test_paywall_config.py`) | *(none)* |
