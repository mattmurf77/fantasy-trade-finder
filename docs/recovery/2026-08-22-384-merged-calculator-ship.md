...
**Follow-up branches, same day.** `afc2811` `docs/384-ship-ledger` (squash PR [#174](https://github.com/mattmurf77/fantasy-trade-finder/pull/174) → `af44c76`) · `b19b382` `fix/guide-v2-lit-test` (squash PR [#175](https://github.com/mattmurf77/fantasy-trade-finder/pull/175) → `049a1fd`, the one CI failure on `main` after #172: the 'ships dark' pin for `onboarding.guide_v2`). Docs/test-only; verified by the squash diffs. Remote branches deleted 2026-08-22. Recovery: `git branch docs/384-ship-ledger afc2811` · `git branch fix/guide-v2-lit-test b19b382`

**W7, same day.** `a8e5ef8` `fix/384-tour-device-feedback` — squash PR [#177](https://github.com/mattmurf77/fantasy-trade-finder/pull/177) → `ff9fcbd` (v1.16.1, EAS build 127). Verified by the squash diff (`solveBandPlacement` in `AnalystGuide.tsx`, `registerGuideScroller` in `guideTargets.ts`, `calc.outlook-row` / `trades.swap-first` / `trades.send-btn` targets on `main`). Remote branch deleted 2026-08-22. Recovery: `git branch fix/384-tour-device-feedback a8e5ef8`
