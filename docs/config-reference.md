...
| `FTF_TESTER_ALLOWLIST` | `backend/experiments.py` | Comma-separated experiment unit ids (account ids and/or `device:<id>` pseudo-ids) that resolve the `is_tester_allowlist` targeting attribute to true. Read on the engine's 60s cache refresh; env (not `model_config`) because `model_config.value` is a Float column. Powers operator-targeted rollouts (e.g. `onboarding_v2_rollout`). |
