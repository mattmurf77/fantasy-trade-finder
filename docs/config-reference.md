...
| `bakeoff_fill_policy` | 0.0 (**leave short**) | What happens to a lane slot its own lane could not fill. `0` = **the group serves short and the shortfall is recorded** per (group, lane) in `bakeoff_runs.groups_json[key].short`. `1` = backfill from the same group's own other lane (then its unlabelled remainder), every substitute stamped `deck_impressions.lane_slot = 'fill'` and the shortfall **still** recorded. Leave-short is the default deliberately: `window` is only ~19% of live divergence supply, so the divergence groups miss outlook slots routinely, and whether arm `gen_v2` can produce outlook-basis divergence ideas *at all* is one of the things the bake-off exists to find out — a silent value-lane backfill would erase that while leaving the deck looking full. |
