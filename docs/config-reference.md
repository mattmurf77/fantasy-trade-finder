...
| `trades.finder_hub` | FB #156 — Trade-Finding Hub (Variant B "Launcher Hub"). When **true** the mobile Trades-tab home becomes `TradeFinderHubScreen` (Trade DNA panel + Guided/Team/Player/Calculator launcher cards); each deck mode opens the `TradeDeck` route (a re-entry of `TradesScreen`) with a lateral quick-switch bar, and Specific Team threads `opponent_user_id` into `/api/trades/generate`. When **false** `TradesScreen` stays the Trades home unchanged (the onboarding trades-first auto-gen flow depends on this, so the flag ships **default false** until the hub is validated). No backend behavior is gated by this flag — the additive `position_needs`/`position_surplus` prefs fields and the `opponent_user_id` generate scope are always live and harmless when unused. |
