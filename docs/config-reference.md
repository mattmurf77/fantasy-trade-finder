...
| `mock_bpa_prob` | 0.20 | First of the **two** fitted parameters of the W2b mixture (value re-fitted in W2c against the corrected consensus snapshot; W2b's was 0.50): the probability that a CPU pick is the strict board pick (`argmin(rank − need_bonus)`, no idiosyncrasy). The complement takes the reach branch below. |
| `mock_reach_decay` | 0.70 | Second fitted parameter (re-fitted in W2c; W2b's was 0.95): the reach branch's per-slot survival ratio — reaching one slot further is `this` times as likely, i.e. `P(reach = d) ∝ this ᵈ`, truncated at the candidate window. Implemented as a per-candidate `Gumbel(0, −1/ln(this))` draw, which by the Gumbel-max identity makes the reach depth exactly geometric. |
