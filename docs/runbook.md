...
| Every Find a Trade (including the automatic one at app open) ends in "Search failed"; server log shows `deck signal-v2 impression logging failed … SSL SYSCALL error: EOF detected` then `ValueError: owner_impression_unavailable`; Postgres log shows `client backend … was terminated by signal 9: Killed` and `the database system is in recovery mode` (2026-09-07, first owner-only activation) | The owner-only deck is uncapped (~1,000–1,500 cards) and every row carries ~21 KB of frozen evidence JSON. `save_deck_impressions` handed the whole deck to one executemany, which SQLAlchemy's insertmanyvalues renders as a single ~28 MB `INSERT … VALUES`; parsing it OOM-killed the backend on the 256 MB Postgres plan, the DB restarted, and exclusive mode correctly refuses to serve treatment cards without impressions — so the job failed | Fixed 2026-09-07: `save_deck_impressions` pages the insert (`DECK_IMPRESSION_INSERT_ROWS` = 100 rows ≈ 2 MB per statement, one transaction). Interim mitigation used before the fix deployed: `owner_pair_budget` 4096→300 and `owner_total_budget` 60000→3000 via `scripts/set_knob.py` (source `owner-only-oom-mitigation-20260907`), which cut decks to ~250 cards; restore both after the paged insert is live. Diagnose with the Render logs API: filter the web service for `bake-off run` / `impression logging failed`, and the Postgres resource for `signal 9`. Guardrail: `backend/tests/test_deck_impressions_paging.py` |
...
## Security hardening rollout and historical session tokens

The security-data-hardening change prevents new bearer tokens entering analytics. It does not erase historical copies or invalidate in-memory sessions by itself. No production remediation was performed during implementation.

1. Validate verified mobile sign-in, new Sleeper source proof/merge, co-owner league activation, ESPN/MFL imported league activation, account deletion and relaunch in TestFlight. Ship the matching browser extension and web recovery code together: browser Sleeper verification requires an installed extension and a signed-in Sleeper tab. ESPN/MFL browser entry directs users to the mobile app.
2. Stop every application worker/writer for maintenance. Use an explicitly selected database through an operator-provided maintenance environment variable; the script never loads secrets.local.env or a default production URL.
3. Preview `python3 scripts/remediate_analytics_tokens.py --database-url-env REMEDIATION_DB_URL`. Default mode is read-only; review event-row and exact affected-session counts. For an offline SQLite copy use `--database /absolute/offline.db`.
4. When authorized, repeat the same command with `--apply`. This scrubs identified legacy token values and deletes only matching durable session hashes in one transaction. Restart all workers after applying to clear in-memory tokens. Do not imply a completed database scrub has revoked running worker sessions.
5. Restrict/expire old database backups, exports and logs that may retain token values. After restoring an old backup, reapply revocation/cleanup before serving traffic. The validation harness executes cleanup, rollback, targeted revocation and concurrent writes against an isolated PostgreSQL database. Validate production maintenance access and take the service offline before applying to real data.
