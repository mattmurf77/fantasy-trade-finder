...
- 2026-09-06 [qa] A broad green suite does not prove action-episode identity. Both independent reviewers found #419 reusing an expired same-ID pass to acknowledge a fresh reason after renewed interest; root reproduced the real route returning false-positive `passed:true`. Contrast old evidence with a genuinely new episode in the regression, while preserving same-episode retries and ordinary companion-swipe coalescing. Do not weaken the PRD to fit the passing tests.
- 2026-09-06 [qa] In-memory SQLite with `SingletonThreadPool` can lose its startup connection/schema as background worker threads enter the pool. One independent full run failed on missing `leagues`/`mock_drafts` while the other and root passed the same source; all 149 affected tests passed unchanged with a fresh file-backed scratch DB. Separate this environment result from app findings, retain the failed run, and use distinct disposable file-backed DBs for full threaded regression without changing fixtures or expected outcomes.
...
- 2026-09-06 [triage] Audit an open backlog against deployed source and fresh runtime flags, not the latest local branch or a historical status. In the 45-item audit, three old reports had shipped implementations despite stale open statuses, five complete implementations remained dark, and a named send-card fix existed only on unmerged branches. Record each report's full clauses and distinguish closure candidates, native verification owed, and unavailable code; research alone does not authorize status changes.
- 2026-09-06 [plan] Correlate typed client failures with the minute-level deployment and decision history before assigning a cause. The Win Now loading refusal followed a process rollover and stopped after session initialization; the next server response exceeded the client's 15-second deadline by only 186 ms. The interested-card report involved an older like followed by an exact mirrored pass outside discovery cooldown. Keep session recovery, timeout explanation, stale offer evidence and ordinary rediscovery rules separate.
- 2026-09-08 [triage] The Render logs API is the fastest minute-level evidence source and needs no prod-DB read: `GET https://api.render.com/v1/logs?ownerId=<team>&resource=<srv-id>&startTime=&endTime=&text=&direction=` with `RENDER_API_KEY` from `secrets.local.env` (service id via `/v1/services`, ownerId from the service record). A tester's report timestamp minus ~1 min found the exact failing request for #428 — and the Sleeper propose path logs the provider's error text verbatim in `[WARNING] sleeper propose write-failed`, which turned "still doesn't work" into "Sleeper says these picks cannot be traded" before any planner ran.
- 2026-09-08 [triage] Response SIZE in the access log is a signal: `/api/league/season-projections` answering 200 with a ~98-byte body is the structured `unavailable` reply, not a forecast; compare against a working league's body size on the same day before assuming the prior fix regressed.
- 2026-09-08 [triage] `league_preferences/set` INFO lines carry user/league/outlook/positions per write, so "I set it and it shows not set" can be split into "did the write land" (yes, twelve 200s) vs "what does the client read" (the actual bug) without touching the DB.
- 2026-09-08 [plan] A "still not working" re-report can be a BY-DESIGN refusal wearing a bad message: #422's FFV3 has 8 kicker/IDP starting slots the simulator never supported, and the 98-byte 200 was `unsupported_roster_slots` with the auto-generated text. The planner reproduced it locally by running the unmodified service against live public Sleeper data with a scratch SQLite — no credentials, no prod DB — and got a byte-identical body. Make "run the real code path against the real public league data" the default planner move before proposing engine work.
- 2026-09-08 [plan] Sleeper's public `/drafts` (draft `status`), `/traded_picks` and a rival league's transaction history settled three pick hypotheses in one planner pass (spent current-season picks stay in `traded_picks`; the tradable window is three classes from the first undrafted one; the transaction object is `{roster_id=original, previous_owner_id=from, owner_id=to}`) — and closed a standing open question (Q-037) whose premise was wrong. Prefer public-API captures to code inference for provider rules.
- 2026-09-08 [plan] The planner found a NEIGHBOURING defect (Lakeview refused for `unknown_starter_availability` because three starters were Questionable) that will hide behind the fixed one all season. Carry such findings into the ship summary as separate candidate items, never fold them into the fast-track group.
- 2026-09-08 [plan] A fast-track bug can still need an API addition (#428's new 422). That is not express and not a reason to stall: rule on it in reconciliation-log.md, keep the scope block's docs rows honest, and let the build proceed.
- 2026-09-08 [plan] Concurrent planners write into sibling item folders on the same session branch; a `git add -A docs/feedback/items` for one group swept another group's uncommitted investigation.md into its commit. Add per-group paths, not the items root.
- 2026-09-08 [build] Pre-created worktrees with a SYMLINKED `mobile/node_modules` from a populated sibling worked for every mobile build/QA agent this run (tsc, all guards, testid-lint) — the 2026-08-10 "never symlink" lesson was about a symlink to an EMPTY main checkout, not about symlinks per se. Keep the 2026-09-02 rule: link to a sibling that `ls … | wc -l` proves populated.
- 2026-09-08 [build] Two 20-minute pytest suites running concurrently on one laptop stretched each to 24–32 min and pushed every build/QA agent past the 600 s foreground cap; the harness backgrounds the process and the agents then "wait for a notification" that may never come. Tell agents up front: if backgrounded, READ the output file for the summary line; and stagger full-suite runs across groups where possible.
- 2026-09-08 [qa] Two independent QA reads converged on the same non-PRD finding in both mobile (G-423: league switch on the plan beat marks the new league done) and backend (G-428: startup-shaped completed drafts no longer excluded) groups — the redundancy paid for itself twice in one run. When a build narrows a pre-existing behavior "per spec", ask QA explicitly whether the old behavior was the safe one.
- 2026-09-08 [qa] Regex guards that match on `node.getText()` are satisfied by COMMENTS (G-428 check 9 passed with the `detail` operand deleted because the comment still said "detail"). Match on the call's argument nodes, and prove each guard RED by deleting the operand, not the whole branch.
- 2026-09-08 [qa] A single full-suite failure under machine load (`test_rookie_scope` 401 `session_expired` mid-loop) was a flake: the file passes 34/34 alone and the sibling agent's run of the same commit was green. Rule: one flaky failure ⇒ re-run that file in isolation and cite the other agent's run before treating the suite as red or green.
- 2026-09-08 [ship] Living-memory inserts done with an unasserted `str.replace(anchor, …)` silently NO-OP when another session's entry has taken the top slot: two TEST_LEDGER bodies (the card-shape fix and the flag flip) were missing for a day while their TOC lines existed. Always `assert anchor in s` (and `count == 1`) before inserting, and anchor on the file's first H2 read at write time rather than on a heading you expect to be first.
- 2026-09-08 [ship] Production is available as REAL runtime evidence for a backend valuation change, and it beat the code-walk: after Render went live, one public `POST /api/trade/evaluate` proved #427's rule end to end (two mid firsts → `give_value 4234.0` face with no `package_depth` row vs 4210.2 → `favors: even`; two mid seconds still −225.9 and taxed). Budget one post-deploy smoke per shipped backend rule, using the route's real key names (`give_player_ids`/`receive_player_ids`/`scoring_format`, not `give`/`receive`/`format`) — the wrong keys return a 400 that reads like a bug.
- 2026-09-08 [ship] Branch-deletion proof for a five-branch batch is cheapest as a transitive argument: every group tip `git merge-base --is-ancestor` the tested release tip, the release tip has complete-tree equality with fetched `origin/main`, and each group's OWNED files diff 0 against main. Comparing a group branch's whole tree to main is meaningless (it lacks the other groups' work) and will scare you into thinking work was lost.
- 2026-09-08 [ship] Worktrees whose `mobile/node_modules` and `secrets.local.env` are symlinks are safe to `git worktree remove` — the link goes, the target stays. Say so in the recovery ledger so the next run does not hesitate.
- 2026-09-08 [ship] End-of-run cleanup must delete ONLY the batch's own worktrees. This repo carries ~20 worktrees from other sessions under `.claude/worktrees/`; `git worktree list` after cleanup should still show them.
