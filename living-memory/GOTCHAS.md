...
| ID | Symptom | Area |
|---|---|---|
| G-042 | Instrumenting `trade_service._consider` yields an all-zeros funnel — prod serves v3 | Backend / trade engine / flags |
...
### G-042 — instrumenting `trade_service._consider` measures a code path production never runs
- **Symptom:** you add counters/logging/a metric to the gate cascade in `trade_service._consider`, ship it, and the funnel reads **all zeros** (or only the handful of jobs that fell back). Nothing looks broken; the code is plainly reached in tests.
- **Cause:** there are **two** deck engines and the docs habitually name the older one. `config/features.json` ships `trade_engine.v3: true`, so ranked-opponent pairs are generated by `trade_optimizer.generate_pair_trades_v3` — `trade_service._generate_for_pair_v2` / `_consider` is the **v2 fallback**. Tests that construct `TradeService` directly exercise v2 happily, which is why this survives CI. The trade-relevance LLD made exactly this mistake: it anchored the P0-6 gate counters on `_consider` alone.
- **Fix:** instrument **both** engines with identical counter keys, and assert the conservation identity `considered == passed + Σ kills` per engine so a missing early-return shows up as arithmetic that doesn't close. v3 has two gates v2 lacks (`gate_shape`, `gate_lineup_feasibility`); v3's pinned-give filter sits on the **outer** loop and kills a whole give×receive row, so count `len(recv_subsets)` there, not 1, or the identity breaks.
- **Prevention:** before instrumenting any engine path, read the flag that selects it (`trade_engine.v3`, `trade_engine.v2`) in `config/features.json` — not the module that sounds canonical. Related trap in the same area: the **consolidation** gate named in older docs lives in the asset-ideas path (`trade_service.py:3787`), not in either deck path; and counters accumulate across the #189 relaxed retry (a targeted empty job re-runs up to 2×), so the funnel is a ratio, never a per-job count.
