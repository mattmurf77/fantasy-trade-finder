...
- **Which native source actually wins (established 2026-08-25, correcting the line above):** in THIS project only `Info.plist` does. `project.pbxproj` sets `INFOPLIST_FILE = DTFDynastyTradeFinder/Info.plist` and never sets `GENERATE_INFOPLIST_FILE = YES`, so Xcode copies the checked-in plist — whose `CFBundleShortVersionString` is a LITERAL — and `MARKETING_VERSION` is inert for the shipped string. The 2026-08-24 HANDOFF said exactly this and left `MARKETING_VERSION` stale on purpose; it was right, and a 2026-08-25 pass initially mis-read the 1.16.4-vs-1.16.5 gap as live drift. **Still set both** (v1.16.6 does): the cost is one line, the two values disagreeing is a standing trap for the next reader, and the day someone regenerates the native project with `GENERATE_INFOPLIST_FILE`, `MARKETING_VERSION` silently becomes authoritative and ships the stale number.
- **Pre-build check:** `grep -A1 CFBundleShortVersionString ios/*/Info.plist` is the value that ships; confirm `MARKETING_VERSION` matches it and that no `GENERATE_INFOPLIST_FILE` has appeared. `app.json` `version` is documentation only. `android/app/build.gradle` `versionName` is stale at 1.0.0 and left alone on purpose (Android is not a shipping target).
...
- **Fix:** before trusting a null result, (a) grep for every read of the knob, (b) check for a hardcoded sibling constant downstream, (c) confirm the patch target matches the *calling* namespace, and (d) check the co-kill data in [`../docs/reviews/2026/2026-08-19-knockout-waterfall.md`](../docs/reviews/2026/2026-08-19-knockout-waterfall.md) for a rule that would catch the same trades.
...
### G-063 — `league.members` EXCLUDES the caller, so any "is X in this league?" check answers NO for the user themselves — and the standard test fixture hides it
- **Symptom, plainly:** a feature that has to confirm the signed-in user belongs to their own league refuses to work — for every user, in every league, 100% of the time — while the test suite stays green. The user-facing message usually blames the *other* person, because the code that resolves both sides of a two-party interaction only reports "someone isn't a member".
- **Cause:** `sess["league"].members` is **caller-excluded by app-wide convention**. `/api/session/init` builds it from the client's `opponent_rosters` (`backend/server.py:18949-18964`) — which every client already filters the user's own roster out of (`mobile/src/api/auth.ts:366-367`, `espn.ts:232-233`, `platformLink.ts:279-280`) — and the DB-member merge then explicitly refuses to re-add them: `existing_member_ids = {m.user_id for m in members} | {user_id, league_user_id}` (`backend/server.py:18974`). So `{m.user_id: m for m in league.members}.get(_league_user_id(sess))` is **always `None` in production**. The caller's own roster lives on the session as `sess["user_roster"]`, nowhere else.
- **This has now bitten four times:** FB #41 → #291 → **#295/#296/#305** (mock draft: the user missing from their own draft) → **#409** (2026-08-30: `POST /api/trades/queue` returned `not_league_member` for every ✓ tap in the merged calculator, from 2026-08-22 to 2026-08-30, and the client rendered it as *"@partner isn't in this league"* — blaming the counterparty for a caller-side miss).
- **Three of the four were hidden by the same thing: a fixture that puts the caller INSIDE `members`.** That shape is easy to write, reads as obviously correct, and production never produces it. `backend/tests/test_calc_trade_queue.py` built its league that way, so the happy path, every refusal case and the idempotency case all passed against an impossible session while the shipped feature refused 100% of real taps for eight days. The mock-draft defect failed identically (`docs/feedback/items/295-mock-user-not-in-draft/hld-2026-08-13.md:433-445`).
- **Fix:** resolve the caller from the **session**, and the members list for everyone else — the `_mock_owner_ids` / `_mock_rosters` pattern (`backend/server.py:15167-15200`). Where a `LeagueMember` object is needed, synthesize one **local to the route** from `_league_user_id(sess)` + `sess["user_roster"]` (`queue_trade_for_opponent`, `backend/server.py:13316-13335`).
...
- **Cause — a recovery cycle, not a retry bug.** `apiFetch` (`web/js/app.js`) has a global 401 branch: if the body is `{"error":"session_expired"}` it clears the token and "recovers" by calling `initSession(savedUser, savedLeague)`, or `boot()` when there is no saved league. That is sound for a Sleeper user, whose recovery path re-mints a session via `POST /api/session/init`. It is a **closed cycle** for an entry user, because the web entry flow (§V3) routes `entry:` ids to `GET /api/{espn,mfl}/leagues`, which **requires** a session and can never create one — so recovery calls the read, the read 401s, the handler calls recovery, with no counter, no backoff and no terminal state.
- **Why it hid:** every functional test had a live session, and the flow is correct end to end with one. Only a *dead* token reaches the branch, and nothing in the happy path or the unit tests ever produces one. It surfaced by accident — a stub server restart left a stale token in `localStorage` and the loop ran underneath an unrelated test, corrupting its result.
- **Fix / prevention:** the platform-snapshot read is a **plain `fetch`** with the token attached by hand, never `apiFetch`, so the global recovery cannot fire on it (the same rule `_entryPost` already followed, for the same reason). A 401 there returns an `ENTRY_SESSION_LOST` sentinel that every one of the five call sites propagates to `_handleEntrySessionLost()`, which clears user + league + token and returns the visitor to the landing to re-claim the team — the only real recovery, and cheap because `entry:` ids are deterministic (same team → same id → same boards). Clearing the saved **user** is what makes it provably terminate: `boot()` and `showLeagueScreen()` both re-enter the entry path while a user is saved. Verified after the fix: exactly **2** requests (the two parallel callers), then the landing.
- **The general rule:** before routing a request through a shared client that has *any* automatic retry or re-auth behaviour, ask whether that behaviour's recovery path can reach this same request. If it can, the request must bypass the shared client. A recovery handler is only safe when its recovery strictly cannot re-trigger its own trigger.

## 2026-09-01

### G-064 — `bakeoff_serve_interleaved` has three different defaults and a docstring that asserts the opposite of what it returns: a missing DB row means the arm SERVES
- **Symptom, plainly:** you read the code to find out whether an experimental trade-generation arm is reaching real users, and every comment, seed and docstring tells you it is dark. It is not necessarily dark. An agent reading this file concluded a whole engine module was dead code and excluded it from an analysis; a second agent, reading the changelog instead of the code, concluded the same module served a third of every deck. Both were reading the repo honestly.
- **Cause — the same constant is declared four times, and they disagree:**
  - `backend/bakeoff_runner.py:247` — `_cfg("bakeoff_serve_interleaved", 1.0) >= 1.0`. The **inline fallback is 1.0**, so an absent `model_config` row resolves to **serving**.
  - `backend/trade_service.py:653` — `_DEFAULT_CFG` carries **0.0**.
  - `backend/database.py:2594` — the DB seed is **0.0**.
  - `backend/feature_flags.py:954` — a GUARDRAIL comment (#360, 2026-08-19) saying **do not raise it**.
  - And the `serve_interleaved()` docstring (`bakeoff_runner.py:229-246`) states *"back to 0.0 = DARK (operator, 2026-08-19)"* and *"Arm C has never served a single card"*.
- **What prod actually read:** [`CHANGELOG.md`](CHANGELOG.md) (2026-08-26, [D-165]) records checking prod first and finding `bakeoff_serve_interleaved = 1.0` **live** — not the 0.0 its seed and [Q-031] both assumed — alongside `bakeoff_include_gen_v2 = 1.0` (seeded 1.0 at `database.py:2602`). This entry does not re-assert today's prod value; the point is that **the repo cannot tell you, and reads as though it can**.
- **Why it matters beyond one knob:** `trade_gen_v2.py` reads **no** positional preference (`avoid`/`acquire`/`trade_away`) and touches `position_needs`/`position_surplus` only inside `_rationale` — it *narrates* positional fit and never optimizes it. So the share of the deck this knob controls is exactly the share on which every positional preference is ignored. It is the standing reason [`trade.avoid_positions`](../config/features.json) (#360/#361) is held dark ([Q-032]).
- **Resolved by a prod read, 2026-09-01 (`GET /api/admin/config`, 237 rows) — and the resolution is the lesson:** the [D-159] consolidation bundle **IS applied** in prod (`filler_min_frac` 0.15, `overpay_adjusted` 0.0, `trade_elo_gap_max` 0.0, `v3_shape_max_delta` 2.0), even though [`TEST_LEDGER.md`](TEST_LEDGER.md):426 recorded it in the future tense, no CHANGELOG/HANDOFF entry confirmed it, and [`NEXT.md`](NEXT.md):86 still listed the bundle as remaining. Two independent analysts spent an afternoon parametric in a value one request would have settled. Same read confirmed `bakeoff_serve_interleaved` 1.0, `bakeoff_include_gen_v2` 1.0, `bakeoff_include_challenger` 1.0 — so **both arm C (gen_v2) and arm D (challenger, [D-095]) are serving live deck groups**; D-095's "Dark" status line and the `serve_interleaved()` docstring are both stale. Unchanged from code defaults: `sweetener_gap_threshold` 1539, `need_fit_weight` 0.15, `need_gate_upgrade_margin` 0.0. Absent (code default applies): `deck_headliner_cap`, `deck_give_headliner_cap`, `user_elo_shrink`, `consensus_both_ways`, `consensus_fairness_floor`.
- **Fix:** make the inline `_cfg` fallback match the seed (`0.0`), and delete the duplicate declaration in `_DEFAULT_CFG` or make one the single source. Rewrite the `serve_interleaved()` docstring — it documents a 2026-08-19 operator decision that a 2026-08-26 prod read contradicted, and it is the specific text that misleads.
- **Prevention:** for any trade-engine question that turns on a knob's value, **read prod `model_config` before reasoning** (`GET /api/admin/config` with `CRON_SECRET` from `secrets.local.env`). Seeds, `_DEFAULT_CFG` and docstrings are three independent claims about a value that lives in a fourth place. This is the third instance of the pattern ([Q-031] found the same knob at 1.0 against a 0.0 seed; [G-058] is the tuning-side cousin). Note `backend/tests/test_avoid_positions.py:718` already asserts the config *mentions* this knob at least three times — a guardrail that counts declarations without reconciling them.

### G-065 — The gap-distribution harness is still not deterministic with `PYTHONHASHSEED=0`: the dominant source is the engine's wall-clock deadlines, and they shape prod decks too
- **Symptom, plainly:** two back-to-back runs of the committed harness recipe, both with `PYTHONHASHSEED=0` as [G-053] instructs, disagree by **3 cards** in one arm (12t / v2_only / B_current: 20 vs 23). G-053 attributes the harness's drift to hash order and says ±1 card; that is real but it is the smaller effect.
- **Cause:** the engine carries per-pair and per-job time limits that fire on the clock, not on work: `_deadline = time.monotonic() + 1.0` per pair (`backend/trade_service.py:6502` consensus, `:7356` divergence; checked at `:6844/:6854/:6866/:6878` and `:7520/:7575/:7628`) and the `full_sweep_budget_s` sweep stop (`:4884`, `:6446`). A slower run — a loaded laptop, a CI runner, a hot Render instance — enumerates fewer candidates and emits a different deck. The prototype worktree hit the 30 s sweep rail at pool-knob 6 on the 16-team fixture and silently served 17 cards instead of 32.
- **This is not only a measurement problem.** The same deadlines run in prod: **the deck a user is served is whatever fit in 1.0 s per pair on that instance at that moment.** Two users with identical leagues and boards can get different decks; the same user can get a different deck on a busy evening. No analysis of "why did the engine suggest X" is complete without knowing whether the pair that should have produced Y hit its deadline first.
- **Fix (measurement):** freeze the clock in the harness copy — set `trade_service.time.monotonic` to a constant so neither cutoff fires; the loops still terminate on the deterministic `max_candidates` / `_iter_budget` caps. Two runs are then byte-identical (verified 2026-09-01, sha256 `78eef9b58b7f6572…`). Every number from a frozen-clock run is the **no-time-pressure** deck; label it so.
- **Prevention:** any harness comparison states (a) seed pinned, (b) clock frozen or not, (c) which side ran on what machine. For prod telemetry, consider stamping `deadline_hit` per pair on `deck_impressions` so a deck can be read as complete or truncated — today that fact is not recorded anywhere.

### G-066 — Arm C (`trade_gen_v2`) stamps every card `basis="divergence"`, so its basis split is fiction
- **Symptom, plainly:** a like-rate readout split by `basis` (consensus vs divergence) — the exact mitigation [D-095] prescribes for grading a two-sided arm — shows arm C at 0% consensus cards. Arm C does serve unboarded partners; it just labels them wrong.
- **Cause:** `backend/trade_gen_v2.py:1204` hardcodes `basis="divergence"` on every emitted card, regardless of whether the partner had a board.
- **Fix:** stamp basis from `member.has_rankings` the way `trade_service` does. **Prevention:** any `basis`-split metric on `deck_impressions` filters arm C out or notes it is unlabelled until this is fixed. Found 2026-09-01 by the knob-sweep harness run (the harness's consensus counter read 0 for arm C in every cell).
