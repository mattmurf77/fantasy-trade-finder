...
- **Cause:** `sess["league"].members` is **caller-excluded by app-wide convention**. `/api/session/init` builds it from the client's `opponent_rosters` (`backend/server.py:18928-18944`) — which every client already filters the user's own roster out of (`mobile/src/api/auth.ts:366-367`, `espn.ts:232-233`, `platformLink.ts:279-280`) — and the DB-member merge then explicitly refuses to re-add them: `existing_member_ids = {m.user_id for m in members} | {user_id, league_user_id}` (`backend/server.py:18952`). So `{m.user_id: m for m in league.members}.get(_league_user_id(sess))` is **always `None` in production**. The caller's own roster lives on the session as `sess["user_roster"]`, nowhere else.
