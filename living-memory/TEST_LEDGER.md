...
- **Pytest:** `test_presentment_rules.py` **45 passed**; touched-module regression sweep **281 passed** (incl. `test_finder_targeting` — pin enforcement holds); **full backend suite on the branch: 2969 passed / 1 skipped / 0 failed (264s)** — note the branch's BASE was already 1-failed (`test_release_flags_mirror_features_json`: `3c0541c` lit `suggestion.telemetry` in features.json without fixture parity; fixed on this branch alongside mirroring the new flag); `import backend.server` OK.
