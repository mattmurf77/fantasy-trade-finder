...
- **Owed on ship:** PR → CI green → merge; then the 3-step TestFlight checklist in scope §3 (Matches send → `deck_outcomes` `action='propose'` row; browse like → `props.source='browse'`; calc ✓ → `'calculator'`). The propose leg is the three-model bake-off's strongest per-arm label — worth verifying on real prod data after the next build.
