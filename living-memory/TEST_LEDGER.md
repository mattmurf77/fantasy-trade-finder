...
## 2026-08-29c — layer-1 decline pass now carries dwell/engagement (client-only fix, on branch, NOT merged)

Branch `claude/zealous-mirzakhani-f1664e` @ `49d3d033` — awaiting merge. Companion to the backend first-pass-wins guard (`deck_pass_outcome_recorded` in `_save_deck_outcome_safe`, shipped from `claude/unruffled-meitner-3596cb`): with that guard live, a layer-1 decline tile tap fires `POST /api/trades/pass-reason` before the swipe POST, so the SURVIVING pass row was the reason write's — which carried no dwell — and the swipe's dwell-bearing row dropped as a duplicate. Every reasoned pass had NULL dwell.
...
- **Runtime note for the next TestFlight pass:** after merge, a layer-1 reasoned pass should produce ONE `deck_outcomes` pass row with non-NULL `dwell_ms` (verify via the impression spine, not the app UI).
