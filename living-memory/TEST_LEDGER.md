...
## 2026-08-14 — Trade-relevance engine P0 (branch `feat/trade-relevance-p0`, pushed, UNMERGED)

- **Change:** LLD build steps B1/B2/B4/B5/B6/B7/B8 + the admin relevance report + PRD R12 doc hygiene. **FULL GATES** (schema, flags, API surface — the bright line, never express). Scope block: [`docs/plans/trade-relevance-engine/scope.md`](../docs/plans/trade-relevance-engine/scope.md). Both user-affecting flags (`deck.dedup`, `deck.class_demotion`) ship **dark**.
- **Backend, merged tree @ `f6ee573`:** `pytest backend/tests/ -q` → **2938 passed, 1 skipped** (3m50s, local SQLite, Python 3.14). Baseline at branch point (`origin/main` @ `21df73f`) was 2877/1, so **+61 net new tests, zero regressions**. `test_rookie_scope.py` is INCLUDED and passes (34) — it is byte-identical to `main`, so the "6 pre-existing py3.14-only failures" carried in the 08-13/08-14 entries **no longer reproduce**; stop excluding it.
- **New suites:** `test_p0_schema` (12), `test_relevance_config` (21), `test_relevance_batch` (14), `test_deck_dedup` (21), `test_pass_ledger` (27), `test_disposition_join` (18 + 2 taste invariants), `test_gate_counters` (17), `test_propensity_freeze` (15), `test_class_demotion` (29).
- **Sabotage discipline (house convention) — every behavioral test names the sabotage that must make it fail, and the load-bearing ones were *executed*, not just asserted:** dedup 15 scripted sabotages all verified red (tau `>=`→`>`, dropped partner/centerpiece clause, keep-lowest-key, likes_you immunity removed, min-cards restore removed, measurement gated on the flag, `key` not pruned, dedup moved below the Thompson draw, empty-union=1.0, merge→clobber, …); ledger 4 (deleted `is_aug25` guard → 3 fail, deleted stale-`running` branch, dropped per-pass try/except → 4 fail, inverted valve polarity → 20 fail); join 4 (swapped label map → all 4 2×2 cells fail, swapped impression binding → all 4 fail, deleted idempotency SELECT, try/except round the insert); B4/B8 4 (widened drift tolerance, dropped multiplier freeze → 4 fail, neutered `pick_swap_ok`, uncounted early return in `_consider` → 4 fail); schema 3; config 4.
- **T-1 tick equivalence (the merge gate for B1):** 5 fixtures — 3 ordinary days (incl. honest-winbacks both ways and replenish-on) and **2 Aug-25 days** — diffed against a verbatim copy of the pre-refactor tick body pulled from `git show 2080540`. Identical call sequence and payload-minus-`passes`. The Aug-25 fixtures exist because splitting `season_start` into its own pass would otherwise have double-sent on one day a year.
- **T-4 propensity integrity:** with the full stack live, sorting by `base × propensity × Π(frozen multipliers)` reproduces the served `card_index` order and the per-row identity holds to 1e-9; an injected unlogged ×1.3 on 1 of 5 cards ⇒ 1 violation, 0.20 > 2% threshold, `untrusted` marker written, pass raises. This is the tripwire for a future reorder layer that neither freezes nor logs its factor.
- **T-23 two-sided in one fixture:** 3 flags/40 exposures ⇒ demotion **exactly 1.0** (noise must never demote) AND 150/500 ⇒ **0.581** (<1.0, ≥ the 0.5 floor) AND a healthy 20/1000 ⇒ 1.0.
- **Not run / owed before merge:** PR CI (green required); the tier-4 sim-gate declaration is argued in the scope block §5 (no user-visible mobile surface, both flags dark) so `githooks/pre-push` will need `FTF_SKIP_SIM_GATE=1`; no Maestro delta (waived on the same ground, §3). No production deploy, no flag flip.
- **Merge hygiene:** built across three worktrees; two merges into the branch. The second had 4 add/add or both-added conflicts (`relevance/passes/__init__.py`, the registry list in `server.py`, the pinned pass-order test, a data-dictionary row) — all resolved keeping **both** sides; full suite re-run **after** resolution, which is the number quoted above.
