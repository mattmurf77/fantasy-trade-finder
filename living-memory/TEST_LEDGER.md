...
## 2026-08-11 — P1-3 email capture: flag flip + privacy policy (NOT merged, branch-only)

- **Change:** branch `p1-remediation-2026-08-11`, two commits (`b35335f`, `5b7b537`). (1) `auth.email_capture` false → true in `config/features.json` **plus the three derived flag fixtures**, the `feature_flags.py` comment rewritten, `web/privacy.html` §1/§2/§5/§6 + effective date + header provenance, and three new tests in `test_email_capture.py`. (2) Docs re-sync: `config-reference.md`, `data-dictionary.md`, `runbook.md` (new mandatory App Store pre-submission checklist section), `2026-07-17-email-capture-spec.md` status block. **Zero production code changed** — the capture path, its gate and its four unit tests were already merged; only the boolean moved.
- **Verified (this worktree):** `python3 -m pytest backend/tests/ -q` → **2507 passed / 1 skipped**, against a **2504 / 1 baseline measured on this same branch before the first edit**. The **+3 is exactly the three tests added** (`test_release_flag_and_privacy_policy_ship_together`, `test_delete_account_removes_email`, `test_export_includes_account_email`) and contains **no pre-existing failures**. The four pre-existing `test_email_capture.py` tests are additions-only in the diff and stayed green — they monkeypatch `_email_capture_enabled`, so movement in them would have meant something other than the flag moved. `test_seed_ui_test_db.py` **76 passed**, i.e. the three fixture-mirror tests survived the flip.
- **Falsification — the pairing test earned its keep on its first run.** It went red immediately, on my own header comment, which quotes both retired sentences as provenance. That is a true positive for a grep and a false positive for a *claim*, so the assertion now strips HTML comments and tests only what the page serves. Then sabotage-proven three ways against a deliberately broken tree: restore "We never store your email address itself" to §1 → **red**; restore "No email addresses" to §2 → **red**; set the flag back to `false` → **skips** (the asymmetry is deliberate — policy ahead of flag is over-disclosure, not a breach). All three reverted; suite re-run clean.
- **Known weakness, recorded not fixed:** the pairing test is a **negative** assertion. A future rewrite that deletes both sentences while saying nothing about capture would pass it. Closing that needs a positive marker (a stable HTML comment token the test greps for); considered and **not adopted**, since it would constrain future legal copy. If the operator wants it, it is one test line plus one HTML comment.
- **Structural checks:** rendered policy text contains none of the three retired claims (comments and `<script>`/`<style>` stripped); `web/privacy.html` parses with zero unclosed tags and zero mismatches; §1/§2/§5/§6 read end to end without contradicting each other — they cross-reference, so a partial edit would read worse than none. `config/features.json` and all three fixtures re-parse as valid JSON.
- **Scope guardrails, verified by diff:** **0** files under `mobile/`; **0** changes to `backend/analytics_taxonomy.py`, `backend/analytics_queries.py`, `backend/accounts.py`, `backend/database.py`. No DDL, no route added or contract-changed. `email_captured` has three hits in `backend/`, all **pre-existing on this branch and all negative** — a taxonomy comment recording the AN-6 cancellation and two assertions in `test_events_api.py` pinning that the name is registered nowhere.
- **`npx tsc --noEmit` NOT run — and deliberately not.** Zero TypeScript files are touched by either commit, and the standing instruction for this worktree forbids `npm install`. Recorded rather than skipped silently.
- **Sim run: none.** Per [D-P1-08](../docs/plans/audit-p1-remediation/DECISIONS-p1.md) the simulator apparatus is retired; independently this is **tier 4** (backend/web/docs, zero `mobile/src`), which requires no sim run under the matrix in [`../docs/runbook.md`](../docs/runbook.md). **No `last-sim-run.json` written — not fabricated.** `githooks/pre-push` only blocks `mobile/src` diffs, so nothing here was enforced; the tier-4 declaration is an honesty obligation, which is why it is logged.
- **NOT verified, and the list matters more than usual because merging begins real collection:** no real Apple sign-in was exercised. Owed on the first deploy — (a) a fresh Apple sign-in writes `email` + `email_source='apple'` + `email_consent_at`; (b) a **"Hide My Email"** sign-in stores an `@privaterelay.appleid.com` value and nothing downstream errors; (c) a re-auth of an existing Apple account whose `email` is NULL either backfills or does not — **the probe that would have predicted this was cancelled by the operator (PV-1)**, so this is now the *first* observation of that behaviour and a surprise here is a finding, not a contradiction; (d) a Sleeper-username sign-in creates no `accounts` row; (e) `/privacy` renders on the deployed URL. **Professional legal review of the policy did not happen** (D-P1-06).
