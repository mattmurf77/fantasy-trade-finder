## 2026-09-17 — Shared trade significance, production release and round-two review

PR #297 merged as `b33b4da`; all four hosted CI jobs green, backend **6058 passed / 1 skipped**. Render exact SHA live at 05:55:05Z. Admin read→write→readback confirmed only `significance_mode` changed 0→2 at 05:55:58Z; owner-only serving and unbounded deck/group settings unchanged. Scoped production replenishment job succeeded for owner's FFv3/Newton leagues; all **2173** new non-ghost impressions recorded enforce/eligible/discovery. Separate 100-card HTML review at localhost:8880, 50 per league, 11 local tests passed, CUA Yes/Undo/low-impact reason checked in non-persisting QA mode, normal view 0/100. Original 32 feedback records preserved. No TestFlight build needed; no production research dispositions or notifications. See [release evidence](../docs/plans/trade-significance/release.md).
