...
## 2026-09-03d — D-178 (FB-418 backend half): the idea routes adopt the deck's awaiting-like exclusion — built on branch, full gates

Branch `feat/fb418-backend-like-exclusion` @ `f13dd96c` (session tree `happy-golick-345cf1`, branch `claude/new-user-feedback-06dabd`). **Not merged, not deployed.** Full gates (operator: *"the full gates plus the API reference update"*) — no express. `FTF_SKIP_SIM_GATE=1` posture per D-056; backend-only behavior, so the evidence below replaces the retired sim gate.

- **What changed:** `POST /api/trades/asset-ideas` and `POST /api/trades/fair-packages` now build the deck's own windowless awaiting-like / `pending`-`accepted`-match exclusion set (`server._load_presentment_exclusions`, G6 R4 #336) under the EXISTING `trade.presentment_rules` flag, and drop every idea whose `(frozenset(give), frozenset(receive))` key is in it. Filter applied at EMISSION inside both generators — before `asset_ideas_group_cap` and before `fair_packages_cap` / the #189 strict-relaxed choice — so a capped answer still fills. One key constructor (`trade_service.presentment_key`), one loader; the asset-ideas `_dismissed` predicate was widened to `_suppressed` so the D-067 dismiss memory and the D-178 like memory cannot drift.
- **pytest `backend/tests`:** **4599 passed / 1 skipped** (415 s). Collection **4600 vs 4588** with the two test files reverted — exactly the +12 added, none lost. Baseline of record is the 2026-09-03b entry's **4587/1**; the 4565/1 in the 2026-09-03 #418/#417 entry pre-dates the resync onto `ca5fac46` and is not this branch's baseline.
- **Red-proof, 12/12** (table + per-test sabotage docstrings in [`backend-prd.md`](../docs/feedback/items/418-shop-send-dismiss/backend-prd.md) §7). **7 RED against the unfixed routes** (`git checkout --` on `server.py` + `trade_service.py`, tests kept): sent-offer-absent ×2 surfaces, retracted-like-returns ×2, cap-refills ×2, live-match-excludes. **5 are posture/regression bars that cannot be red on the baseline** and were proven by targeted sabotage of the fix: S1 flag guard removed ⇒ both `flag_off_is_byte_identical` fail; S2 the load hoisted out of the loader's `try/except` ⇒ both `serves_unfiltered` fail with the injected `RuntimeError`; S3 `_suppressed` narrowed to the like set ⇒ 4 dismiss tests fail incl. `test_route_dismiss_behaviour_is_unchanged`.
- **Fixture honesty (2026-08-30b lesson):** every like written by the shipped `POST /api/trades/queue`, every retraction by the shipped `POST /api/trades/awaiting/dismiss` (#318), and the `league_members` snapshot persisted through `upsert_league_members` — the production writer. Without that snapshot `load_awaiting_trades` cannot resolve the counterparty and silently drops the like, so a test that skipped it would have proved nothing. `test_asset_ideas.py` gained a `route_db` fixture: the route now READS the DB, so the route tests are hermetic instead of touching `data/trade_finder.db`.
- **Mobile:** one COMMENT correction in `ShopOffersBody.tsx` (the "there is NO server-side memory for a queued like" sentence became false). `npm run test:shop-deck` **153 PASS** (k8's three named `#418` sites intact) · `npx tsc --noEmit` clean. No behavior, no testID, no structural-suite delta.
- **Code-walk proof:** `backend-prd.md` §6 — send → `save_trade_decision` (`server.py:13371`) → `load_awaiting_trades` → `_load_presentment_exclusions` (`:5811`) → route build (`:12352-12362` / `:12604-12609`) → `_suppressed` (`trade_service.py:5526`) at `:5540`/`:5591`/`:5713` → cap at `:5848`; and the fair-packages mirror at `:5992` → cap at `:6058`.
- **Docs:** `docs/api-reference.md` both route rows (response semantics; the now-false "a LIKE never excludes here" clause deleted) · `docs/config-reference.md` flag row + the R4-bypass paragraph (the two new consult sites are deliberately outside `r4_bypass()`) · scope block + PRD in the item folder · D-178 status moved to built-on-branch.
- **Owed:** CI on the PR; the operator's 3-step TestFlight check (send → reopen the window → gone; retract → back); the prod `shop_opened` → `visibleIdeas.length` volume read D-178 asks for.

### QA-resolution pass, 2026-09-03e (same entry; both QA agents PASSed, findings closed)

Session tree `happy-golick-345cf1`, branch `claude/new-user-feedback-06dabd`, from `77a4e33b`. Still **not merged, not deployed**. Full gates again — this pass IS an API contract change (two additive response fields), so the bright line applies and no express was declared.

- **What changed, four things.** (1) **QA-B B-1, full parity:** both idea routes now also consult the deck's `like_days` = 7 LIKE subset of `past_decision_keys`, carried on the live `TradeService` (`recent_like_keys()`) and unioned inside the SAME loader. R4 alone was not the deck's whole like memory — `load_awaiting_trades` subtracts a like the moment ANY match row exists (status-unfiltered) and `load_matches_for_exclusion` re-adds only `pending`/`accepted`, so a **declined** offer was in neither set and returned to the shop at once while the deck held it a week. (2) **QA-A A-1:** the `_emit_best` variant pre-filter and the downgrade-combo skip are now pinned. (3) **QA-B C-4:** `asset-ideas` returns additive `excluded_count` + `excluded_by_group`, `fair-packages` returns additive `excluded_count`, and both log the set size + drop count per request. (4) **QA-B B-5/B-3/B-2/B-6 (mobile):** the queued ✓ invalidates the `shop-ideas` cache rows (`refetchType: 'none'`), the Same-value auto-widen stands down on an exclusion-caused zero, and two empty states say *"you have offered every …"* instead of blaming the market — each branch reachable only on a non-zero server count.
- **pytest `backend/tests`:** **4605 passed / 1 skipped**. +6 over the first build's 4599/1, nothing lost. The two D-178 files alone: **107 passed** (101 → +6).
- **Red-proof, 6/6, four named mutations of the FIXED code** (the code they pin did not exist at `f13dd96c`, so a baseline revert is not the honest red here). **Q-B** — `_emit_best`'s variant filter and the downgrade-combo skip narrowed to `_dismissed_decision_keys`, the `_emit` backstop intact ⇒ **1 failed, 106 passed**, and only `test_an_excluded_variant_yields_its_slot_to_the_runner_up`, which is exactly QA-A's point: that mutation was invisible to the entire 4600-test suite before. **B-1-off** — the routes drop `trade_service=` from their `_load_presentment_exclusions` call (the shipped `77a4e33b` behaviour) ⇒ **2 failed, 105 passed**, both `test_a_declined_offer_stays_suppressed_like_the_deck`. **set-size** — `excluded_count = len(exclusion_keys)` ⇒ **2 failed, 105 passed**, both count tests (each seeds a second, unrelated like that can never be a candidate). **flag-gate-removed** — `if True:` at both new call sites ⇒ **4 failed, 103 passed**: the two new count tests plus both pre-existing byte-identity bars. All mutations restored; suite re-verified green after each.
- **Fixture honesty:** the declined-offer tests drive the production writers end to end — the shipped `POST /api/trades/queue` for the like, then `create_trade_match` + two `record_match_disposition` calls to reach status `declined` — and each asserts `_load_presentment_exclusions(user, league) == set()` BEFORE asserting the idea is still absent, so the test proves R4 genuinely holds nothing rather than assuming it. The `like_days` subset is rebuilt in-test the way `session_init` builds it, from the same `load_trade_decisions` read.
- **Mobile:** behaviour changed this time (the invalidation), so `check-shop-deck` gained **k10** — the `invalidateQueries` call in the same `res.queued` branch k4 gates, with the `['shop-ideas', …]` key and `refetchType: 'none'`. **154 PASS** (153 + k10); RED with the call deleted (`no invalidateQueries call in the queued branch`), green restored. `npx tsc --noEmit` clean · `bash scripts/testid-lint.sh` **OK** · k8's three `#418` sites intact · no testID added or renamed.
- **Docs:** `docs/api-reference.md` both rows (B-1 parity, the C-4 fields with an explicit statement of what they do and do not do to flag-off byte identity, the log line, A-2's softened cap claim) · `docs/runbook.md` presentment-rules kill procedure (killing the flag also un-fixes #418; prefer the per-rule knobs; the new log lines are the only tripwire) · `backend-prd.md` R-10…R-13 + §10 finding-by-finding disposition + §8 named follow-ups · `backend-scope.md` §3/§4/§5/§6 · D-178 consequences.
- **Owed, unchanged plus:** the 3 added TestFlight steps in `backend-scope.md` §3 (the 30-second reopen, the declined round-trip with a second account, and the honest-empty screenshot). Deliberately NOT built and named in `backend-prd.md` §8: QA-B B-4 (a #417-surface item), B-7's client `already_queued` prop, B-9, C-1, and C-3 (a re-send affordance — **needs an operator ruling**).
