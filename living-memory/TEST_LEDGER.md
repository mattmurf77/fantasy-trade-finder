...
- **`pytest backend/tests/test_entry_platform_route.py`:** **25 passed** (23 + 2 new: `test_mfl_entry_leagues_snapshot_carries_the_claimed_franchise`, `test_espn_entry_leagues_snapshot_carries_the_claimed_team` — pin the `GET /api/{espn,mfl}/leagues` contract the web's `buildPlatformRosterData` reads). Full suite `pytest backend/tests`: still running at write time — the PR's `backend-tests` job is the record (only a test file changed on the backend side).
