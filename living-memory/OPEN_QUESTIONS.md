...
- **Owner:** operator · **Asked:** 2026-09-02 · **Resolved 2026-09-08 (D-189):** premise was wrong — Sleeper's element is `{roster_id = original, previous_owner_id = from, owner_id = to}` and both runbook captures were acquired-pick trades with orig ≠ from; field 1 = original roster confirmed from public `traded_picks`. Closed.
...
- **Why it matters:** [`docs/reviews/2026/2026-08-22-trade-model-restrictiveness.html`](../docs/reviews/2026/2026-08-22-trade-model-restrictiveness.html) ranks eleven fixes, and three of them are product or budget decisions an agent must not make alone. The review is read-only; no engine line changed.
...
> Full detail + defaults: [`../docs/plans/archive/2026/perf-optimization/artifacts/questions-for-user.md`](../docs/plans/archive/2026/perf-optimization/artifacts/questions-for-user.md).
...
### Q-036 — #414: the card survived a gap the overpay gate should have killed — which arm/path served it?
- **Resolution (2026-09-02, read-only prod `deck_impressions` query, operator go-ahead):** **the main engine, and the gate was right.** Four impressions of exactly the card, 2026-08-27 → 08-29, league `1312140920132497408`, `model_arm = current`, `basis = consensus` (MangoPatti had **no** published board — the operator's guess that his rankings drove it was wrong), shape 1x1, `fairness_threshold` 0.75. Give Drake London **5,932.8** / receive CeeDee Lamb **7,328.9** (the other impressions 5,962.6 / 7,284.3 and 5,989.5 / 7,398.9). Gap ≈ **1,396 = 19%** of the richer side — R1 `overpay_ok` passes it correctly (0.19 < 0.25). `gap_sweetener: None`, `need_fit: 0.5` (WR-for-WR — the identically-0.5 shape). The bar's "over a first" was the badge unit (a late/current first), which is smaller than the sweetener's flat **1,539** floor — so the app labelled the gap a first in one unit and declined to close it in another. **Explanation (a)**, not (b): no arm bypasses R1; the two experimental arms are not implicated by this card.
- **What it decides:** the #414 fix is exactly the relative-band + best-effort sweetener (`docs/plans/sweetener-relative-band/`): at floor 750 / frac 0.12 the effective trigger on this card is max(750, 0.12 × 7,329) = 879 < 1,396, so it fires; `filler_ok` at 0.15 needs an equalizer ≥ ~890, which closes the gap to ~500. The card also re-served four times in three days with no decision recorded — legitimate under the 7/14-day windows, but it is the #350 "same card again" feeling with a different cause.
- **Lesson:** the arithmetic that made this look like a gate bypass assumed one unit for "a first" (the D-161 current-year floor, 2,185) when the bar uses another. Two units for one word cost an hour of speculation that one row settled — the same shape as [G-064] (the record cannot tell you; read the row).

### Q-035 — Does #350's "that player's valuation is way too high" mean FTF's consensus seed, or the user's own board?
- **Resolution (2026-09-02, operator, from memory — "not positive"):** **the consensus price.** The recurring player was **Davante Adams**; the operator's own board had him well below consensus, and the discomfort was not the repetition itself but *not feeling confident selling him as a piece* while the app priced him at consensus. The operator then reasoned through it himself: "if I am lower on Davante Adams than consensus, it actually does make sense that he continues to show up in trades — he is one of the guys I should be looking to trade based off of the market valuation versus my own." **That is the divergence thesis working as designed**, and the lead agrees: a player the user rates below the market is exactly the engine's sell piece.
- **What this closes and what it leaves:** the repetition half is addressed by the deck caps that shipped *after* #350 was filed (C4 `deck_headliner_cap` 2 on 2026-08-18, C4b `deck_give_headliner_cap` 3 on 08-19; #350 is dated 08-17) — and because the operator means the **consensus** price, those caps are the right instrument (they key on `_job_seed_elo`); the `shrink_pseudocount` branch of this question is moot. The confidence half is a **presentation** gap, not a model one: a card built around a give-side player the user rates below consensus should *say so* on the card (the arbitrage is the reason the piece is there). Candidate: a deterministic `trade_narrative.py` line keyed on user-vs-consensus divergence of the give headliner. Not built; operator to say whether he wants it.
- **On the operator's broader idea — "combine consensus with personal rather than two separate rank sets":** this already exists as `_shrink_user_elo` (`trade_service.py:~1740`): the user's board is a blend `w = n/(n+shrink_pseudocount)` of personal and consensus *per player by comparison count*, so an un-compared player sits at consensus and a well-compared one keeps the personal value. What the operator is imagining — a single global blend — would erase the divergence signal the product is built on (two identical blended boards have no arbitrage). The lever that *does* exist and is untuned is `shrink_pseudocount` (4): raising it trusts consensus more for lightly-compared players. Adams is not that case — the operator had ranked him with confidence, which is why the engine believed him.
- **Lesson:** the question was framed as "which lever fixes this" and the answer was "the user was right to be uncomfortable, and also the model was right" — the fix is telling the user why the piece is there, not changing which piece it is.
...
- **Resolution (2026-08-19, [D-088](DECISIONS.md)):** **No.** The badge was a wrong inverse, not a price. `GET /api/league/picks` inverted `pool_value` (stored in `elo_to_value` units) with `data_loader.seed_elo_for_value`, which inverts DynastyProcess's raw 0-10000 scale instead. The two maps agree at exactly Elo 1548.0 and diverge either side, inflating every rung below a mid-1st — Mid 3rd 1320 → **1383.5** (+63.4), Mid 4th 1240 → 1339.3 (+99.3) — so 1383.5 cleared D-084's new `second` floor of 1370. The pick's real price is Elo 1320, **45 points inside `third`**. Fixed with `trade_service.value_to_elo`; no seed, band, client mirror or stored price moved. Memo: [docs/reviews/2026/2026-08-19-pick-badge-scale.md](../docs/reviews/2026/2026-08-19-pick-badge-scale.md).
