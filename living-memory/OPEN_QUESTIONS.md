...
- **Direction answered (2026-09-02, operator):** the user was **giving Drake London** (badged "worth two firsts") and **receiving CeeDee Lamb** (badged "three firsts"); the value bar said the user was **winning by over a first**; the counterparty was MangoPatti, who *does* have a board — the operator suspects that board drove the card. So this is the **benign** branch: the user wins on consensus, the sign test passes, and `user_gain_ok_1for1` was never the issue. The sweetener diagnosis stands — the user's ask ("there are players I can add to make it more fair") is exactly what `close_value_gap` exists to do and is structurally unable to reach.
- **But the arithmetic does not close.** If "over a first" means the current-year first (≈ 2,185 after [D-161]), then at *any* CeeDee price up to the #1 asset in the game (≈ 7,737) the gap ratio is ≥ 0.28 — above R1's 0.25 kill line — so `overpay_ok` should have killed the card before any sweetener ran. Two explanations, and only the card record distinguishes them: **(a)** the bar's "first" unit is the late-1st line (1,539) or the pick badge tier, in which case the ratio sits at 0.20–0.24 and R1 passes but the sweetener window is the empty/near-empty one already diagnosed; **(b)** the card came from a path that does not run R1 — arm D (`challenger`, both-directions with a 0.75 floor: a 0.77 fairness passes it) or arm C (`gen_v2`, own sweetener call, own gates), **both serving in prod** ([G-064]). If (b), users are being served overpay cards the main engine forbids, which is a different and larger finding than #414.
- **Action to unblock:** read the card from `deck_impressions` for the operator's account around 2026-08-31 (give ∋ Drake London, receive ∋ CeeDee Lamb, league with MangoPatti): `model_arm`, `basis`, `give_value` / `receive_value` / `gap`, `fairness_score`. Read-only; needs `DATABASE_URL_PROD` — the admin API has no per-card lookup. **Operator go-ahead requested 2026-09-02.**
- **Discovered by:** 2026-09-01 dual-agent review; direction settled by operator 2026-09-02; re-scoped by the lead on the R1 arithmetic.
- **Owner:** lead (the read), then whoever owns the sweetener fix.
