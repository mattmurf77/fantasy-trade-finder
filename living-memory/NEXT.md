...
6. **Merge trade-relevance P0, then graduate or kill `deck.value_model`.** *(branch is the immediate item; the flag decision is P1)* **P0 is BUILT** on `origin/feat/trade-relevance-p0` @ `f6ee573` — 2938 passed / 1 skipped, both user-affecting flags dark, unmerged. Next actions in order: open the PR (owes CI green + a TEST_LEDGER-recorded tier-4 sim declaration + `FTF_SKIP_SIM_GATE=1` on push, all argued in `docs/plans/trade-relevance-engine/scope.md`); merge; **soak the ledger ≥3 days before flipping anything** (it refactors live push-sending code); then `deck.dedup` ON alone, then `flag_agg` dark ≥7d → operator reviews the demoted-class report → `deck.class_demotion` ON alone. Only after that does P1 start: the signed-off D4 criterion (pinned artifact, 21 counted nights, symmetric kill) + the `train.value_model` flag split replace ad-hoc gate-reading. Full state in [`HANDOFF.md`](HANDOFF.md); the operator decision queue that gates P2+ is at the end of `../docs/plans/trade-relevance-engine/reconciliation-log.md`.
