...
3. **Knockout programme — R5/R1/R2/shape SHIPPED 2026-08-24 (D-159; prod-confirmed applied 2026-09-01, G-064). `consensus_fit_weight` SHIPPED LIVE 2026-09-02 (D-172).** Remaining, re-ordered by the 2026-09-01 research: (a) prod verification of D-172 per HANDOFF; (b) fix G-066 then read arm D's basis-split like/match rate — the "partner wins on value" question; (c) sweetener best-effort fallback + relative band (#414) — knob alone regresses; (d) divergence pool-fit at N≤2 with the junk-stuffing fix (prototype @ `60aa4572`); (e) `need_fit_score` shape fix (identically 0.5 on same-position swaps; one-sided saturation) before any weight change; (f) `tier_up` value floor ≈1490 + sink unranked below waivers (#363). Dropped: `need_gate_upgrade_margin` (measured inert). Audit `waiver_slot_cost` = 425 alongside (c).
