...
0h. **DONE 2026-08-13 (built, awaiting operator ship confirmation — bright-line taxonomy change).** Dropped-emitter backlog cleared on branch `claude/elegant-mccarthy-ef63f8`: 27 names registered as-shipped (+8 NON_INTENT rows), `quickset_completed` client emitter removed per the namespace-disjointness rule. Addendum: [`2026-08-13-dropped-emitter-backlog.md`](../docs/business/analytics/2026-08-13-dropped-emitter-backlog.md). Backend suites green (243 tests); mobile typecheck owed at merge (no node_modules in worktree). [G-031]
