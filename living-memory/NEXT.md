...
3. **Knockout programme + trade-model review — now ONE measurement stream (2026-08-27).** The prod bundle is already live (`filler_min_frac` 0.15, `trade_elo_gap_max` 0, `v3_shape_max_delta` 2 flipped 2026-08-24; R5 dual-rescue + R2 starter-relief lit at seed) — but `overpay_adjusted` went **0.0 in the same batch**, reverting D-159's R1 currency change with no record: **[Q-034](OPEN_QUESTIONS.md)**. The [2026-08-27 review](../docs/plans/trade-model-review/champion-recommendation.md) recommends: champion stays `current`; hold the interleaved read stable (~5–7 weeks to a powered per-arm read, conditional on the [Q-031](OPEN_QUESTIONS.md) call — preference: keep interleave, drop `bakeoff_include_gen_v2` to 0). **Any further knockout flip (`filler_min_frac` 0.10, R2 starter-depth predicate, viewer-must-win Q-030a) restarts the arm clock — sequence behind the read or an explicit operator trade-off.** Candidate build items the review surfaced (full gates each): give-side far-first exposure treatment (H1: 9.3% like), divergence-side partner-anchor insult rule (H8), SF/TEP de-conflation + QB pricing settlement (H5). Audit `waiver_slot_cost` = 425 alongside.
