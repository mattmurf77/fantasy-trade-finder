...
**Consequences:** Boarded members can now carry `basis:"consensus"` cards — a new but already-instrumented combination on the existing deck spine. Because `global_target` caps the deck at 30 and boarded members are visited first, rescuing them **displaces** consensus cards from unranked members; the deck's composition improves, its size does not change. A single multiplicative factor cannot undo a *nonlinear* compression, so calibration alone did not rescue MangoPatti or Bcork — quantile-matching is the candidate follow-up ([Q-017](OPEN_QUESTIONS.md)). Both flags are dark; flipping either is deploy-free and each is its own kill switch. Flag off ⇒ engine byte-identical, pinned by paired tests in `backend/tests/test_compressed_board.py`.
