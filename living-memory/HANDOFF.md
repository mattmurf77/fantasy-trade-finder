...
## 2026-09-03b — API audit fixes on PR #273 (D-178); finding 3 held, web push deferred
...
**Owed:**
1. **Operator TestFlight checklists on build 140** — #407 mini-PRD **6 steps** (step 6 = the now-closed B-1 probe: after Clear, expect ALL teams) + #406 PRD **§E-4, 8 steps** (step 8 = the documented zero-result canvas-wipe corner). The only runtime evidence under D-056, and nothing in this batch has been seen on a device.
...
## 2026-08-29c — NULL-dwell decline-pass fix (client half of the #242 guard) SHIPPED via PR #246

**Where:** `main` @ `293b5f80` (PR [#246](https://github.com/mattmurf77/fantasy-trade-finder/pull/246), squash on green CI ×3). Client-only companion to the #242 `deck_pass_outcome_recorded` guard: the layer-1 decline reason POST lands before the swipe POST and now carries the same `dwell_ms`/`detail_expanded`/`calc_opened` the swipe sends, so the surviving pass row keeps its dwell. Three files: `mobile/src/api/declineReasons.ts` (optional fields + wire mapping), `mobile/src/screens/TradesScreen.tsx` (`handleReasonLayer1`, first-tap-only), `mobile/src/hooks/usePresentationSignals.ts` (same defect on the presentation-v2 surface, same fix). Evidence + code-walk: TEST_LEDGER 2026-08-29e. Branch swept by content (recovery: [docs/recovery/2026-08-29-null-dwell-decline-pass-ship.md](../docs/recovery/2026-08-29-null-dwell-decline-pass-ship.md)).

**Owed:** (1) worktree sweep from the main checkout — `.claude/worktrees/zealous-mirzakhani-f1664e` could not remove itself (branches deleted + ledgered, nothing uncommitted). (2) Mobile-only change: reaches users with the next EAS build, nothing to flip; until an installed build carries it, layer-1 reasoned passes keep writing NULL dwell. Runtime check for that build's TestFlight pass: one `deck_outcomes` pass row per reasoned pass, non-NULL `dwell_ms`.
...
## Table of Contents
- [2026-09-03b — API audit fixes on PR #273 (D-178); finding 3 held, web push deferred](#2026-09-03b--api-audit-fixes-on-pr-273-d-177-finding-3-held-web-push-deferred)
