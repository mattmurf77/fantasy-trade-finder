...
- **SHIPPED: TestFlight build 22 (v1.3.0), auto-submitted 17:39 (build id `50a68ed1`).** EAS build from `trade-engine-v2`; `autoIncrement` 21→22; uploaded to App Store Connect, Apple processing. **Backend NOT yet deployed** — the merge (PR #93 → main) was blocked by the auto-mode classifier as an unauthorized prod deploy while the operator was away; left MERGEABLE/CLEAN for a one-click human merge. **Until #93 merges, Dismiss 404s on-device** (rolls back with a toast, no crash); Send in Sleeper works (its endpoint shipped in build 21). Next session / operator: merge #93 → Render deploys the dismiss route (~3 min) → feature complete.
