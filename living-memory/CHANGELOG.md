...
## 2026-08-27c — Outlook/age plan authored (dual-agent, 4 rounds): pick-flow steering is the build, age bias is a hold recommendation

Follow-on to the trade-model review, at operator direction ("mix outlook into the engines… users get more picks offered and give up picks less if rebuilding… plan out adding age bias"). Three new grounded docs in [docs/plans/trade-model-review/](../docs/plans/trade-model-review/): **outlook-age-grounding.md** (F1–F6 measured: consensus already encodes the RB-27/WR-30 age cliffs; user boards lean *against* the youth premium, u23 Δz −0.080; the rebuilder pick-flow inversion — asked to GIVE picks 43.9% vs receive 11.1% — existed with the `outlook_direction` reranker both ON and OFF, so it is generation-level; the 08-20 flip-off experiment's own metric, read out at last, argues against re-lighting #175 as-is), **outlook-age-code-map.md** (the dark machinery: `outlook_blend` curves/α, `outlook_direction`, classifier flags, the age-default-25 hydration hazard — measured ~zero blast radius today, and the fact that BOTH sweetener passes can draft a viewer's pick as currency under `picks_in_pool`), and **outlook-age-plan.md** — the execution plan, dual-agent drafted and adversarially reviewed to joint sign-off (4 rounds; recon log in-file). Shape: WS2 builds generation-level pick-flow levers dark behind 0-default knobs (give-pick demotion A1 + global far-first demotion A2 + both-seam sweetener bias B; deck quota C as backstop) with `(outlook, source)` plumbing and a declared-only-default scope knob; WS3 recommends **holding** `outlook_blend` dark (F1/F2 — lighting it would also reverse the 2026-07-17 age-tiebreak ruling; overturn is an explicit operator decision); WS4 wires the composite classifier into the engine as the inferred-coverage prerequisite; everything flips batched at the arm-read round boundary, once.

## 2026-08-27b — Trade-model review: the community's gripes about KTC, tested against OUR model — champion stays `current`, and the biggest finding is give-side far firsts

Executed the operator-directed review plan (handed off from the app-launch-scorecard session; plan + all four deliverables in [docs/plans/trade-model-review/](../docs/plans/trade-model-review/)). Docs-only; prod read-only via the deck-eval mirror method. **Champion recommendation: keep the incumbent engine; no arm separates in the live interleaved read (current 15/32, challenger 33/74, gen_v2 19/47 — ~5–7 weeks to a powered read).** What the data DID say: (H1) cards asking users to **give a far-year 1st are liked at 9.3%** vs 38.8% player-only — the D-079/D-161 flat-firsts ruling makes the engine spend far firsts as currency, and gen_v2 does it on 46.4% of its cards; fix is exposure/presentment, not repricing (rulings stand). (H7) divergence-basis cards do **not** outperform consensus for any user — the core pitch is unproven at current n (Q-030's match-rate grading basis still unmeasurable at 15 matches). (H8) insult guardrail on the comparable population: consensus-basis first-5 **1.48%, identical to 08-15, PASS**; divergence cards carry an 8.18% consensus-haircut exposure needing its own partner-anchor rule. (H3 inverted) users, not the model, are youth-biased — recv-u23 47.8% vs recv-30+ 15.0%. (H4 inverted) our risk is staleness (≤daily refresh) and deploy-driven value jumps, not knee-jerk volatility. Empty-deck August 5.05% — sitting on the gate. Live-config diff surfaced `overpay_adjusted` flipped OFF in the same batch as the D-159 bundle with no record → **[Q-034](OPEN_QUESTIONS.md)**; interleave-hold recommendation made conditional on the pending Q-031 call. NEXT's knockout item and this review are now one measurement stream.
...
`feat/jon-360-362` — built dark 2026-08-19, held unmerged a week — merged onto current `main` (123 commits of drift; 22 conflicts in 18 files, plus 3 semantic fixes the conflicts did not flag). ID de-collision: D-098→**D-166**, Q-026/027/028→**Q-031/032/033**, G-053→**G-062**, M-005→**M-006**, plus five PRD-local decisions renumbered item-scoped; the ship ruling is **D-165**.

**The decision ID had to move THREE times in one day**, which is the finding, not an aside. A concurrent session shipped the entry-page work in three waves while this branch was gating, taking D-163 (#210), then D-164 (#213) — each time claiming the number this branch had just renumbered *into*. Sequential IDs in one shared file are a structural race between concurrent sessions, and M-006's rule ("grep for the max immediately before writing") is necessary but **not sufficient**: the gap between grep and merge is minutes of CI, and that is long enough to lose. Worth a real fix — reserve-on-branch, or an ID allocation that does not depend on merge order.
...
## 2026-08-26c — Entry v2.1: "sign in" as a first-class option beside "enter a league ID"

Operator, after seeing v2 live: users should be able to LOG IN to ESPN/MFL rather than only entering league ids. Built by an **Opus subagent**, reviewed line-by-line and shipped by the lead session. `POST /api/entry/platform` gains two sessionless account-discovery actions that **store nothing**: ESPN `my_leagues` (fan-profile list for the WebView-captured cookie pair, byte-identical to `GET /api/espn/my-leagues`; gated `espn.league_picker`) and MFL `auth_leagues` (the same login+myleagues pair `/api/mfl/auth-link` uses; gated `mfl.auth_link`; password transient, never logged). ESPN entry: "Sign in to ESPN" is a first-class button — capture with no league id typed lists your leagues in the existing picker. MFL entry: "Sign in with MFL" lists leagues **with your franchise already known**, so one tap mints directly (no team-claim step) → canonical import → best-effort credential re-store under the fresh session (Send-in-MFL works later; password held in a ref, dropped before that one call). Linked flows byte-identical (the MFL sign-in JSX renders from one extracted block at its original position). Evidence: entry pytest 13→23 (with a stored-nothing assertion helper), guard 36→61, full suite green, tsc/lint clean; subagent sabotage-proved both flag gates. Scope/code-walk §V2.1.

## 2026-08-26b — Platform entry decoupled from Apple: claim your team, no account (D-164)

Operator follow-up on the morning's D-163 ship: "decouple the Apple account dependency from ESPN and MFL." Done the way Sleeper already works — claim-based. New sessionless `POST /api/entry/platform` (gated on `landing.platform_options` + `espn.link`/`mfl.link`): preview a league in the link routes' exact wire shapes, then mint a session at the team claim via the same `_extension_build_session` behind `/api/extension/auth`, keyed to a **deterministic** id (`entry:espn:<SWID>` / `entry:espn:<lg>.t<n>` / `entry:mfl:<lg>.f<id>` — distinct from the never-routable `espn:`/`mfl:` placeholders) so re-claiming a team recovers the same boards. The canonical `/api/{espn,mfl}/link` import then runs under the fresh token — zero duplicated import logic. Client: `EspnLinkSheet`/`PlatformLinkSheet` gain an `entry` mode (preview transport swap, mint-at-pick, session-dependent paths suppressed), hosted by SignInScreen's panels — the Apple button left the panels; private ESPN leagues sign in to **ESPN** via the signed-out-capable WebView capture. Entry user pinned `account_only` → LeaguePicker's platform merge + auto-skip finish activation. `signin_*` gains method values `espn`/`mfl` (addendum doc). Evidence: 13 new pytest (incl. mint→canonical-import end-to-end), guard extended to 36 assertions, tsc + lint green. TestFlight checklist v2 owed.
...
Two record corrections came out of the release, both in living memory now: (1) **G-012** — the `MARKETING_VERSION` vs `Info.plist` gap is NOT drift; `INFOPLIST_FILE` is set and `GENERATE_INFOPLIST_FILE` is not, so the checked-in plist's literal ships and the 2026-08-24 HANDOFF was right to leave it stale (we still sync both, as trap removal). (2) The local `test_deck_signal_v2` failure is the **stale `data/trade_finder.db`**, not Python-version skew as this session first claimed — `git stash` doesn't touch a gitignored DB, which is why the clean-tree check looked exonerating. Also fixed en route: the Aug-25 `season_start` branch made three winback tests fail exactly one day a year ([G-061](GOTCHAS.md)).
